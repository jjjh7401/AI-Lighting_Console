"""t299 Phase 2 프로브 — 확대 전/후를 같은 명령으로 잰다.

`server/safety/` 를 고치지 않는다. 배치된 `blacklist.yaml` 을 그대로 읽으므로
확대 전에 한 번, 확대 후에 한 번 돌리면 두 회차가 바로 대조가 된다.

Phase 1(`probe_p1.py`)과 다른 점은 대상 구분뿐이다. 이 회차가 넣는 것은
`Store Sequence` 하나이고, `Store Cue` 는 Phase 3 이므로 **열린 채로 남아야
한다** — 그것을 확인하는 것도 이 프로브의 일이다.

실행:  uv run python reports/classifygap-t299-p2/probe_p2.py
"""
...
#: Phase 2 가 닫는 명령. 이 회차의 유일한 대상이다.
P2_TARGET = "Store Sequence 210 Cue 3 /Merge"

#: Phase 1 이 이미 닫은 둘. 그대로 걸려 있어야 한다(회귀 확인).
P1_CLOSED = ("Store Group 3", "Store Timecode 9")

#: Phase 3 대상. 이 회차에서는 **안 걸려야** 한다 — 걸리면 범위를 넘은 것이다.
P3_TARGET = "Store Cue 1"

#: 배차서가 못 박은 음성 대조군. 프로그래머 값이라 확대 뒤에도 안 걸려야 한다.
MUST_STAY_SAFE = "Fixture 1 At 50"
...
def _gate(approval):
    tmp = Path(tempfile.mkdtemp(prefix="t299p2-"))
...
def main() -> None:
    ...
    print("\n[A] 배차서가 지정한 다섯 명령 — 이 순서 그대로")
    for c in (*P1_CLOSED, P2_TARGET, P3_TARGET, MUST_STAY_SAFE):
        print(_line(c))

    print("\n[A-검산] 이 회차가 지켜야 하는 세 성질")
    p2 = _classify(P2_TARGET)
    p3 = _classify(P3_TARGET)
    safe = _classify(MUST_STAY_SAFE)
    print(f"  Phase 2 대상이 걸린다        : {p2.risky} (matched={p2.matched_entry!r})")
    print(f"  Phase 3 대상은 아직 열려 있다: {not p3.risky} (matched={p3.matched_entry!r})")
    print(f"  프로그래머 값은 안 걸린다     : {not safe.risky} (matched={safe.matched_entry!r})")
