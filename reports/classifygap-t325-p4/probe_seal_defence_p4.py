"""t325 Phase 4 — `SEAL_DEFENCE` 귀속을 **재서** 표를 채운다.

Phase 2 가 자리를 10 -> 11 로 늘리고 seal-only 를 7 -> 2 로 줄였다. 이 회차는
`Store Cue` 를 더하므로 그 귀속이 또 움직일 수 있다 — **손으로 고치지 않고**
재서 옮긴다.

Phase 1 의 `probe_seal_defence.py` 는 「표와 실측이 같은가」를 물으므로 표에 없는
자리에서 `KeyError` 로 멈춘다. 이 프로브는 비교를 하지 않고 **측정만** 한다.
표에 없는 자리는 `(표 없음)` 으로 찍고, 마지막에 `SEAL_DEFENCE` 에 그대로 붙여
쓸 수 있는 형태로 전체를 출력한다.

실행:  uv run python reports/classifygap-t325-p4/probe_seal_defence_p4.py
"""
...
def main() -> None:
    ...
    for name, drive, _kind in SITES:
        tmp = Path(tempfile.mkdtemp(prefix="t325p4-seal-"))
