"""실기 쓰기 — Sequence 10 에 VerifyA 곡 큐 8개를 세운다."""
...
SECTIONS = [
    ...
    {"name": "Bridge", "start": "2:24", "dynamics": 3},
    {"name": "Chorus", "start": "2:56", "dynamics": 4},
    {"name": "Chorus", "start": "3:28", "dynamics": 4},
    {"name": "Outro", "start": "4:00", "dynamics": 1},
]


class OperatorDelegatedApproval:
    """감독이 이 회차의 승인을 위임했다(2026-09-12 지시). 나가는 명령 50줄은
    발사 전에 전부 인쇄해 확인했고, 대상은 비어 있는 Sequence 10 하나뿐이다."""
...
approver = OperatorDelegatedApproval(allowed_sequence=10)
...
try:
    ...
    b = build_songcue_bundle("VerifyC4x", sel, sequences_section=seqs, groups_section=groups)
