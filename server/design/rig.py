...
_LAYER_GROUP_ALIASES: Mapping[str, frozenset[str]] = {
    ...
    # 남긴다.
    #
    # 🔴 위 BLIND 유보는 **2026-09-15 감독 답으로 해소됐다 — `effect` 다.**
    # 블라인더가 객석을 향해 쏘는 것은 맞지만, 감독의 판단은 **운용 방식이
    # 스트로브와 같은 계열**이라는 것이다(상시 유지하는 광이 아니라 순간에
    # 터뜨리는 장비). 그래서 "blind"/"blinder" 를 여기 넣는다.
    #
    # 이 어휘의 근거는 코드도 문서도 아니라 감독의 연출 판단이다 — 뒤집으려면
    # 추론이 아니라 감독에게 다시 물어야 한다.
    #
    # 블라인더 안전 예산은 이 표와 **다른 층**이 지킨다: `server/design/lint.py`
    # 의 L14 가 곡당 2회·D5 전용을 강제하고, 그 판정은 큐 문면(`_AUDIENCE_TOKENS`)
    # 을 읽는다 — 이 역할 이름표를 안 본다. 즉 여기 어느 역할로 넣든 L14 는
    # 그대로 동작한다(같은 개념이 두 군데 있고 소비자가 다르다, 카드 t395).
    "effect": frozenset({"effect", "fx", "aerial", "beam", "strobe", "haze", "blind", "blinder"}),
    # 카드 t395 — "foh" 가 `key` 로 나갔다(위 주석). 2026-09-15 감독 답으로 마지막
    # 후보였던 BLIND 도 `effect` 로 갔으므로, 이 리그에는 `audience` 를 받는 그룹이
