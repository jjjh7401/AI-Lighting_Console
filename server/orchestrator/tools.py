...
class CommandOutcome:
    ...
    Patch-verification statuses (patch_fixtures only — the server never
    executed the command; the status is a re-read verdict):
...
#: **66.7 ms**, and the walk spends ``len(SPATIAL_FIXTURE_PROPERTIES)`` reads per
#: fixture. The cap therefore IS a fixture count:
#:   240 -> 60 fixtures (16.0 s) — the previous value
#:   320 -> 80 fixtures (21.3 s) — an 80-fixture rig measured on 2026-08-18,
#:          where the old cap refused every all-fixture aim/preset request with
#:          "3D 좌표 응답이 … 조회 한도에 도달" while every coordinate read itself
#:          worked (POSX/POSY/POSZ answered).
#: 480 (120 fixtures) would be 32 s and breaches the 30 s interactive budget, so
#: the partial-read signal is kept rather than raised further.
#:
#: The STRUCTURAL fix is fewer round trips, not a bigger ceiling: the responder's
#: child rows carry no ``fid`` and cap at ``max_children`` 24, so an 80-fixture
#: read also pays ~80 slot-recovery state reads. A batched spatial reply
#: (fid+POSX/POSY/POSZ per row, paged) would turn ~400 round trips into ~3.
...
#: Rows requested per ``props`` page. The responder trims a page that would
#: overflow its ~1900-byte transport and names the slot to resume from, so this
#: is an upper bound per trip, not a promise.
SPATIAL_BATCH_PAGE_ROWS = 40

#: Ceiling on ``props`` pages per spatial read. 40 rows x 40 pages = 1600
#: fixtures — far past any real rig — and it exists only so a responder that
#: keeps naming a resume slot can never spin here forever.
SPATIAL_BATCH_PAGE_CAP = 40

#: The batch asks for the container's own child NAME too, so a page carries
#: everything the enumeration needs and the slot-by-slot listing recovery
#: (one state trip per omitted slot) becomes unnecessary.
SPATIAL_BATCH_PROPERTIES = ("name", *SPATIAL_FIXTURE_PROPERTIES)


def _batch_fixture_properties(
    property_port: PropertyQueryPort,
    fixtures_path: str,
    child_count: object,
) -> dict[int, dict[str, str]]:
    """``{slot: {property: value}}`` from ``props`` pages — ``{}`` when unavailable.

    Returns an EMPTY MAP rather than raising for every "not here" case: a port
    without the method (an older wiring, a test double) and a responder older
    than 1.6.0 (which answers ``ok:false`` for an unknown verb) are both simply
    "walk instead". A page that fails MID-walk keeps what already arrived — the
    caller falls back per missing slot, and ``childCount`` still decides whether
    the read was complete.
    """
    reader = getattr(property_port, "query_properties", None)
    if not callable(reader):
        return {}
    values: dict[int, dict[str, str]] = {}
    slot = 1
    for _page in range(SPATIAL_BATCH_PAGE_CAP):
        try:
            reply = reader(fixtures_path, slot, SPATIAL_BATCH_PAGE_ROWS, SPATIAL_BATCH_PROPERTIES)
        except Exception:
            return values
        ...
        for row in reply.get("rows") or []:
            if not isinstance(row, dict):
                continue
            row_slot = row.get("i")
            row_values = row.get("p")
            if isinstance(row_slot, bool) or not isinstance(row_slot, int):
                continue
            if not isinstance(row_values, dict):
                continue
            values[row_slot] = {
                key: str(value) for key, value in row_values.items() if isinstance(key, str)
            }
        resume = reply.get("next")
        if isinstance(resume, bool) or not isinstance(resume, int) or resume <= slot:
            break
        if isinstance(child_count, int) and len(values) >= child_count:
            break
        slot = resume
    return values
...
def read_spatial_fixtures(
    ...
    *,
    ...
    # ── Batched read first (responder 1.6.0 `props`). ──
    # The per-child walk below costs one round trip PER PROPERTY at a measured
    # 66.7 ms, plus one state trip per slot the truncated listing omitted. On an
    # 80-fixture rig that measured ~400 trips (~26 s) and refused every
    # all-fixture request through the query cap; a 200-fixture rig cannot finish
    # inside any interactive budget at all. One `props` page carries many
    # children x many properties, so the same read costs a handful of trips.
    #
    # It is an ACCELERATOR, never a new source of truth: rows fill the same
    # per-slot map the walk would have built, gaps still fall through to the
    # per-property path, and `childCount` remains the completeness authority.
    # An older responder answers `ok:false` for an unknown verb and a port
    # without the method has nothing to call — both mean "walk instead".
    batched = _batch_fixture_properties(property_port, fixtures_path, child_count)
    if batched:
        known_slots = {
            child["i"]
        ...
        # Rows the first listing page never mentioned ARE the rig's other
        # fixtures — the batch already carries their slot and name, so the
        # slot-by-slot recovery below has nothing left to probe.
        for slot in sorted(batched):
            if slot in known_slots:
                continue
            children.append(
                {
                    "i": slot,
                    "name": str(batched[slot].get("name") or f"Fixture {slot}"),
                    "class": "Fixture",
                }
            )
    ...
    for child in children:
        ...
        # No batch to lean on -> the ORIGINAL guard, unchanged: stop before a
        # fixture this call cannot afford to read.
        if not batched and budget < per_fixture:
        ...
        if not isinstance(slot, int) or isinstance(slot, bool):
            ...
            continue
        cached = batched.get(slot) if batched else None
        if cached is not None:
            # Already answered in a page — this fixture costs no round trip, so
            # the ceiling (which counts round trips) must not fire on it. Rows
            ...
            reads = {
                key: PropertyRead(
                    name=key,
                    ok=cached.get(key) is not None,
                    value=cached.get(key),
                    error=None if cached.get(key) is not None else "absent from batched row",
                )
                for key in SPATIAL_FIXTURE_PROPERTIES
            }
        else:
    ...
    *,
    ...
    def patch_fixtures(call: ToolCall, context: ExecutionContext) -> ToolExecution:
        ...
        mode_read = read_type_mode_widths(
            state_port,
            property_port,
            root=DEFAULT_RIG_CONTEXT_PATHS["fixture_types"],
