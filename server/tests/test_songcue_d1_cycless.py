...
`_require_proposed` 헬퍼로 건너뛰었고, `TestTheSkipIsNotAPermanentPass` 가 그 건너뜀이
조용한 합격으로 굳지 않게 잡고 있었다.

SPEC-COPILOT-D1GRANT-001 이 승인된 통로로 두 룩을 넣으면서 그 트립와이어는 자기
목적을 다했다. 이제 이 파일에 skip 게이트는 **하나도 없다** — `_require_proposed` 와
`TestTheSkipIsNotAPermanentPass` 는 사라졌고, 남은 세 클래스가 무조건 실행된다.
`-rs` 로 돌려 skip 0건임을 확인할 수 있다.
