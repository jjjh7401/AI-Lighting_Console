...
* 구간 경계 ``0 / 16000 / 24000 / 32000`` ms — 그 자리에서 진폭이 계단으로 바뀐다.
  네 구간은 16 / 8 / 8 / 8 초이고, 128 BPM 에서 각각 **8.5 / 4.3 / 4.3 / 4.3 마디**다.

  🔴 **네 구간 전부 4마디 이상인 것이 요구사항이다** (t411). 이전 값은
  ``0 / 16000 / 32000 / 36000`` 으로 마지막 두 구간이 4초씩(**2.15마디**)이었는데,
  분석기가 구간 하한을 마디로 재기 시작하자 그 두 경계가 지워졌다. 지워진 것이
  옳다 — ``docs/proposals/song-structure-lighting-standard.md`` §4 (Harmonix 912곡
  9,214구간)의 **25% 하한이 라벨마다 4.0마디**이므로 2.15마디 구간은 실측 분포에
  나타나지 않는다. 픽스처가 음악에 없는 것을 정답으로 선언하고 있었던 것이고,
  그래서 분석기를 약하게 만드는 대신 픽스처를 고쳤다.
...
#: 네 구간 16 / 8 / 8 / 8 초. 128 BPM 에서 8.5 / 4.3 / 4.3 / 4.3 마디로, 전부 실측
#: 하한 4마디 이상이다 — 위 docstring 의 t411 설명 참조.
FIXTURE_BOUNDARIES_MS = (0, 16_000, 24_000, 32_000)
