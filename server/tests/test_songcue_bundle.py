...
_TOOLS_EXPECTED_HUNK_OLD_STARTS = (
    ...
    1225,
    1231,
    # 카드 t350 (룩 저장 축 판정 배선, 2026-09-11) — **목록은 안 바뀐다.** 그래도
    # 여기 적는 이유: 이 튜플은 재고 목록이라 `tools.py` 를 건드린 브랜치는 갱신이
    # 필요한지 **재어서** 판단해야 하고, 「안 바뀌었다」도 잰 결과여야 한다. 안 적으면
    # 다음 카드가 「t350 은 재고를 안 봤다」와 「재봤고 그대로였다」를 구별할 수 없다.
    #
    # 실측 델타: 84 -> **84** hunks. 새 시작점 **0**, 사라진 시작점 **0** — t350 의
    # 추가분(`_look_axis_source` 클로저 · `axes=` 인자 · payload 두 갈래의 `capability`
    # 절 · 도구 설명)이 모두 **기존 hunk 안쪽**에 떨어져 경계를 새로 긋지 않았다
    # (`--unified=0` 은 순수 삽입이 기존 hunk 를 쪼갤 때만 시작점을 늘린다).
    # 보호 구간 둘(234..238 / 524..569) 무접촉 — 겹침은 아래 검사의 `_overlaps` 로
    # 다시 재어 **0** 이다.
)
