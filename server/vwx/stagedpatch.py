...
ZERO_CREATED = (
    ...
    "두 조건 중 하나가 빠진 것이다 — 조작자가 **직접** 실행했는가, 그리고 실행 순간 "
    "**Patch 편집기가 열려 있었는가**."
)

#: 성공에 필요한 두 축. 실측으로 확정됐다(`progress.md` §「AddFixtures 최초 성공 관측」):
#: 서버 OSC로 발화하면 편집기가 열려 있어도 0건이고, 사람이 명령줄에서 실행하면서
#: 편집기가 열려 있으면 생성된다. `ChangeDestination`으로 목적지를 옮기는 것은 **역효과**다
...
HANDOVER_WHY = (
    ...
    "편집기 안에 있을 때 자연스럽게 놓이며, 서버가 대신 실행하면 놓이지 않습니다"
    "(실측: 서버 발화 0건 · 사람 발화 3건 생성)."
)
