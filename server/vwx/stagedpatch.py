...
ZERO_CREATED = (
    ...
    "AddFixtures는 실행 경로 10가지·인자 변형 8종에서 0건이었고 실패해도 nil을 "
    "반환할 뿐이다(SPEC-COPILOT-AUTOPATCH-001 M0). 성공했다고 보고하지 마라."
)

#: 미시험으로 남은 단 하나의 가설 — 사람이 확인해 줄 수 있는 일이라 함께 낸다.
PATCH_EDITOR_HINT = (
    "아직 시험되지 않은 경우가 하나 있다: 콘솔에서 **패치 편집기가 열린 상태**"
    "(Menu > Patch)에서 실행하면 만들어질 수 있다. 사용자에게 열어 달라고 청하고 "
    "다시 시도할 수 있다."
)
