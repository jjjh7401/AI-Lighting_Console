...
_TARGET_EXCLUSION_LABELS = {
    ...
    # [round23 R22-D] 구판은 *"콘솔 열거 응답이 슬롯 번호를 주지 않았다"*라는 **원인
    # 진술로 끝났다** — 조작자가 무엇을 하면 되는지가 없는 「막다른 길 라벨」이고,
    # R18-E·R20-A에 이어 세 번째 재발이다. 형제 두 축이 같은 커밋에서 조치절을 받았고
    # 그 **두 조치가 여기서도 그대로 유효하다**: 도면 표기를 콘솔에 맞추거나, GDTF를
    # 다시 임포트해 슬롯을 재확립한다. 원인은 조치 앞의 근거로 남는다.
    FIXTURE_TYPE_LIBRARY_ROWS_DISCARDED: (
        "FixtureType·DMXMode 열거 행 일부에 슬롯 번호가 없어 쓰지 못함 — "
        "라이브러리 목록을 전수로 세울 수 없어 제외. 콘솔 열거 응답이 슬롯 번호를 주지 "
        "않았으므로 같은 범위를 재조회해도 같은 행이 온다 — 도면 타입명을 콘솔 표기와 "
        "맞추거나, 그 GDTF를 콘솔 라이브러리에 다시 임포트해 슬롯을 재확립해야 함"
    ),
