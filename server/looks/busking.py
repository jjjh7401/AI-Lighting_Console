...
@MX:NOTE: [AUTO] 장르 조회가 ``matching.match_looks``를 쓰지 않는 것은 취향이
  ...
  10룩이므로 그 경로를 타는 순간 **2건이 조용히 사라진다** — 상한이 8이라
  장르가 몇 룩이든 8룩이 돌아오고, 번들이 만들어지고 실행이 성공하므로
  아무도 알아채지 못한다. 사라지는 건수는 장르 크기에 따라 움직이지만
  **사라진다는 사실 자체는 안 움직인다**: 이 모듈이 이 경로를 안 타는 이유가
  그것이다.
