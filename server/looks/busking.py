...
@MX:NOTE: [AUTO] 장르 조회가 ``matching.match_looks``를 쓰지 않는 것은 취향이
  ...
  도구의 올바른 규율이지 "이 장르 전부"를 돌려주는 도구의 것이 아니다. 세 수를
  구별해야 한다 — 장르의 룩 수, 상한 ``MAX_TOOL_MATCHES``(=8), 그리고 그 차이만큼
  사라지는 건수. EDM은 2026-09-06 부터 10룩이므로(SPEC-COPILOT-D1GRANT-001 이 cyc
  없는 리그용 D1 룩을 더했다) 그 경로를 타는 순간 **2건이 조용히 사라진다** —
  상한만큼인 8룩이 돌아오고 번들이 만들어지고 실행이 성공하므로 아무도 알아채지
  못한다. 사라지는 건수는 라이브러리가 자랄수록 늘어난다.
