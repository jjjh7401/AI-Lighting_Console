...
| `song_audio_upload` | `file_name`, `mime_type`, `content_base64` | (MUSICSYNC M2, additive) Attach one song (`.wav`/`.flac`/`.mp3`/`.m4a`, decoded ≤64 MiB). Store-only; rejection is `error` kind `song_audio_rejected` with a Korean reason. Single frame — only fits files whose base64 frame is under uvicorn's default 16 MiB `ws_max_size`; the UI uses the chunked trio below. |
| `song_audio_upload_begin` | `file_name`, `mime_type`, `total_bytes: int ≥ 1` (≤64 MiB), `chunk_count: int ≥ 1` | (REQ-MUSICSYNC-027, additive) Start a chunked song upload. Resets any unfinished upload on this connection. |
| `song_audio_upload_chunk` | `index: int ≥ 0`, `content_base64: string` (≤4 MiB chars) | One piece, strictly in order from 0. Out-of-order, missing, or over-total pieces are rejected (`song_audio_rejected`) and the buffer is dropped. |
| `song_audio_upload_end` | — | Finish. Only if piece count and byte total match `begin` does the assembled file go to the same store path as `song_audio_upload` (one `notice` with sha256 + byte count). The partial buffer is connection-local and never becomes an attachment. |
| `song_audio_analyse` | `sheet_bpm?`, `fx_rate?`, `beats_per_cycle?` | (MUSICSYNC M2) Analyse the attached song; the reply is a `question_request` confirmation card, or `error` kind `song_audio_missing`. |
