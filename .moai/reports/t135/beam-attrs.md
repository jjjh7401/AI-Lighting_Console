...
`server/web/session.py:4942` 가 이미 적어 둔 구조적 한계(Programmer/Selection 판독 별칭 없음)를 이 카드에서 다시 부딪혔다. 리드에 따르면 오늘 sync 레인도 같은 벽에 부딪혔다 — **두 레인이 독립으로 같은 한계를 확인**했다. t105 결론("값 채널은 존재를 답하고 값은 안 준다")의 확장이다.
