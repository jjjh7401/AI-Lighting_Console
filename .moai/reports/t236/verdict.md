...
못박았고, 그 단위는 **지금 이 채널로는** 못 잰다(t235). 리드 배차도 같다.
...
- **콘솔에 아무것도 안 쐈다.** 이 카드는 쓰기 0이다. `Attribute 'Zoom' At 45` 가
  어떻게 해석되는지는 여전히 미측정이고, **지금은** 이 채널로 못 잰다(t235).
  > ⚠️ **만료 고지 (t240, 2026-09-02)** — 이 문서가 처음 쓸 때 쓴 「원리적으로」는
  > 과했다. `t235` 가 원인을 **응답기 `ROOT_ALIASES` 의 별칭 부재**로 좁혔다 — 제거 가능한
  > 미구현이지 채널의 한계가 아니다. 결론(지금은 못 잰다)은 그대로다.
