...
>
> **이 절의 크기 주장은 틀렸다.** SPEC 의 원래 문면(「bm 임포트가 통째로 0건」)이
> 맞다. `parse_preset_csv` 가 헤더에서 종류 하나를 정하고(`preset_parser.py:380`·`:434`)
> 그 프로덕션 호출지가 `import_lxseq_presets` 한 곳뿐이라(`tools.py:4883`, 인자는
> 시트 한 장), `result.planned` 는 **구조적으로 단일 종류**다. bm 이 열려도
> dim·col 로 번지지 않는다.
>
> ⚠️ **그러나 파생시키지 않은 판단 자체는 여전히 옳다.** 자동 해제는 그날 그 bm
> 임포트를 0건으로 만들고, 그것만으로도 막을 이유가 된다. 트립와이어의 **동작**도
> 안 틀렸다 — 틀린 것은 설명 문구의 크기뿐이고, 코드 쪽 문구는 t154 가 고쳤다.
>
> 틀린 형태: 소비 루프만 읽고 **생산 지점**을 안 읽었다. 경위는
