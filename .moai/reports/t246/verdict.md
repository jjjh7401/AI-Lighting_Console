...
| `Fixture 27` | 안 움직인다 | 슬롯 축도 안 비친다 ⇒ 갈래 B 강화 (다만 「FID 축이라 27 이 없는 FID 였다」와 안 갈린다) |

이 쇼파일의 FID 는 101 이상만 쓴다(101-118 · 201-212 · 301-316 · 401-430 ·
501-528 · 601-622 — §4 의 전수 86개). **27 은 FID 로는 없고 슬롯으로는 있다.**
그래서 `Fixture 27` 한 발이 두 축을 가른다.

⚠️ 한계 하나 — **이 FID 범위는 이름에서 추론한 것이지 FID 속성을 읽은 것이 아니다.**
§4 가 읽은 것은 `name`(`"MOVER-U 501"`)과 슬롯 인덱스 `i`(=27) 둘뿐이다. 이름 끝의
수가 FID 라는 것은 이 저장소의 관례이고(`t241` 이 `patch.csv` 를 근거로 501 을 MOVER-U
FID 로 적는다), 이 채널로 확인한 값이 아니다. 이름과 FID 가 어긋난 픽스처가 하나라도
있으면 위 표의 둘째 행이 흐려진다 — 첫째 행(움직이면 슬롯 축)은 그래도 선다.
