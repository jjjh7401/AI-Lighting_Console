...
> 🔴 **2026-09-02 반증됨 — 이 판정서의 결론은 틀렸다.**
> `t248` 이 같은 콘솔에서 `Fixture 501` 을 다시 쏘고 `Selection` 의
> **`COUNTTOTALSELECTED` 가 0 → 1 → 0** 으로 움직이는 것을 관측했다.
> **이 채널은 프로그래머 점유를 비춘다.** 아래 §0·§6 의 「비추지 못한다 /
> 갈래 B」를 그대로 읽지 마라 — §6.1 을 먼저 읽어라.
> 원문은 지우지 않는다. 무엇이 어떻게 틀렸는지가 같이 사라지기 때문이다.
...
### 6.1 🔴 정정 (2026-09-02, t248 실측) — 채널이 없었던 게 아니라 필드를 안 봤다

**정정 후 사실**:

```
Fixture 501 직후 (t248, 같은 콘솔 · 응답기 1.6.3)
  COUNTTOTALSELECTED   0 -> 1      🟢 움직였다
  COUNTFULLYSELECTED   0 -> 1      🟢 움직였다
  COUNT                0            (불변)
  ACTIVE               false        (불변)
  childCount           0            (불변)   <- 이 판정서가 본 필드
Clear 직후:  1 -> 0 복귀
```

**왜 틀렸나 — 기전.** 이 회차는 「선택했는데 안 비친다」와 「선택이 안 됐다」
둘만 후보로 세웠다. **실제 답은 셋째였다: 선택은 됐고 비추고도 있었는데,
내가 비추는 필드를 안 봤다.** `Selection` 노드에 `COUNTTOTALSELECTED` 가
있었고 이 회차는 `state` 의 `childCount` 하나만 읽었다.

**「채널이 눈멀었다」가 아니라 「내가 한 필드만 봤다」** 이고, 그 차이가
다음 사람에게 훨씬 유용하다 — 전자는 채널을 버리게 만들고, 후자는 **필드를
더 찾게** 만든다. 실제로 t248 은 `introspect` 로 필드 목록을 먼저 받아
그 자리를 찾았다. 이 회차는 그 한 단계를 안 밟았다.

같은 회차에서 주소 축도 닫혔다 — `Fixture 27` 이 `Illegal object` 로 **거절**되어
`Fixture <n>` 은 FID 를 겨눈다는 것과, 콘솔이 픽스처 피연산자를 검증한다는 것이
확정됐다. **그 거절은 §5.1 이 세운 두 갈래 어디에도 없던 셋째 결과다** —
되읽기 결과만 후보로 세우고 **실행 자체가 거부되는 갈래**를 빠뜨린 설계 결함이다.

⚠️ **`COUNT` 와 `ACTIVE` 를 죽은 필드로 읽지 마라.** 선택에는 안 움직였을 뿐,
무엇에 움직이는지는 안 쟀다. **안 움직인 것과 안 작동하는 것은 다르다.**

⚠️ **소급 판독은 불가능하다.** 이 회차의 `Fixture 501` 이 그때도 선택했는지는
그때 `COUNTTOTALSELECTED` 를 안 읽었으므로 영영 확인할 수 없다. t248 이 답한
것은 「지금 쏘면 선택된다」이지 「그때도 선택됐다」가 아니다.

측정 원문: `.moai/reports/t248/verdict.md` · `.moai/reports/t248/evidence-raw.txt`
