...
## 만료·정정 고지 (2026-08-31, 2단계에서 추가 — 위 본문은 고치지 않는다)

**1) 위 §2.1 의 프레이밍이 과했다. 리드가 좁혔고 맞다.**
「S1 과 S3 를 소비자가 못 가린다」는 `refusal` 필드만 볼 때의 말이다. 같은 결과
객체의 `console_read_incomplete` 가 S1 참(`:276`) / S3 거짓(`:418`) 로 **이미 가른다** —
그리고 그 값은 **위 표에도 이미 찍혀 있었다**. 내 프로브가 출력한 값을 두고도
한 필드만 보고 결론을 넓혔다(규약 §4.1: 인용의 범위와 결론의 범위를 나란히 적어라).

실제 결함은 더 좁고 그래서 더 날카롭다 — **사유가 한 번에 하나만 나간다.**
S1 에서 단면 사유는 상류에 이미 있는데 안 실리고, FID 문제를 고친 소비자가 단면
문제를 그때서야 새 놀람으로 만난다.

**2) 위에 인용된 프로브 출력은 수리 전 시점의 기록이다.**
2단계가 `group_mapper.py:271` 을 고쳤으므로 지금 같은 프로브를 돌리면 S1 의
`refusal` 은 `None` 이 아니라 `section_unread` 가 나온다. 위 값들을 현재 상태로
읽지 마라 — 현재 상태는 `stage2.md` 다.

**3) 위 §4 의 「이 갈래를 지키는 검사는 없다」는 2단계에서 해소됐다.**
`test_lxseq_group_section_request.py` 7절(`TestBothAxesReportTogetherWhenBothFail`)
세 검사가 그 자리를 지킨다. 뮤테이션 2종으로 확인했다(`stage2.md` §2.4).
