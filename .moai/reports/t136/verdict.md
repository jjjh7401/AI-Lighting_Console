...
⚠️ 관련해 `Patch/Stages/1/Fixtures childCount 80` 이 t105 실측이라며 전달됐다.
M0 의 41 과 다르다. **다만 이건 내가 안 쟀다 — 인계문에서 온 값(세션 미상)이다.**
🔴 초판은 이 값을 「리드 보고」로 적었는데 부정확하다. 현 리드 세션(t98)이 자기는
보낸 적 없다고 확인했다 — 이전 리드 세션 또는 인계문 경유다. 세션이 바뀌면
「리드」라는 라벨은 귀속을 흐린다. 이 카드는
...
| `Patch/Stages/1/Fixtures = 80` | **인계문에서 온 값(세션 미상). 내가 안 쟀다.** 초판의 「리드 보고」 표기는 정정됨 — 현 리드 세션은 보낸 적 없다 |
