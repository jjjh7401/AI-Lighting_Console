...
### 7.2 회귀 — 통과 수의 차이로 말한다

exit code 가 아니라 **잰 두 수**를 적는다(규약 §3):

| 언제 | 명령 | 결과 |
|---|---|---|
| 신규 파일 제외 (M3 뮤테이션 아래) | `uv run pytest -q --ignore=<신규파일>` | **10513** passed, 12 skipped |
| 신규 9검사 포함 (뮤테이션 되돌린 뒤, 커밋 경계) | `uv run pytest -q` | **10522** passed, 12 skipped |

차이 **+9 = 추가한 검사 수와 일치**한다. 즉 이 파일은 새 통과만 더했고 기존을
움직이지 않았다.

세 번째 수도 **쟀다**(한 명령이면 답이 나므로 추론으로 두지 않았다, 규약 §7):
뮤테이션 없는 깨끗한 트리에서 신규 파일만 제외 -> **10513** passed, 12 skipped.
M3 아래의 10513 과 같은 값이다 — 즉 M3 은 기존 통과 수를 움직이지 않았고,
차이 +9 는 온전히 이 파일이 더한 것이다.
