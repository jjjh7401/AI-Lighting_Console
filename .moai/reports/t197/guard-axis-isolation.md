...
> 🔴 **읽는 법 — 요약 3·4번과 §3 이 네 줄 틀렸다. §11 이 정본이다.** 규약 §5 대로 틀린 문면을 고치지 않고 §11 에 정정을 붙였다. §3 을 인용하려면 §11.1 표를 같이 봐라.
...
### 3.2 상호작용 — 단축 축으로는 안 보이는 것

리드가 짚은 방법론 사각이다. **「한 호출에 한 형태」(규약 §3.3)는 단일 방아쇠를 가르는 데는 옳지만, 상호작용은 원리적으로 못 본다.** 위 3.1 의 단축 표만으로는 통과해야 할 명령이 거절되는 자리가 남는다. **단축 표가 완성된 뒤에 쌍을 쏴야 한다.**

(규약 §3.3 에 붙일 단서 후보다. 판정은 리드가 한다.)

#### 리드 표본 (t184 워크트리, 각 1회, 2026-08-31) — 명령줄 전문

```
A  S=/tmp/moai_probe_x                                       통과
   echo $S

C  S=/etc/hosts                                              통과
   wc -l $S; echo "--- tail ---"; grep -c localhost $S

D  echo hello > /tmp/moai_probe_z.txt                         통과
   wc -l /tmp/moai_probe_z.txt

E  S=/etc/hosts; echo hi > /tmp/moai_probe_v.txt; wc -l $S    통과 (한 줄)

B  echo hello > /tmp/moai_probe_y.txt                         거절 (i)
   S=/tmp/moai_probe_y.txt
   wc -l $S; echo "--- tail ---"; grep -c hello $S

F  echo hi > /tmp/moai_probe_u.txt                            거절 (i)
   S=/tmp/moai_probe_u.txt; wc -l $S
```

리드 가설: **해석 안 되는 변수 + 리다이렉트, 둘이 같이 있으면 거절.**

#### 🔴 그 가설은 반증됐다 — E 와 F 를 가르는 변수가 둘이었다

E 는 한 줄이고 대입이 앞이다. F 는 두 줄이고 대입이 뒤다. **줄바꿈**과 **순서**가 같이 달라 안 갈린다. t197 워크트리에서 하나씩만 바꿔 쐈다:

```
S=README.md; echo hi > f; wc -l $S     통과   대입이 앞 (한 줄)
echo hi > f; S=f; wc -l $S             거절   대입이 뒤 (한 줄)   <- 줄바꿈이 아니다
S=README.md 줄바꿈 wc -l $S            통과   두 줄, 리다이렉트 없음  <- 줄바꿈 무죄
echo hi | cat 줄바꿈 S=…; wc -l $S     거절   파이프, 리다이렉트 없음 <- 리다이렉트가 아니다
echo hi; S=README.md; wc -l $S         거절   파이프도 리다이렉트도 없다 <- 결정적
echo hi >> f 줄바꿈 S=f; wc -l $S      거절   >> 도 같다
echo hi > f 줄바꿈 ls -d $HOME         통과   $HOME 은 리다이렉트와 겹쳐도 통과
```

**결정적인 줄은 다섯째다.** 파이프도 리다이렉트도 없는데 거절된다. 그러므로 **리다이렉트는 리드 표본의 교란 변수였고, 이것은 상호작용이 아니다.** 참인 규칙은 §11.4 의 R1 — **명령 머리에 연속으로 붙은 대입만 추적된다.** 리드의 E 가 통과한 것은 대입이 머리였기 때문이고, B·F 가 거절된 것은 아니었기 때문이다. 리다이렉트는 무관하다.

**다만 리드의 관찰 자체는 값이 있었다** — 단축 표만으로는 이 자리가 안 보였고, 리드가 안 보이는 자리를 지목했기 때문에 R1 이 나왔다. 규칙은 틀렸고 **가리킨 자리는 맞았다.**

#### 리드가 안 잰 칸 — 이 회차에서 메웠다

| 리드가 남긴 물음 | 답 |
|---|---|
| 환경 변수 + 리다이렉트 | 통과 (`$HOME`). 다만 `$HOME` 외 환경변수는 리다이렉트 없이도 거절 |
| 리다이렉트 대신 파이프면? | **거절.** 그리고 파이프도 원인이 아니다 (위 다섯째 줄) |
| `>>` · `2>` · `<` | `>>` 는 `>` 와 같다. `2>` · `<` 는 **안 쟀다** |
| 표본 1회 | 이 회차도 각 1회다. 반복 재현 **안 했다** |
| t184 트리에서 쟀다 | t197 트리에서 F 를 **재현했다** — 트리가 달라도 같다 |
...
## 11. 🔴 정정 — 위 §3 이 네 줄 틀렸다 (같은 세션, 머지 직후)

계보 파일 `lesson-bash-guard-trigger-is-the-unquoted-brace-pair`(354행)를 **머지한 뒤에** 열었다. 열어 보니 그 파일이 여덟 번 경고한 「한 덩어리 두 변수」를 내가 두 자리에서 저질렀다. 격리해서 다시 쟀고 네 줄이 뒤집힌다.

⚠️ 규약 §5 대로 위 문면은 **안 고친다** — 틀리게 주장했다는 사실 자체가 감사 기록이다. **아래가 정본이다.**

### 11.1 뒤집힌 줄

| 원래 결론 | 원래 자극 (두 변수) | 격리 자극 (한 변수) | 결과 | 정정 |
|---|---|---|---|---|
| for 루프가 방아쇠 | for i in 1 2; do echo $i; done | for i in 1 2; do echo hi; done | **통과** | **루프는 무죄.** 범인은 $i |
| while 루프가 방아쇠 | while read x; do echo $x; done | while true; do break; done | **통과** | **루프는 무죄** |
| 히어독이 방아쇠 | cat <<EOF | cat <<'EOF' | **통과** | **히어독은 무죄.** 범인은 **따옴표 없는 구분자** |
| 히어독 뒤 리다이렉트가 방아쇠 | cat > f <<EOF | cat > f <<'EOF' | **통과** | 같은 이유 |
| 단순 매개변수 확장은 통과 | echo $HOME | echo $PATH · $PWD · $USER · $SHELL · $undefined | **넷 다 거절** | **$HOME 만** 통과한다 |

확인 사격: for i in 1 2; do echo $HOME; done → **통과.** 루프 안이어도 값이 정해지면 통과한다.

### 11.2 통합 모델 (정본)

방아쇠는 형태의 목록이 아니라 **성질 하나**다 — **스캐너가 낱말의 값을 정적으로 못 정하면 거절한다.**

**정할 수 있다 → 통과**

- `$HOME` (유일하게 통과하는 환경변수)
- 같은 명령 안에서 대입된 변수 — `x=1; echo $x`
- 산술 확장 `$((1+1))`
- glob `*` — 파일계가 답한다
- 홑따옴표 구간 전부
- 따옴표 붙은 히어독 구분자의 본문
...
- `$HOME` 을 뺀 모든 `$VAR`
- `${VAR}` — **HOME 이어도 거절**. 중괄호 형태를 아예 안 읽는다
- 명령 치환 `$( )` 와 백틱 — 큰따옴표로 감싸도 거절
- 중괄호 확장 `{a,b}` · `{1..2}` — 다만 `{}` 와 `a{b}c` 는 확장이 아니라 통과
- 따옴표 없는 히어독 구분자
- 리다이렉트 대상이 변수 — `R=f; echo x > $R`
- 히어독 연산자 **뒤**에 오는 리다이렉트
- 격리 밖을 가리키는 `git -C` (이것만 사유 문구가 다르다)

### 11.3 히어독 다섯 형태 (완결)

| 형태 | 결과 |
|---|---|
| cat <<EOF (구분자 따옴표 없음) | **거절** — 본문이 확장되므로 |
| cat <<'EOF' | 통과 |
| cat > f <<EOF | **거절** — 구분자 때문이지 리다이렉트 때문이 아니다 |
| cat > f <<'EOF' | 통과 |
| cat <<'EOF' > f | **거절** — 리다이렉트가 히어독 연산자 **뒤**. t105 축 재현 |

### 11.4 변수 규칙 — 두 개다

**R1 (변수 해석).** 명령 **머리에 연속으로 붙은 대입**만 추적된다. 첫 비대입 문장이 나오는 순간 추적이 끝나고, 그 뒤의 대입은 안 보인다.

```
x=1; echo $x                        통과   대입이 머리
A=1; B=2; echo $A $B                통과   연속 대입 둘 다 추적
S=README.md; wc -l $S; echo done    통과   뒤에 문장이 더 와도 상관없다
echo hi; S=README.md; wc -l $S      거절   대입이 머리가 아니다
echo hi | cat / S=…; wc -l $S       거절   같은 이유 (파이프는 교란)
echo hi > f / S=f; wc -l $S         거절   같은 이유 (리다이렉트도 교란)
```

환경에서 오는 변수 중 통과하는 것은 **`$HOME` 하나**다. `$PATH` · `$PWD` · `$USER` · `$SHELL` · 미정의 변수 전부 거절.

**R2 (리다이렉트 대상).** 리다이렉트 대상은 **리터럴이어야 한다.** R1 로 해석 가능한 변수여도 거절된다 — `R=f.txt; echo hi > $R` 은 대입이 머리인데도 거절이다. R1 과 별개의 규칙이다.

**리드가 다른 워크트리에서 독립 재현했다 (t184, 2026-08-31).** 대입 위치 하나만 바꾼 두 팔이다 — 파이프도 리다이렉트도 없다.

```
echo hi; S=/etc/hosts; wc -l $S      거절 (i)
S=/etc/hosts; echo hi; wc -l $S      통과
```

이 표본은 t197 트리가 아니라 **t184 트리**에서 나왔다. R1 이 한 워크트리에 갇힌 관측이 아니라는 뜻이라, 결론의 넓이가 그만큼 늘어난다.

### 11.5 계보 파일과 대조 (이번엔 열고 나서 쓴다)

위 §5 의 「부분 반증」 판정은 **과했다.** 나는 계보 파일이 아니라 `MEMORY.md` 색인 줄과 카드 요약을 대조 대상으로 삼았다. 파일을 열어 보니:

- 파일의 **방아쇠 A**(살아 있는 명령 텍스트의 확장 구문 — 명령 치환 · 파라미터 확장)는 **이미 맞다.** 내 「새 발견」의 절반이 여기 있었다.
- t105 축(히어독 연산자 뒤 리다이렉트)도 **이미 있고 재현됐다.**
- 「for 루프 리터럴 인자는 통과 / 파라미터 확장을 더하면 거절」도 **이미 있고 재현됐다.** 나는 이걸 안 읽고 for 루프를 방아쇠로 적었다.

이 파일이 「기존 기록보다 넓다고 말하려면 그 기록을 **먼저 열어라**」고 적어 둔 지점에서, 그 문장을 안 읽었기 때문에 내가 **아홉 번째**로 걸렸다.

### 11.6 그래도 새로운 것

1. **가드 소유자 분리** — `claude` 대 `moai`, 문자열 계수로 확정
2. **환경 축 셋, 조건은 EnterWorktree 격리** — cwd 가 워크트리인지가 아니다
3. **도구 축 네 문구** — 서로 바이트 다르다
4. **한 가드 안에서 열 개 원인이 한 문구로 접힌다**
5. **`${VAR}` 가 `$VAR` 과 갈린다** — HOME 이어도 중괄호 형태는 거절 — **왜 갈리는지는 모른다. 알려진 미지로 둔다.**
6. **환경변수 중 통과하는 것은 `$HOME` 하나**
7. **R1 의 「머리 대입만 추적」** — 파일에 없던 규칙
8. **중괄호 확장 `{a,b}` 가 방아쇠 A 의 구성원**이고 `{}` 는 아니다
