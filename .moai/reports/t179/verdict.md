...
- M3 이 린트 축을 함께 건드린 것은 **뮤턴트 설계의 문제**였다. 같은 형태(`if False and`)를
  쓰는 다음 사람도 같은 오염을 만든다. **후보로 t184(리드 소유)에 박혀 있다** —
  규약 §3 이 74행이라 문면을 더하기 전에 **절 쪼개기 판정이 먼저**이기 때문이고,
  t181 의 예외-종류 건과 함께 그 카드가 들고 있다. 여기와 그 카드 두 자리에 남긴다.

---

## 6. 이 회차에서 제일 값나간 기록 — 규칙을 아는 사람도 같은 실수를 한다

규약 §3 의 「우연히 덮고 있을 수 있다」가, **그 규칙을 만든 카드(t177)의 형제가 쓴
새 검사에서 그대로 났다**(§2.7). 그리고 **뮤테이션이 그것을 잡았다** — 「어느 팔이
죽었나」를 봤기 때문이다. 「죽었다」만 봤으면 못 봤다.

M4 도 같은 형태다. **사유 코드 단언에서만 죽었다** — 「거절됐다」만 단언했으면 M4 는
살아남았고, 미판독이 절단 코드로 나가는 것을 아무도 못 봤다. 리드 조건 2(「어느
사유로」를 문자열로 단언하라)가 값을 했다는 실물 증거다.

두 기록의 공통점: **뮤테이션 결과를 「죽었다/살았다」로만 읽으면 둘 다 놓친다.**
어느 단언이 죽였는지까지 읽어야 나온다.
