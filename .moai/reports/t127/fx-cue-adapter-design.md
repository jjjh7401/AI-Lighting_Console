...
- **13개 그룹 이름이 지금 리그에서 해결되는지.** 매퍼의 존재만 확인했고
  **실행은 0회**다. 미측정으로 남는다.
  ...
  > **정정 (t128, 2026-08-30)**: 초판은 이 항목에 「지금 콘솔이 기본 쇼파일이라
  > RIG 팩 이름이 하나도 없다」는 사유를 달았다. **그 사유는 반증됐다** — 두 채널이
  > 독립으로 뒤집었다: `Patch/Stages/1/Fixtures` childCount **80**, 그리고 preview 의
  > 점유자가 `Robin LEDBeam 350 21` 로 우리 장비다. **우리 리그 80대가 이미 패치돼
  > 있다.** t128 도 `Patch/FixtureTypes` 에서 우리 기종 8종을 직접 읽었다.
  > 미측정은 그대로 남지만 **사유는 지운다** — 미해결일 수도 있으나 그 이유는 아니다.
  > 정확한 상태는 「리그는 우리 것, 프리셋은 혼합」이다.
