...
- 감독 결정 2026-09-23(리드 경유): 후렴 분할 큐를 밝기·움직임으로 구분하는 후보 셋(D 행 범위 단계·「악기 추가」 +5%·FX 축 추가)은
  모두 기각되었다. 이유는 「음악에 따라 변수가 너무 많아 하나로 고정하기 힘들다」. 그래서 이 판정서의 설계(같은 큐 둘이면 쪼개지 않음)를 확정한다
...
- 후렴 안의 밝기·움직임 빌드는 규칙으로 넣지 않는다. 감독이 곡마다 대화나 수정 요청으로 넣는다(감독 결정 2026-09-23).
