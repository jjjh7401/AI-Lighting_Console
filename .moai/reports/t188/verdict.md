...
> 🔴 **주장 6은 틀렸다. 고쳐 덮지 않고 §7 에 정정을 붙였다** — 규약 §5.
...
## 7. 정정 — 리드가 반증했고 내가 다시 쟀다 (같은 회차, 2026-08-31)

위 본문은 고치지 않았다. 틀리게 주장했다는 사실 자체가 기록이다(규약 §5).

### 7.1 🔴 주장 6은 틀렸다 — `unreadable_fids` 는 죽은 코드가 아니다

내가 「`patchplan.py:1523-1525` 의 `unreadable_fids` 갈래는 실기에서 죽은 코드다」
라고 적었다. 근거는 「프로덕션 `query_property` 가 `ok=False` 를 반환하지 않는다」
하나였다. 그 근거는 참인데 **결론이 넓었다.**

`:1521` 은 `_fid_int(response.get("value")) if response.get("ok") is True else None`
이고 `_fid_int`(`:1606`)는 값이 `int` 도 `decimal str` 도 아니면 **`None` 을 돌려준다.**
그래서 `ok=True` 라도 값이 깨져 있으면 `:1524` 의 `fid is None` 에 걸린다.

`_t188_correction.py` 로 다시 쟀다:

    D0 대조군: 정상 값 '102'        unreadable_fids=0 complete=True fids=(101,102,103)
                                    reason: 콘솔 FID 판독이 완전하다
    D1 ok=True + 깨진 값 None        unreadable_fids=1 complete=False fids=(101,103)
                                    reason: 열거된 슬롯 1개의 FID 값을 얻지 못했다
    D2 ok=True + 포인터 문자열       unreadable_fids=1 complete=False fids=(101,103)
                                    reason: 열거된 슬롯 1개의 FID 값을 얻지 못했다

**죽은 것은 카운터가 아니라 `ok is not True` 삼항의 한 팔뿐이다.**
D2 는 원전 독스트링 3번이 말한 실제 형태(포인터 문자열)다.

이것이 (B)의 근거를 바꾼다 — **죽은 처리기를 되살리는 것이 아니라, 살아 있는
처리기에 실기 실패 형태 하나를 잇는 것**이다. (B)가 더 강해진다.

내가 틀린 기전: `ok=False` 경로가 죽은 것을 확인하고 **그 갈래 전체가 죽었다**로
넓혔다. 삼항의 두 팔을 갈라 보지 않았다. 규약 §4.1 의 「인용은 참이었고 결론만
넓었다」와 같은 형태다.

### 7.2 임포트 경계를 내가 넓게 읽었다

t182 가 세운 경계를 나는 「`server/vwx` 는 밖을 임포트하지 않는다」로 읽었다.
아니다. 실측(base `11f9d13`):

    server/vwx 가 server.prechk 를 임포트하는 파일: 5개
      address.py:16,17 · addressfit.py:25 · diff.py:20,21
      patchplan.py:10  · apply.py:44,45

경계는 처음부터 **`server.safety` 하나**였고, 이유는 자기완결이 아니라 도메인 층이
전송 층을 임포트하는 것을 피하려던 판단이다.

그리고 그 경계를 **강제하는 검사가 없다.** 경계 검사는 넷 있는데 vwx 것이 아니다:
...
⚠️ 리드는 임포트를 **4파일**, 경계 검사를 **둘**로 적었다. 내 실측은 **5파일 · 넷**이다.
요지(경계는 safety 하나 · vwx 강제 검사 없음)는 그대로이고 수만 다르다.
옮겨 적지 않고 다시 쟀으므로 여기 값이 이 트리의 값이다.

### 7.3 내가 안 짚었던 것 — `:1515` 를 안 감싼 것은 누락이 아니다

리드가 짚었고 확인했다. `patchplan.py:1558-1559` 가 명시적으로 적고 있다:

> 열거된 슬롯의 읽기는 감싸지 않는다 — 그쪽은 콘솔이 스스로 목록에 올린 슬롯에
> 대한 전송 결함이고, 투기적 프로브와 성질이 다르다.

즉 `:1515` 의 무방비는 **기록된 결정**이다. (B)는 그 결정을 건드리므로 「빈자리를
채운다」로 쓰면 안 되고 그 주석에 대고 논증해야 한다.

논증이 서는 근거: 그 주석이 막는 것은 **조용히 삼키는 것**이고(`probe_failures` 가
그 형태다 — 진단으로만 센다), `unreadable_fids` 는 삼키지 않는다.
`:1298` 에서 `complete` 판정에 들어가고, `:1348` 에서 사유 문자열로 나가며,
`:1385` 에서 페이로드 키로 나간다. 7.1 의 D1·D2 가 그 사유가 실제로 나오는 것을
보인다.

### 7.4 수리 카드의 수용 기준 (리드가 정함)

**고친 뒤 그 사유 문자열이 실제로 사용자에게 나가는가.** 안 나가면 (B)가 아니라
삼킨 것이다. — 이 기준을 여기 박아 둔다. 수리는 별도 카드(t194).
