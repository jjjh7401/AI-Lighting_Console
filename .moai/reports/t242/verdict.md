...
### 7.1 🔴 만료 고지 (2026-09-02, t248 실측) — 갈래 B 의 처방이 틀렸다

위 표의 갈래 B 가 「`SelectionCount()` / `SelectionFirst()` **새 동사가 유일한
경로다**」라고 적었다. **틀렸다.** 원문은 그대로 둔다 — 처방만 갈아끼운다.

**정정 후 사실**: 그 정보는 새 동사 없이 **지금 `prop` 으로 읽힌다.**

```
prop Selection COUNTTOTALSELECTED   ->  ok=true, number
prop Selection COUNTFULLYSELECTED   ->  ok=true, number
Fixture 501 직후:  0 -> 1      ·  Clear 직후:  1 -> 0
```

**왜 틀렸나 — 기전.** 이 문장은 「그 정보에 닿는 채널이 없다」를 전제했는데,
채널은 있었다. `Selection` 노드에 **필드가 하나 더 있었고 아무도 안 봤다.**
당시 판독은 `state` 의 `childCount` 만 봤고, 그 필드는 선택에 반응하지 않는다.
없는 것이 아니라 **안 본 것**이었다.

🔴 **이 오류의 대가는 「틀린 사실」에서 끝나지 않는다.** 이 문장은 **없어도 되는
작업을 처방하고 있었다** — 응답기에 새 동사를 얹고 회신 형태부터 새로 정하는,
이 카드가 「별칭보다 큰 작업」이라고 스스로 적은 그 작업이다. 읽는 사람은
`prop` 한 줄이면 되는 자리에서 응답기 확장을 설계했을 것이다.

⚠️ **다만 「`COUNTTOTALSELECTED` 를 봐라」로 좁혀 읽지 마라.** 같은 회차에서
`COUNT` 와 `ACTIVE` 는 선택 전후로 **안 움직였다**. 그 둘이 무엇을 세는지는
안 쟀다 — **안 움직인 것과 안 작동하는 것은 다르다.** 죽은 필드로 단정하지 마라.
