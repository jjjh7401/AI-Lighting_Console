...
## 8. 🔴 구현 착수 조건 — 이 절을 읽고 시작해라

§7 의 잔여 위험 중 하나는 **위험이 아니라 조건**이다. 잔여 위험 절에만 두면 다음 사람이 안 읽으므로 여기로 올린다.

### 8.1 성분을 하나도 안 버리는지를 **검사가** 지켜야 한다

col 은 `(R, G, B)` **3성분 고정**이라 「판정기와 판독기가 같은 술어를 부른다」로 충분하다 — 개수가 상수이므로 한쪽이 성분을 흘리면 형태가 바로 깨진다.

bm 은 **가변 길이**라 그 보장이 안 선다. `_bm_components()` 가 `(("Zoom","45°"), ("Iris","50%"))` 를 냈는데 빌더가 앞의 하나만 쓰면:

    판정  storable = True          ← 통과
    적용  Group 1 ; Attribute 'Zoom' At 45   ← Iris 가 조용히 사라진다
    되읽기 슬롯 점유만 확인된다     ← 값은 원래 못 읽는다

**`apply_untranslatable` 보다 조용한 실패다.** 그쪽은 0건으로 크게 실패하고 사람이 본다. 이쪽은 「성공」으로 보고되고 콘솔에는 절반만 들어간다 — 「조용히 틀린 것이 크게 없는 것보다 나쁘다」에 정면으로 걸린다.

**착수 조건**: `_bm_components()` 가 낸 성분 수와 빌더가 명령에 실은 성분 수가 **같다**는 것을 단언하는 검사를 ①과 **같은 커밋**에 넣는다. 뮤테이션으로 그 검사의 판별력을 확인한다(빌더가 마지막 성분을 버리게 고쳐서 빨개지는지).

### 8.2 `Zoom` 의 값 단위를 **빌더를 쓰기 전에** 실기로 잰다

`Attribute 'Zoom' At 45` 의 `45` 가 콘솔에서 **도인지 퍼센트인지 이 저장소는 모른다.**
- dim: 퍼센트 — 실측 확정
- col: 16비트 선형 매핑 — t134 실측 확정(오차 0)
- **Zoom: 비어 있다**

단위를 모른 채 빌더를 쓰면 `45°` 를 `At 45` 로 옮기게 되는데, 콘솔이 퍼센트로 받으면 **45% 줌**이 되어 시트가 뜻한 45도와 다른 자리에 선다. 값은 되읽을 수 없으므로 **틀린 것이 조용히 남는다.**

**착수 조건**: 어휘를 여는 회차는 빌더를 쓰기 전에 그 한 줄을 실기로 쏘고 되읽어 단위를 확정한다. 확정 전에는 `_bm_components()` 가 **원문값을 그대로 나르기만** 하고(이 설계가 그렇게 돼 있다) 빌더는 만들지 않는다.

### 8.3 순서를 뒤집으면 그 시트가 0건이 된다

§1 이 실측으로 재현한 것이다. ③(어휘 열기)을 ①(적용 경로)보다 먼저 하면 그 임포트가 통째로 0건이 된다. **①과 ③을 한 커밋에 넣어도 순서는 지켜야 한다** — 커밋 경계가 아니라 구조가 지키는 것이 §3.1 의 단일 술어다.
