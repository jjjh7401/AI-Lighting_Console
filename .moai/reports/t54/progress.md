...
## 커밋 색인 (리뷰용 — 9개, 기준 `fa08651`)

순서대로 훑지 않아도 되게 「무엇이 어느 커밋에 있는지」만 적는다. 각 커밋은
테스트 파일 + `progress.md` 를 함께 담는다(증거와 변경이 같은 커밋에 있다).

| 커밋 | 담은 것 | 테스트 파일 | 자리 | 이 문서 |
|---|---|---|---|---|
| `68a57f1` | 배치 1 — 소형 5파일 | review · session_progress · reply_discovery · console_probe · tauri_seams | 7 | §1~§7 |
| `663c2c9` | 배치 b + **분류 뮤테이션 소급** | dash · layout_image | 8 | §5.4 · §8 |
| `dbabd36` | 배치 c + **「구별 불가」 문면 정정** | handshake | 6 | §10 · §11 |
| `b0cbc5c` | 배치 d — 두 헬퍼 공존 첫 파일 | e2e | 5 | §12 |
| `7421316` | 배치 e + **`_fresh_cue_monitor` 발견** | cue_monitor (앞) | 9 | §13 |
| `7ffa0e3` | 배치 f (파일 완결) + **무한 주장 하향** | cue_monitor (뒤) | 11 | §14 |
| `666dffd` | 배치 g + **상한 레버 규칙 개정(KILL≥1)** | app (앞) | 10 | §10 · §15 |
| `e87c64e` | 배치 h — **대상 66자리 완결** | app (뒤) | 10 | §16 · §17 |
| `3097c3e` | 문서만 — 자 정정의 귀속 | — | 0 | §17.4 |

**리뷰어가 먼저 볼 곳**: 결론은 §17(카드 종합), 판정 규칙은 §10(뮤테이션 레버),
가장 논쟁적인 판정 세 자리는 §12.2(e2e 전부 recv_frame 인 이유) · §14.2(`:700`) ·
§15.3(`:81`). 이 카드가 **사지 못한 것**은 §17.3 둘째 문단에 있다.

**코드 변경의 모양** — 주장이 아니라 실측이다. 카드 전체 diff 의 모든 변경 줄을
형태별로 세었다:

```
$ git diff fa08651..HEAD -- server/tests --stat
 11 files changed, 77 insertions(+), 66 deletions(-)

$ git diff fa08651..HEAD -- server/tests | grep -E '^[+-]' \
...
| 갈래 | 줄 수 |
|---|---|
| 삭제 — `*.receive_json()` 형태 | **66** |
| 추가 — 위에 1:1 대응하는 `recv_frame(*)` 형태 | **66** |
| 추가 — `from .conftest import recv_frame` (모듈 10 + 함수지역 1) | **11** |
| 그 밖의 변경 | **0** |

66 + 11 = 77 = diff stat 의 insertions, 66 = deletions. **자리 수까지 맞는다.**

따라서 로직 변경 0 · 단정 변경 0 · 테스트 추가/삭제 0 은 **센 값**이다.
리뷰어는 「무엇이 바뀌었나」가 아니라 **「자리마다 판정이 옳은가」**(§3 · §8.1 ·
§11.1 · §12.1 · §13.2 · §14.1 · §15.2 · §16.1 의 판정 근거 열)만 보면 된다.
...
`exit_on_timeout` 이 **true** 다. 그 그물은 **내가 t52 에서 직접 넣은 것**이고 그
사실이 같은 파일 51-52행 주석에 이미 적혀 있었다.

**2차 정정 — 하향의 근거도 미관측이었다(리드 자기정정 수용).**

1차 정정에서 「60초 뒤 프로세스가 통째로 끊긴다. 무한이 아니고 그물은 딱딱하다」로
적었다. 그런데 그것은 **설정을 읽고 추론한 것이지 그물이 발사되는 것을 본 게
아니다.** 이 문서가 스스로에게 두 번 요구한 규율(§1 · §17.4)에 비추면 그것도
미관측 주장이다.

계기가 된 관측 하나가 있다 — 리드가 `t52` 트리에서 **`test_ws_wait_guard.py` 를 물고
2시간 19분째 사는 pytest 프로세스**(pid 93285, ppid 1)를 쟀다. `%CPU 0.0` ·
`TIME 0:02.53`(5초 간격 재측정에도 불변) — 잠들어 있고 부하는 없다.
⚠️ **이것은 그물의 반증이 아니다.** 그 프로세스가 테스트에 진입하기는 했는지
(수집 단계나 픽스처에서 막혔으면 `faulthandler_timeout` 은 애초에 무장되지 않는다),
그 시점 그 트리에 설정이 있었는지 **아무것도 재지 않았다.** 반증도 확증도 아니다.

그래서 정확한 문면은 이것이다:

> 그물은 **설정돼 있다**(`pyproject.toml:54-55`). 그물이 **이 형태에서 실제로
> 발사되는지는 미측정**이다. **60초라는 상한은 설정값이지 관측값이 아니다.**
>
> **t58 이 가장 먼저 잴 것**: 90초 자는 테스트 하나로 그물이 실제로 60초에
> 프로세스를 끊는지 확인한다. **안 끊기면 t58 의 등급이 다시 올라간다.**

정리하면 이 한 자리에서 등급이 두 번 움직였고 **두 번 다 미관측 단정 때문**이었다 —
1차는 run 레인의 「영원히 돈다」(과장), 2차는 리드의 「60초에 끊긴다」(설정 읽기를
관측으로 취급). 등급은 증거를 따라간다. 지금 증거로 말할 수 있는 것은
**「회수 상한이 없다」** 하나뿐이고, 그 대가가 얼마인지는 아직 아무도 안 쟀다.
...
### 17.5 카드 종료 전량 스위트 — 초록

리드가 창을 넘긴 뒤 1회 실행(반복 없음).

```
$ .venv/bin/python -m pytest server/tests -q -p no:cacheprovider
10023 passed, 12 skipped in 144.71s (0:02:24)
```
...
```
$ .venv/bin/python -m pytest server/tests --collect-only -q -p no:cacheprovider
10035 tests collected in 1.82s
```

10023 + 12 = 10035 — 자리 수까지 일치하므로 **이 실행은 t54 트리에서 났다.**

그리고 이 수는 배치 1 시점 기준선(§4.4)과 **완전히 같다**: `10023 / 12 / 10035`.
이 카드가 테스트를 추가하지도 삭제하지도 않았다는 §커밋 색인의 diff 형태 전수
결과와 **독립적인 경로로 같은 결론에 도달한다** — 한쪽은 diff 를 셌고 한쪽은
수집기를 셌다.

부하 참고: 실행 중 같은 기계에 2시간 19분째 잠든 pytest 프로세스가 있었으나
