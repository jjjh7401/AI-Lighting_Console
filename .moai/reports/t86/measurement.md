...
# 3차 측정 — Label 분리가 만든 구멍을 메움

리드가 Label 별건 분리를 승인했다. 그러면 게이트 개정은 `Store Preset` **단독**이 되는데,
1차에서 나는 그것을 `Label Preset` 과 **묶어서만** 13건으로 쟀다. 단독 비용은 미측정이었다.
SPEC 이 제안할 바로 그 형태이므로 잰다.

| 넣은 항목 | 코퍼스 | 실패 | 통과 | 총 테스트 |
|---|---|---|---|---|
| — (기준선) | 0/21 | 0 | 10167 | 10179 |
| `Store Preset` **단독** | 2/21 | **13** | 10158 | 10183 |
| `Store Preset` + `Label Preset` | 2/21 | 13 | 10162 | 10187 |

갈래 분포가 **동일**하다 — 둘 다 looks_instantiate 6 · measurement_runner 4 · writegate 2 ·
safety_ruleset 1. 즉 A 의 13건은 전부 `Store Preset` 몫이고 `Label Preset` 은 0건을 더한다.
**Label 분리가 게이트 비용을 바꾸지 않는다.**

원문: `run_A1_store_preset.txt`. 복원 체크섬 `3027d4cf…42fb` 일치.

## 12. 측정 단위의 함정 — 분모가 움직인다 (뒤늦게 발견)

위 표의 「총 테스트」 열을 보면 기준선 10179 · 1항목 10183 · 2항목 10187 이다.
**블랙리스트 항목 1개당 테스트가 4건 늘어난다.** `blacklist.yaml` 헤더가 그렇게 설계했다 —
「Tests iterate this file's content directly, so a revision here auto-extends the FN corpora」.

따라서 「실패 N건」은 고정 분모 위의 값이 아니다. 내가 1·2차에서 낸 표는 실패 수만 적고
분모를 안 적었다. 수치 자체는 틀리지 않았지만 **비교 단위로 쓰려면 분모가 필요하다.**
전체 대조:

| 형태 | 실패 | 통과 | 총 |
|---|---|---|---|
| 기준선 | 0 | 10167 | 10179 |
| `Store Preset` | 13 | 10158 | 10183 |
| `Store Preset`+`Label Preset` | 13 | 10162 | 10187 |
| `Label` | 17 | 10154 | 10183 |
| `Store` | 67 | 10104 | 10183 |
| `Store`+`Label` | 70 | 10105 | 10187 |

항목 수가 같으면 총계가 같다(10183 / 10187) — 산술이 맞는다.
이 사실은 SPEC-COPILOT-UNREQ-001 의 REQ-UNREQ-013 으로 옮겨 적었다.
