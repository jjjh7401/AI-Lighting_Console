...
> 🔴 **2026-09-23 정정 (t451)**: 아래 판정의 「이 쇼의 3D 창에서는 어떤 기구도 빛을 낸 적이 없다」는 **틀렸다.** 감독 스크린샷에 여러 기구가 켜져 있었고, 안 켜지는 것은 타입 8(Mac Aura XB)·9(Rush Par 2 RGBW)뿐이다. 감독 답을 반대로 읽었거나 질문이 모호했다. 그래서 네트워크 DMX 출력(`OUT=false`)은 외부 출력용 참고일 뿐 3D 창의 원인이 아니다. 원문은 무엇이 틀렸는지 남기려고 지우지 않는다 — `.moai/reports/t451/verdict.md` §3 을 먼저 읽어라.
