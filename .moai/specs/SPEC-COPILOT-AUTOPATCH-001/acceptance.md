...
>
> **[round16 (b)] ④(G4)에 "제거가 끝내 불가능한 경우"의 판정 규정이 없다 — 공백이다.**
...
> 해야 하며 그 절차는 아직 미정이다(`plan.md` §B M8 착수 전제 ⑤ · `M8-REDEFINITION-DRAFT.md`
> §3 P5). **제거하지 못한 채 세션이 끝나면 ④를 통과로 적어야 하는지 실패로 적어야 하는지가
> 이 문서·`plan.md`·`M8-REDEFINITION-DRAFT.md` 어디에도 없다.** 이 SPEC은 그 공백을 임의로
> 메우지 않는다 — **AC 본문 수정은 사용자 승인 사항**이므로, 공백을 공백으로 적고 결정을
> `spec.md` §C 「사용자 결정 대기」에 등록해 둔다. **AC 총계 27 · AC-026 ①~⑦은 불변이다.**
>
