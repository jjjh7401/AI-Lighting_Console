...
> **[v0.1.5 · round14 T11 고지] 이 개정으로 "픽스처가 실제로 생성된다"를 요구하는 AC가
> 사라졌다.** 이전 판 ①("승인한 장비가 도면 주소에 도면 타입으로 생성되고 검증 읽기가
> 확인한다")이 G1 전달 조항으로 대체됐고, 등가물을 두지 않았다. 그것은 **의도된 결과**다 —
> 사람 실행 경로가 이 빌드에서 성립하는지가 미확정인 상태에서 "생성 성공"을 통과 조건으로
> 두면, 서버가 옳게 한 세션도 SPEC 실패로 적히기 때문이다(⑥의 근거와 같다).
>
> **그 대가를 숨기지 않는다**: §F DoD 1번("AC 27건 전량 PASS")은 이제 **제품 효능을 포함하지
> 않는다.** 픽스처 0대가 생성된 세션도 ①③④⑤⑥⑦을 만족할 수 있다. §B 시나리오 1은 여전히
> "3대가 생성되고 검증 읽기가 3건 모두를 확인한다"를 서술하는데, 그것은 **목표 서사**이지
> 통과 조건이 아니다 — 둘이 다르다는 사실을 여기 적어 둔다.
>
...
- 기대 결과:
  ...
  - ⑥ **G1이 통과하고 G2가 실패해도 그것은 이 SPEC의 유효한 결과다.** 이 빌드에서
       ...
       **코드 결함의 직접 증거**이며, ②가 바로 그 경우를 잡으라고 있다. 배제되는 것은
       "전달물이 옳았는데도 실패했다"는 경우뿐이고, **둘을 가르는 것은 G3 결과와 전달물
       원문 대조**다. 이전 판은 증거 종류를 선험적으로 배제해 ②·⑤와 충돌했다.
