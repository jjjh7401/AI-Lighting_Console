...
> **[round19 #3 신설 · 세션 운용 문서 포인터] 이 마일스톤을 실제로 도는 절차서는
> `M0-SHOWFILE-SPEC.md`다** — 테스트 쇼파일 사양 · 제작 절차 · 세션 체크리스트 ·
> 미확인 18건(U-01~U-18)이 거기 있고, ⑦의 네 기재물을 적는 양식도 거기 있다.
> **[치명 · round19에 해소] 그 문서 §1.3이 도출했다 — `d03597b`에서는 현행 39대 쇼파일에서
> FID 사전검사가 **상시 실패**해 M8 대상이 **구조적으로 0건**이었다**
> (`_existing_fids_from_console`에 절단 복구 스윕이 없었고 PRESERVE 형제 리더
> `server/prechk/inventory.py:401-406`에는 있었다 — 실물 콘솔은 19대에서 절단되고 현행
> 쇼파일은 39대라 `unseen ≥ 15`였다). **round19가 같은 함수에 유계 `1..childCount` 스윕을
> 넣어 해소했다** — 오케스트레이터 실측 재현: `childCount=39` · 열거 19 · **`complete=True`** ·
> `fids` 39건. **다만 해소는 조건부다**: 슬롯이 `1..childCount` 밖인 희소 풀 · 프로브 무응답 ·
> 슬롯 번호 없는/중복 행 셋은 그대로 fail-closed로 남고, **완전 열거 상한은 여전히 미확인**
> (U-01)이다. 따라서 **④의 세션 전 드라이런(적격 대상 수 확인)은 면제되지 않는다** —
> **대상 0건 세션은 `G1 미검증`이며 M8 PASS가 아니다.** 선택지 3안(**S1** 전용 쇼파일 신규
> 제작=**권고, 스윕이 있어도 유효** / **S2** `assumption_71='negative'` 우회=**금지** /
> **S3** 39대로 들어가 결과로 기록=폴백)과 전문은 그 문서 §1.3과 `progress.md` §0 2d에 있다.
> **세션 착수 전에 반드시 읽어라.**
>
> **[round21 R20-A·R20-B·R20-D 신설 · 형제 축] 같은 문서 §1.3a가 라이브러리 열거 축을 담는다** —
> §1.3이 **픽스처** 열거 축이면 §1.3a는 **타입 라이브러리** 열거 축이고, 절단 개시가
> **현실 대역 16~17종**(하드 캡 24보다 **1900바이트 payload 예산이 먼저 문다**)이라
> 실제 공연 쇼파일에서 발화한다. round21이 ⓐ정직한 계수 보고(비용 0) ⓑ**표적** 회수 스윕
> (전수는 200종·모드20·GO에서 **7,392왕복 ≈ 8분**이라 기각 · 표적은 **217왕복 ≈ 14초**)
> ⓒ막다른 길 라벨 정정으로 닫았고, **닫힘은 조건부다**(§1.3a가 열 가지를 열거한다 —
> 특히 **폐기 행**과 **`DMXModes` 축**은 표적 스윕의 회수 대상이 아니다).
> **세션 GO 조건 ①(세션 중 GDTF 임포트 금지)은 유지되며 근거만 교체된다** — 상세는 §1.3a.
> 전문·처방 근거는 `progress.md` **§E.2ac**이고 §0 2d에 넷째 표면으로 등재됐다.
>
> **[round22 R22-A·R22-C·R22-E — HARD] 위 「닫았고」를 그대로 읽지 마라 — 스윕 경로에서
> 다시 열렸다.** round21 반영분에 대한 재감사 **round22가 같은 표면에서 치명 2건을 더 냈고,
> 둘 다 위 처방 ⓑ가 만들었다**: `[round22 R22-A]` 스윕 반환의 손열거 재구성이 **폐기 축
> 두 필드를 지워** *"GDTF를 먼저 임포트해야"*라는 **round21이 닫은 거짓 문장을 재출현**시키고,
> `[round22 R22-C]` *"찾으면 멈춘다"*가 포함관계 퍼지와 곱해져 **fail-closed 배제를
> 「잘못된 타입 확정」으로 뒤집는다**. `[round22 R22-E]` 완전성 마커는 **아무 처분도 읽지
> 않는다**(`resolved`와 `complete:false`가 같은 행에 공존). **세션 GO 조건 ①은 여전히
> 유지되며 이제 근거가 하나 더 늘었다.** 전문·귀속은 `progress.md` **§E.2ad**이고
> §0 2d에 재발이 등재됐다.
>
> **[round23 R23-1 — HARD · 그리고 이 마일스톤에 대한 판정이 갈라졌다] round22 반영분에 대한
> 재감사 round23은 FAIL / 릴리스 NO-GO이지만 M8 세션 재개는 GO다.**
> 위 셋(R22-A·C·E)은 **닫혔다.** 그런데 그 닫음이 **한 축에만** 걸렸다 —
> `[round23 R23-1]`(high) R22-E 처방을 **점유폭 부재 갈래**에 걸지 않아
> `_absence_assertable`과 확정 게이트의 **유일한 차이가 `mode_child_count is not None`**
> 하나가 됐고, **같은 스냅샷이 갈래에 따라 반대로 말한다**(점유폭 일치면 `library_incomplete`,
> 불일치면 **하드 스톱 + *"모드 열거를 전부 읽었고 절단도 없었다"*라는 거짓 문장**).
> **확정을 막는 것보다 강한 주장이 더 느슨한 술어로 나갔다.**
> **[HARD] 세션 재개 GO의 근거를 그대로 옮긴다** — 실물 responder(`copilot_responder.lua`)가
> `childCount = #children`을 **항상** 실어 `R23-1`의 발화 조건이 **오늘의 라이브 경로에
> 나타나지 않는다.** **무해하다는 뜻이 아니라 오늘 실현되지 않는다는 뜻**이며
> **릴리스 NO-GO는 그대로**다. 그 밖의 셋은 `R23-2`(medium, 비용 기준이 `k(1+c·m)` 항 누락) ·
> `R23-3`·`R23-4`(low)이고 **전부 반영·종결**됐다.
> 전문·귀속·**음성 결과 전수**는 `progress.md` **§E.2ae**, 세션 운용 쪽 정본은
> `M0-SHOWFILE-SPEC.md` **§1.3a**의 `[round23 …]` 블록이다.
> **round23 반영분에 대한 재감사는 round24이며 미착수다.**

> **[2026-08-07 사용자 결정 — 착수 전제 ④⑤⑥이 확정됐다]** 정본 등기는 `spec.md` §C
> 「사용자 결정 대기 → 결정 완료」다. 아래 ①~⑥의 논거·선택지는 근거 보존을 위해 **그대로 남긴다** —
> 확정값만 여기 먼저 적는다.
>
...
>
> **⑦ [신설 · AC-026④ ㉢ 승격 — 본문 열거 ⑦이 정본이다(round18 D-10)] `P5 절차가 세션 전에
> 문서로 확정되지 않으면 세션에 착수하지 않는다.`** 사용자가 ㉢(P5를 차단 조건으로 승격)을 택했으므로 `acceptance.md`는 **개정하지
...
**착수 전제**: ① 독립 코드 감사 PASS — **[round23 갱신] round11~23 열세 라운드 전부 감사
FAIL이었다.** `[라운드계수셀=23]` round20은 **감사 FAIL이면서 세션 GO(조건 3)**였고
(2026-08-08 · §E.2ab), 그 갈라짐이 정확하다 — 관문은 *"감사가 무결함"*이 아니라
*"감사가 세션 착수를 승인하는가"*다(2026-08-08 사용자 결정 「round20은 M8 세션 직전 1회」는
**집행됐다** — 정본 등기는 `spec.md` §C 결정 표). **round22는 두 감사자 전원
FAIL / NO-GO이고 판정 문구가 「실사용 불가」였다** — 치명 2(R22-A·R22-C) · high 1(R22-E) ·
medium 1(R22-B) · low 2(R22-D·R22-F)이며 **치명 2건이 전부 round21 처방의 산물**이다
(정본 `progress.md` **§E.2ad**). **round23은 FAIL / 릴리스 NO-GO이면서 M8 세션 재개는
GO다** — high 1(R23-1) · medium 1(R23-2) · low 2(R23-3·R23-4) · 뮤테이션 28건 중 26 KILLED
(92.9% 전체 / 96.3% 행동) · 하네스 **약화 4점 공시**(정본 `progress.md` **§E.2ae**).
**round20에 이어 두 번째로 판정이 갈라졌고 그 갈라짐도 정확하다** — 위 관문 문장 그대로다.
**round23 반영분에 대한 재감사가 round24이고 미착수다.**
**[HARD · 계수 규율] round21에서 `=21`로 올리지 않은 것은 round21이 R20-A·R20-B·R20-D
반영이지 감사가 아니었기 때문이다. round22·round23은 감사이므로 올린다 — 그 구별을 유지하라**
(반영 라운드는 셀을 올리지 않고, 감사 라운드만 올린다).
이전 판은 `round11~19`였고 **여덟 셀 중 하나(`progress.md` 프론트매터 `next:`)는 표지 토큰이
아예 소실된 상태였다** — 경위와 처방은 §E.2ac `R21-B`이며 **그 결함은 round23에
`DocGate`가 닫았다**(아래 ① 검사기 항 참조).

  **[HARD · round18 D-05 · round19 #1 확대] 이 셀은 매 라운드 바뀐다 — 규칙으로 적는다.**
  라운드 계수를 담은 셀은 **여덟**이며 라운드마다 **여덟을 함께** 고친다:
  **① `plan.md` §B 착수 전제 ①(이 셀) · ② `M8-REDEFINITION-DRAFT.md` §3 P1 ·
  ③ `spec.md:44` v0.1.5 HISTORY 행 · ④ `progress.md` 프론트매터 `next:` ·
  ⑤ `progress.md` §0 헤드라인 「독립 코드 감사 round11~N」 ·
  ⑥ `progress.md` §0 「이 SPEC이 1단계와 근본적으로 다른 점」 직전 문단의
  「착수 전제는 독립 코드 감사 재감사 PASS다(round11~N …)」 ·
  ⑦ `design.md:3` status 줄의 「독립 코드 감사 round11~N 지적 반영」 ·
  ⑧ `M0-SHOWFILE-SPEC.md` §5.1 P1 행.**
  **[round19 #1] 규칙 자체가 형제 넷을 빠뜨리고 있었다.** round18은 이 목록을 **"넷"**이라
  적었는데, round19의 전수 grep이 넷을 더 찾았다 — ⑤(`round11~`**`17`** · **두 라운드 stale**) ·
  ⑥(`round11~`**`16`** · **세 라운드 stale**) · ⑦(`round11~`**`16`** · **세 라운드 stale**;
  **round15 #7이 정확히 그 줄을 고치고도 형제 전파를 다시 놓쳤다**) ·
  ⑧(round18 이후 신설된 형제 아티팩트). *어떤 규율을 적용하고 형제 표면에는 적용하지 않는다*가
  **규칙을 적는 문장 그 자체에서** 재발한 것이며 **아홉 번째 동형 재발**이다.
  **규칙이 넷을 셀 때 실제로는 여덟이었다** — 열거를 신뢰하지 말고 매 라운드 아래 grep을 돌려라.
  경위를 그대로 남긴다: round16 #3이 이 셀을 정정했고 → round18이 같은 셀을 다시 stale로
  잡았고 → round19가 그 규칙의 **열거 자체**를 stale로 잡았다.
  **정정도, 정정을 규칙으로 적는 것도 재발을 막지 못한다.** round18은 열거를 규칙으로 적었고
  그 열거가 절반을 빠뜨렸다. **[round19 #1] 그래서 열거 대신 기계 검사기를 둔다** — 여덟 셀
  전부에 **표지 토큰 `` `[라운드계수셀=N]` ``**을 박았다. 매 라운드 아래 두 줄을 돌려라:

  ```bash
  S=.moai/specs/SPEC-COPILOT-AUTOPATCH-001
  grep -rno '\[라운드계수셀=[0-9]*\]' $S | wc -l          # → 8 이어야 한다(셀 삭제·누락 탐지)
  grep -rho '\[라운드계수셀=[0-9]*\]' $S | sort | uniq -c  # → 한 줄이어야 한다(값 불일치 탐지)
  ```

  **round19 실측: 첫 명령 `8` · 둘째 명령 `8` + 값 `19` 한 줄.** 두 번째 명령이 두 줄을 내면 **어느
  셀이 stale이다**. 서술형 grep(`round11~`)은 정정 산문 안의 과거 값(`round11~16`·`~17`)까지
  잡아 **탐지기로 쓸 수 없다** — round19가 실제로 그것을 확인했다(값 4종이 나왔다).

  **[round21 실측 · `R21-B`] 첫 명령이 `7`이었다 — 규칙은 `8`이다.** 오케스트레이터가 HEAD
  `f05e47f`(M8 세션 커밋)에서 `progress.md` 프론트매터 `next:` 줄을 통째로 다시 쓰면서
  **표지 토큰을 함께 지웠다.** round19가 프로브로 예측한 실패 형태 ⓐ(*"셀 하나를 통째로
  지우면 8 → 7로 떨어져 잡힌다"*)가 **그대로 발생했고, 검사기는 작동했으나 아무도 돌리지
  않았다.** 값도 stale이었다(7개 전부 `19`인데 round20 감사는 이미 §E.2ab에 기록돼 있었다).
  **진단**: 이 SPEC은 코드 축에서 *"규율을 세우면 대조군을 붙인다"*를 스무 번 반복했으나
  **문서 축에는 같은 원칙을 적용하지 않았다** — 명령은 문서에 있고 실행은 사람에게 있다.
  round21이 여덟 셀을 `20`으로 맞추고 토큰을 복원했으며, **강제(테스트) 신설은 코드 축
  과제로 별건 등재**했다(§E.2ac `R21-B` 처방 후보).

  **[round22 실측 · 문서 축 반영 직후 직접 실행 · 출력 원문]**
  (아래에서 `<셀토큰>` = 토큰의 접두부. **완성형을 적으면 검사기가 자기 설명문을 세므로
  치환해 옮긴다** — 바로 아래 [HARD] ②이고, **round22가 실제로 한 번 밟아 `9`를 냈다.**)

  ```
  $ grep -rno '\[라운드계수셀=[0-9]*\]' $S | wc -l
  8
  $ grep -rho '\[라운드계수셀=[0-9]*\]' $S | sort | uniq -c
        8 [<셀토큰>=22]
  ```

  **첫 명령 `8`(규칙과 일치) · 둘째 명령 한 줄(값 `22`로 전 셀 일치).**
  `R21-B`가 만든 상태(7/8 · 값 stale)에서 정상으로 복귀했고, 이번 라운드는 **반영 전에
  돌리고 반영 후에 다시 돌렸다** — 그것이 `R21-B` 진단(*"명령은 문서에 있고 실행은 사람에게
  있다"*)에 대해 **그 시점에** 가능한 전부였다. **강제(테스트) 신설은 그때 미착수였고**
  `R21-B`는 열려 있었다(§E.2ac 처방 후보 · §E.2ad 「미검증 잔여」).
  **[HARD · round22 신규] 이 검사기는 테스트 계수 네 자리를 보지 않는다** — 같은 `next:` 줄이
  **계수도 잃고 있었고**(반영 전 `next:` 안에 `passed` 0건) 검사기는 `8`을 냈다.
  처방 후보 ④로 등재했다(§E.2ac `R21-B` 처방 후보).

  **[round23 실측 · 문서 축 반영 직후 직접 실행 · 출력 원문]**
  (`<셀토큰>` 치환은 위와 같은 이유다 — **완성형을 적으면 검사기가 자기 설명문을 센다.**)
  ...
  **첫 명령 `8`(규칙과 일치) · 둘째 명령 한 줄(값 `23`으로 전 셀 일치).**

  **[HARD · round23 — `R21-B`가 닫혔다. 이 검사기는 이제 테스트로도 존재한다.]**
  **`server/tests/test_autopatch_specdocs.py`**(23건, round23 `DocGate` 신설)가 위 두 명령과
  **같은 것을 기계로 검사한다** — 토큰 개수 · 값 일치 · **그리고 이 규칙 문장이 스스로
  선언하는 셀 수(「여덟」)와의 3자 일치**까지 본다. 문서를 고쳤으면 돌려라:

  ```bash
  uv run pytest server/tests/test_autopatch_specdocs.py -q
  ```

  **[HARD] 이 문서의 검사기와 그 테스트가 갈라지면 그 테스트가 알린다** — 셀을 늘리는 것은
  개선이므로 막지 않되, **위 규칙 문장의 「여덟」과 그 파일의 `EXPECTED_CELL_COUNT`를 함께**
  올려야 통과한다. **그 결합이 요점이다**: round19 #1의 결함이 정확히 여기였다(규칙이 「넷」이라
  적는 동안 실제는 여덟이었다). 검사 명령을 문서에만 두는 것이 `R21-B`였고, 이제 실행이
  사람에게만 있지 않다.
  **[HARD] 그래도 이 테스트가 보지 않는 것 셋을 알고 써라**(그 파일 독스트링에 명기 ·
  round22 프로브 ⓓ가 실증한 **NOT CAUGHT**): ① 토큰 없는 문단·표가 통째로 사라져도 조용하다 ·
  ② **여덟 셀이 같은 값으로 함께 stale이면 통과한다**(값의 진위는 사람이 본다) ·
  ③ `next:`의 세 계수는 **존재만** 보고 값 일치는 보지 않는다. round22가 남긴 처방 후보
  (*"§0 두 자리와 값이 일치하는지 단정하는 검사"*)는 **아직 미착수**다.

  **[HARD] 두 가지 한계를 알고 써라.** ① **셀을 새로 만들면 토큰도 함께 박아라** — 토큰 없는
  셀은 이 검사기에 보이지 않는다(round19가 프로브로 확인했다: 표지 없는 stale 셀을 심어도
  두 명령 모두 통과했다). ② **이 규칙을 설명하는 산문 안에 토큰의 완성형을 쓰지 마라** —
  검사기가 자기 설명문을 셀로 세어 계수가 하나 늘어난다(round19 작성 중 실제로 9가 나왔다).
  설명할 때는 `` `[라운드계수셀=N]` `` 처럼 숫자 자리를 비워 둔다.

  경위: round14는 처음으로 치명 0건·fail-open 0건이었고 두 감사자 모두 "차단 문구 결함만
  고치면 코드 축은 GO"로 판정했으나, round15는 *"코드는 옳은데 그것을 지키는 테스트가
  게이트가 아니다"*(뮤테이션 48건 KILL 68.8% · 치명 2)로, round16은 *"게이트를 네 표에 붙이고
  형제 두 표에는 붙이지 않았다"*(79건 KILL 81.0% · 치명 SURVIVED 1 · 안전 high 3)로,
  round17은 *"게이트가 모듈 경계에서 멈춘다"*(치명 5건 전부가 미뮤테이션 모듈 + 신규 치명
  R17-A)로, round18은 *"규율을 같은 함수 안의 형제와 바로 옆 수치 축에 적용하지 않았다"*
  (뮤테이션 152건 KILL 91.4% / **행동 대조군만 83.6%** · 치명 **R18-A** — `_parse_fid_range`가
  `end < start`만 검사해 **음수·0 FID가 전달물에 실린다**)로,
  **round19**(사용자 결정에 따른 **축소 감사 1회** — round18 변경분만 겨냥했고 전체 재감사가
  아니다)는 *"고치는 행위가 새 결함을 두 건 만들었고 형제 표면을 세 건 또 빼먹었다"*
  (뮤테이션 23건 KILL **95.7% / 행동 대조군만 95.7%** — 두 수치가 같다는 것은 구조 전용
  방어로만 지켜지는 게이트가 **0개**라는 뜻이다 · 치명 0 · major 5 · minor 4 ·
  아홉 건 전부 fail-closed지만 **조작자에게 나가는 거짓 문장 4건**)로 FAIL했다.
  **round19 지적 반영에 대한 재감사가 통과해야 한다** · ② 세션 전 쇼파일 테스트용 재확인 · ③ 파괴적 조작 전 사전 상태 기록 ·
④ 테스트 자원 분리 — **[round16 추가 · round18 D-09 라벨 정정 → 2026-08-07 ㉠ 확정] 분리는
FID 대역(`fid_range`)으로만 가능하다.** `server/vwx/apply.py` `HandoffEntry` 독스트링(**[round19 #1]** 이전 판은 `:127`)이 `address`는 **"언제나 도면 주소
...
선택지는 **㉠ FID 대역만 분리하고 주소는 도면대로 간다** / **㉡ 분리된 유니버스로 도면을 다시
만든다** 둘이며, **[round18 D-09 정정] 2026-08-07 사용자가 ㉠(501~503)으로 확정했다** —
정본 등기는 `spec.md` §C 「사용자 결정 대기 → 결정 완료」다. **㉡은 폐기가 아니라 폴백**이다
(아래 드라이런 0건 갈래). 두 선택지를 남기는 이유는 근거 보존이지 미결이어서가 아니다.
...
Save As / Load 콘솔 절차는 전면 미실측이고, `server/safety/gate.py:283-292`(**[round19 #3] 이전
판은 `:286-293`이라 적었다 — 3줄 어긋난 인용이었다. 283행이 `@MX:NOTE` 시작이고 292행이 마지막
줄이며 **293행은 빈 줄**이다. 같은 오기가 `spec.md` §C P5 항과 `acceptance.md` AC-026 round17
블록에도 있었고 셋 다 고쳤다**)가 restore를
의도적으로 미구현으로 둔 사유를 명시한다. **[round18 D-09 정정] 2026-08-07 사용자가
`P5-D` + `P5-0` 필수로 확정했다**(정본 등기 `spec.md` §C 「사용자 결정 대기 → 결정 완료」).
4안과 증거 등급은 근거 보존을 위해 남긴다 — A·B·C는 P5-D의 로드가 실패했을 때의 폴백이다.
세션 전에 남은 일은 선택이 아니라 **아래 ⑦의 네 기재물을 적는 것**이다 ·
...
⑦ **[신설 · AC-026④ ㉢ 승격 · round18 D-10으로 본문 열거에 편입] P5 절차가 세션 전에 문서로
확정되지 않으면 세션에 착수하지 않는다.** 이전 판은 이 항목을 위 결정 블록쿼트 안에만 두어
**본문 열거가 ①~⑥에서 끝났다** — 착수 차단 조건이 열거 밖에 있으면 열거만 읽는 사람에게
닿지 않는다. 사용자가 AC-026④ 공백에 대해 **㉢**(P5를 착수 차단 조건으로 승격)을 택했으므로
`acceptance.md`는 **개정하지 않고**(AC 27 · AC-026 ①~⑦ 불변) **그 강제가 여기에 있다.**
구체적으로 세션 착수 **전에** `progress.md` §E.2 M8 절에 다음 **네 항목**이 적혀 있어야 한다:
**① 스냅샷 파일명 ② 스냅샷을 뜬 시각 ③ 재로드 절차 ④ 재로드 후 responder 재배포·OSC 토글 계획.**
네 항목은 `spec.md` §C 결정 표 P5 행 · `M8-REDEFINITION-DRAFT.md` §3 P5 행과 **같은 넷**이다 —
셋 중 하나만 고치면 어긋난다(round18 D-06이 정확히 그 어긋남이었다).
**[HARD] ⑦은 "제거 불가"를 배제하지 않는다** — 스냅샷이 **있음**을 보장할 뿐 **로드가 성공함**을
보장하지 않는다. 제거하지 못한 채 세션이 끝나면 **AC-026⑤대로 "G4에서 제거 수단 부재로 실패"**라
적고 ㉠·㉡을 다시 묻는다.
