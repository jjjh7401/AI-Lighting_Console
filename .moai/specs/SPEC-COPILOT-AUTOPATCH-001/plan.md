...
**착수 전제**: ① 독립 코드 감사 PASS — **[round18 D-05 재정정] round11~18 여덟 라운드
전부 FAIL이었다.**

  **[HARD · round18 D-05] 이 셀은 매 라운드 바뀐다 — 규칙으로 적는다.** 라운드 계수를 담은
  셀은 **`plan.md` §B 착수 전제 ①(이 셀) · `M8-REDEFINITION-DRAFT.md` §3 P1 · `spec.md:44`
  v0.1.5 HISTORY 행 · `progress.md` 프론트매터 `next:` 넷이며, 라운드마다 넷을 함께 고친다.**
  한 곳만 고치면 형제가 반례로 남는다(round16 #3은 셋 중 둘만 알고 있었다).
  **round16 #3이 정확히 이 셀을 정정했는데 round18이 같은 셀을 다시 stale로 잡았다** —
  *어떤 규율을 적용하고 형제 표면에는 적용하지 않는다*의 **여덟 번째 동형 재발**이고, 이번
  형제는 **한 라운드 뒤의 자기 자신**이었다. 정정 자체가 재발을 막지 못한다는 것이 이 셀의 교훈이다.
  ...
  `end < start`만 검사해 **음수·0 FID가 전달물에 실린다**)로 FAIL했다.
  **round18 지적 반영에 대한 재감사가 통과해야 한다** · ② 세션 전 쇼파일 테스트용 재확인 · ③ 파괴적 조작 전 사전 상태 기록 ·
④ 테스트 자원 분리 — **[round16 추가 · round18 D-09 라벨 정정 → 2026-08-07 ㉠ 확정] 분리는
FID 대역(`fid_range`)으로만 가능하다.** `server/vwx/apply.py:127`이 `HandoffEntry`의 `address`는 **"언제나 도면 주소
