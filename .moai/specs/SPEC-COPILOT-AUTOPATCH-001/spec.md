...
| 0.1.5 | 2026-08-06 | orchestrator | **M8 재정의 amendment (사용자 승인 2026-08-06) + 독립 코드 감사 round13·round14 반영.** `AC-AUTOPATCH-026`이 v0.1.2 자동 실행 모델의 문장("실행"을 왕복의 한 칸으로 두고 "시스템이 1회 통과")을 그대로 이고 있었다 — v0.1.3이 REQ-AUTOPATCH-018을 반자동으로 조정할 때 함께 고쳐지지 않은 누락이다. **4관문(G1 서버 전달 · G2 사람 실행 · G3 서버 검증 · G4 원복)으로 분리**해 책임과 실패 모드를 갈랐고, **G1 통과 + G2 실패를 유효한 결과로 명시**했다(사람 실행 경로는 한 번도 시험된 적이 없으므로 그 실패는 새 실측이지 코드 결함이 아니다). **표시 문자열 판별 실험**을 세션 항목으로 편입(⑦). **REQ 26건·AC 27건 수 불변** — AC-026 내부만 정밀화했다. 함께: **독립 코드 감사 round11·12·13·14 — 넷 다 FAIL, 지적 누계 59건 반영.** round11(4명 전원 FAIL, 치명 2건)·round12(round11 수정이 만든 치명 2건)·round13(round12 수정이 만든 치명 1건)·round14(**처음으로 치명 0건·fail-open 0건**, 두 감사자 모두 "차단 문구만 고치면 코드 축 GO") — 네 건 모두 작성자 테스트를 전부 통과한 채 살아 있었다(`progress.md` §E.2 round11·round12 절, `.moai/reports/plan-audit/…-round11.md`·`-round12.md`). **[round16 #3 정정 · round18 D-20 재정정]** 이 행은 *"독립 코드 감사 round11·12·13·14 — 넷 다 FAIL"*에 멈춘 stale이었다 — v0.1.5는 이후 **round15·16·17·18 지적 반영까지** 담았고 **독립 감사는 round11~23 열세 라운드 전부 감사 FAIL**이다 `[라운드계수셀=23]` **[round23 갱신]**(round20은 **감사 FAIL이면서 세션 GO(조건 3)** — §E.2ab. **round21에서 `=21`로 올리지 않은 것은 round21이 R20-A·R20-B·R20-D 반영이지 감사가 아니었기 때문이고, round22·round23은 감사이므로 올린다** — §E.2ac·§E.2ad·§E.2ae) **[round19 #1 재정정]**(round15: 뮤테이션 48건 KILL 68.8% · 치명 2 · 문서 major 6/minor 2 · round16: 뮤테이션 79건 KILL 81.0% · 치명 1 · 안전 high 3 · 문서 major 4/minor 6 · round17: 치명 5 + 신규 치명 R17-A · round18: 뮤테이션 152건 KILL 91.4%/**행동 대조군만 83.6%** · 치명 R18-A 1건 · 문서 major 7/medium 9/minor 5; round19 — 사용자 결정에 따른 **축소 감사 1회**, 뮤테이션 23건 KILL **95.7%/행동 대조군만 95.7%**, 치명 0·major 5·minor 4, 아홉 건 전부 fail-closed지만 **거짓 문장 4건**; **round22 — round21 반영분 재감사, 두 감사자 전원 FAIL/NO-GO 「실사용 불가」, 안전 축은 쓰기 EPERM으로 뮤테이션 미산출(round19식 약화 4점 공시 승계)·적대 축 95건 KILL 88.4%/행동 대조군만 88.4%, 치명 2·high 1·medium 1·low 2, 치명 2건이 전부 round21 처방의 산물이고 그중 R22-C는 fail-closed 배제를 「잘못된 타입 확정」으로 뒤집었다**; **round23 — round22 반영분 재감사, FAIL / 릴리스 NO-GO이면서 M8 세션 재개는 GO**(GO 근거는 실물 responder가 `childCount`를 항상 실어 R23-1의 발화 조건이 오늘의 라이브 경로에 나타나지 않는다는 것 — 무해가 아니라 오늘 실현되지 않는다는 뜻), 뮤테이션 28건 KILL **92.9%/행동 대조군만 96.3%**(SURVIVED 2 = 등가 1 + R23-4 1), 하네스 **약화 4점 공시**(쓰기 전면 EPERM은 round19·22에 이은 세 번째 재발), high 1·medium 1·low 2, **둘 다 round22 처방의 산물**이고 R23-1은 R22-E 처방을 점유폭 부재 갈래의 **한 축에만** 걸어 **같은 스냅샷이 갈래에 따라 반대로 말하게** 만들었다 — **확정을 막는 것보다 강한 주장이 더 느슨한 술어로 나갔다**; 반영 4축 중 `AxisParity`는 **처방을 그대로 따르지 않고** *「두 갈래는 다른 질문이다」*를 입증해 **분해**를 택했고(공유 하위질문 → `_absence_unverifiable` 한 자리), **감사가 놓친 형제 둘**도 함께 닫았다; `R21-A`·`R21-B` 종결 — `R21-B`는 `server/tests/test_autopatch_specdocs.py` 23건 신설로 **문서 규율에 처음으로 기계 강제**가 생긴 것이다). **[round18 D-20 · round19 #1 확대 · 규칙] 이 셀은 매 라운드 바뀐다 — 라운드 계수를 담은 셀은 **여덟**이며 항상 함께 고친다: 이 행 · `plan.md` §B 착수 전제 ① · `M8-REDEFINITION-DRAFT.md` §3 P1 · `progress.md` 프론트매터 `next:` · `progress.md` §0 헤드라인 「독립 코드 감사 round11~N」 · `progress.md` §0 「착수 전제는 독립 코드 감사 재감사 PASS다(round11~N …)」 문단 · `design.md:3` status 줄 · `M0-SHOWFILE-SPEC.md` §5.1 P1 행. 정본 열거는 `plan.md` §B 착수 전제 ① 아래 [HARD] 블록이다. round16 #3이 셋 중 둘을 고친 뒤 round18에서 **같은 셀이 다시 stale**로 잡혔고(여덟 번째 동형 재발), **round19는 이 규칙의 열거 자체가 형제 넷(`progress.md` §0 헤드라인 — `round11~`**`17`** 두 라운드 stale · `progress.md` §0 착수 전제 문단 — `round11~`**`16`** 세 라운드 stale · `design.md:3` status 줄 — `round11~`**`16`** 세 라운드 stale · `M0-SHOWFILE-SPEC.md` P1 행)을 빠뜨리고 있었음을 전수 grep으로 찾았다 — 규칙이 **넷**을 셀 때 실제로는 **여덟**이었다. 아홉 번째 동형 재발이며 이번엔 규칙을 적는 문장 자체였다.** 한 곳만 고치면 형제가 반례로 남는다. **REQ 26·AC 27 수는 그대로다.** 또 위 문장의 *"그 실패는 새 실측이지 코드 결함이 아니다"*는 **round14 T12가 폐기**했다 — 정본은 `acceptance.md` AC-AUTOPATCH-026⑥이며, 배제되는 것은 "전달물이 옳았는데도 실패했다"는 경우뿐이고 기본 분류는 `판별 불가`다. |
...
>
> **[HARD] round20을 "생략"으로 읽지 마라.** 미착수이지 면제가 아니다.
> **[round21 갱신]** 그리고 **집행됐다** — round20은 2026-08-08 세션 직전에 1회 돌아
> 감사 FAIL · 세션 **GO(조건 3)**로 P1을 통과시켰다(`progress.md` §E.2ab).
> **round21은 그 감사의 지적 3건(R20-A·R20-B·R20-D)에 대한 사용자 지시 반영이며
> 감사가 아니다** — 그 반영에 대한 재감사가 **round22**다(§E.2ac). `plan.md` §B M8 착수 전제 ①
> (독립 코드 감사 PASS)은 **그대로 유효**하며, 이 결정은 그 게이트를 **언제 통과시킬지**만 정한 것이다.
> 세션 직전 감사가 FAIL이면 세션은 착수하지 않는다.
> **[round22 갱신] round22도 집행됐고 판정은 두 감사자 전원 FAIL / NO-GO · 「실사용 불가」다**
> — 치명 2(`R22-A`·`R22-C`) · high 1(`R22-E`) · medium 1(`R22-B`) · low 2(`R22-D`·`R22-F`)이며
> **치명 2건이 전부 round21 처방의 산물**이다(정본 `progress.md` **§E.2ad**).
> **round22 반영분에 대한 재감사가 round23이며, 그것도 집행됐다.**
> **[round23 갱신 · 집행·판정] round23의 판정은 FAIL / 릴리스 NO-GO이면서 M8 세션 재개는 GO다**
> — high 1(`R23-1`) · medium 1(`R23-2`) · low 2(`R23-3`·`R23-4`) · 뮤테이션 28건 중 26 KILLED
> (92.9% 전체 / 96.3% 행동) · 하네스 **약화 4점 공시**(정본 `progress.md` **§E.2ae**).
> **round20에 이어 두 번째로 판정이 갈라졌고 그 갈라짐은 이 결정이 정한 관문 그대로다** —
> 관문은 *"감사가 무결함"*이 아니라 *"감사가 이 착수를 승인하는가"*다.
> **세션 재개 GO의 근거**: 실물 responder가 열거 응답에 `childCount`를 **항상** 실어
> `R23-1`의 발화 조건이 **오늘의 라이브 경로에 나타나지 않는다** — **무해하다는 뜻이 아니라
> 오늘 실현되지 않는다는 뜻**이고, **릴리스 NO-GO는 그대로다.**
> **[HARD] 그러므로 착수 전제 ①(독립 코드 감사 PASS)은 여전히 미충족이다.** 이 결정이
> 정한 것은 *"세션 직전 감사가 세션을 승인하면 그 세션은 착수한다"*이며 **릴리스 승인과는
> 다른 관문**이다 — 둘을 뭉개서 *"round23이 GO를 냈으니 P1이 통과했다"*로 읽지 마라.
> **round23 반영분에 대한 재감사가 round24이고 미착수다.**
