...
### 사용자 결정 대기 → **[round17 · 2026-08-07 사용자 결정 완료] 네 건 전부 확정됐다**
...
> | 항목 | 결정 | 남은 조건 |
> |---|---|---|
> | **P5 원복 절차** | **P5-D** — 세션 **전** GUI Save As로 별도 사본 → 세션 후 Load. **P5-0**(플러그인 슬롯 `Delete Plugin <slot>` 역순 제거)은 **어느 안이든 필수**로 함께 수행 | Save As / Load 콘솔 절차는 **전면 미실측**이다. 스냅샷 파일명을 `progress.md` §E.2 M8 절에 기록하고, 재로드 후 **responder 재배포·OSC row 토글**이 필요할 수 있다 |
...
- **[round17 등기 · P5] 세션 후 원복 절차** — 서버는 삭제 명령을 **생성하지 않으므로**(아래 §D
  ...
  **P5-D의 강점은 문법 증거가 아니라 FID·슬롯 오지정 위험이 0이라는 구조적 성질에서 온다** —
  ...
  `server/safety/gate.py:286-293`이 restore 미구현 사유를 명시한다). 사용자가 **세션 전에**
  하나를 확정해야 한다.
...
- **[round16 추가 · AC-026④] AC-AUTOPATCH-026④(G4)가 "제거 불가"를 판정하지 못한다 — 규정 공백** —
  ...
  하는 일이며(위 P5가 미정인 이유), **끝내 제거하지 못했을 때 ④를 어떻게 판정하는지가
  `acceptance.md`·`plan.md`·`M8-REDEFINITION-DRAFT.md` 어디에도 없다.** 선택지는 **㉠·㉡·㉢
  ...
  **[round17 정정] "P5-D를 기본값으로 고르면 이 공백이 닫혀 AC-026④ 개정이 불필요해진다"는
  ...
  상태의 확률을 크게 낮출 뿐 배제하지 않는다.** 따라서 이 항목은 P5 결정과 **독립으로** 남는다.
