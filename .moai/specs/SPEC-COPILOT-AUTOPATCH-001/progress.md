...
uv run pytest server/tests -q             # 현재 기준선: 5123 passed / 7 skipped (착수 4898)
...
2d. **~~[BLOCKER]~~ [해소] `Inventory` 절단은 M7 후속에서 막았다 — 되돌리지 마라**:
...
next: "**M8만 남았다**(라이브 종단, AC-026 — 사람 실행 단계를 포함하도록 **재정의 필요**, 미착수). 코드 마일스톤 M1~M7은 전부 완료다. **M7 후속으로 `Inventory` 절단 가드도 넣었다**(+11) — 미판독이 남은 재조회에서는 생성 대상을 전부 막고 검증도 "없음"을 단정하지 않는다(§E.2 M7 후속 절). **M7 완료**(툴 배선 · 회귀 · PRESERVE, +30, §E.2 M7 절) — `apply_vectorworks_patch` 5지점 등록, 1단계 payload에 도면 열 3종 추가(최상위 계약은 **변경 전 코드에서 뜬 스냅샷**과 대조해 불변 확인), PRESERVE 0-diff를 심었다 되돌리는 대조군으로 확인, SONGCUE의 tools.py 훅 스냅샷 재산책(순수 추가 +262 −0 · 보호 구간 겹침 0). **M6 완료**(+38) · **M5 완료**(+63) · **M4 완료**(+38). round10 독립 감사가 v0.1.4를 **PASS(0.865)** 했고 지적 누계 N1~N55 **전량 반영**. 남은 사용자 접점 2건: ① **L2 측정**(패치 편집 세션 진입 — 선택. 긍정이면 REQ-AUTOPATCH-018을 v0.1.2 형태로 복원 가능) · ② M8 라이브 종단 세션 일정. 착수 전 `spec.md` §A 사실 7·8과 §0 함정 9·10·11, 그리고 §0 항목 2b·2c·2d(M5·M6·M7이 남긴 계약)를 읽어라. **M8은 독립 재감사(round11) 대상이기도 하다 — M5~M7 전량이 아직 감사되지 않았다.**"
```
