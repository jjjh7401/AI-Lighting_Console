...
next: "**M8만 남았고, 남은 것은 ① round19 반영에 대한 재감사(round20 — **[2026-08-08 사용자 결정] M8 세션 직전 1회**. 미착수이지 면제가 아니다) ② 사용자의 라이브 세션 둘뿐이다.** 독립 코드 감사를 **아홉 번**(round11~19) 돌렸고 **아홉 다 FAIL**이었다 `[라운드계수셀=19]`. **round19는 사용자 결정에 따른 축소 감사 1회**(전체 재감사가 아니라 round18 커밋 `d03597b`가 건드린 것만 좁게 겨냥) 였고 **치명 0 · major 5 · minor 4 = 결함 9건 · 뮤테이션 23건 KILL 95.7%(전체) / 95.7%(행동 대조군만)**로 FAIL/NO-GO였다. **두 수치가 같다는 것 자체가 결과다** — round18 게이트 중 구조 전용 방어로만 지켜지는 것이 0개이고, 생존 1건은 진짜 공백(R18-A 라벨 무방비)이다. 감사의 제1질문 *「고치는 행위가 새 결함을 만들었는가」*에 답은 **그렇다 두 건**, *「형제 표면을 또 빼먹었는가」*에도 **그렇다 세 건**이며 **아홉 라운드 연속 같은 기제**다 (형제가 ① 같은 payload 안의 다른 키 ② 같은 함수 20줄 아래 ③ 고지가 말하는 두 축 중 한 쪽에 있었다). 아홉 건 전부 fail-closed지만 **조작자에게 나가는 거짓 문장 4건**이 남아 통과시키지 않았다. **round17 치명 5건과 R18-A는 CLOSED로 독립 확인**됐다. 별건 둘: **[치명] 현행 39대 쇼파일에서는 FID 사전검사가 상시 실패해 M8 대상이 구조적으로 0건**(§0 2d 경고 · `M0-SHOWFILE-SPEC.md` §1.3) · 형제 아티팩트 `gate.py` restore 블록 인용 드리프트 3건 정정(`:286-293` → **`:283-292`**). 하네스 위양성 기제는 **다섯에서 여섯**으로 늘었고(⑥ 임시 디렉터리 부재), **파일 격리가 EPERM으로 불가능할 때의 in-memory 대체 경로**가 §0에 정식 등재됐다. 라이브 세션은 사용자 결정에 따라 **테스트 쇼파일 사양·체크리스트를 먼저 문서로 만든 뒤** 잡는다. 테스트 **6,870 passed / 7 skipped**(round19 문서 축 반영 시점 · 형제 코드 축 반영 뒤 오케스트레이터가 갱신한다 — 갱신할 자리 넷 중 ⓓ가 이 필드다)."
```
