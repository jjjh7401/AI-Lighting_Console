...
**독립 코드 감사 round11·12·13·14 — 넷 다 FAIL, 지적 59건 반영.
round14는 이 사이클 최초로 치명 0건·fail-open 0건이었고 두 감사자 모두
"차단 문구만 고치면 코드 축 GO"로 판정했다(그 반영이 이번 커밋이다).**
**남은 것은 ① round15 재감사 ② 사용자의 라이브 세션 둘뿐이다.**
테스트 **5,234 passed / 7 skipped**(착수 4,898 → +336, 회귀 0) ·
커밋 **run-phase 23건**(`9f7516c..HEAD` · `ca00bc5..HEAD` 총 27건 — **본 갱신을 담은 커밋을
...
uv run pytest server/tests -q             # 현재 기준선: 5173 passed / 7 skipped (착수 4898)
...
spec_version: "0.1.5"     # v0.1.5 = v0.1.4 + M8 재정의 amendment(AC-026 4관문, 사용자 승인) + 독립 코드 감사 round11·12·13·14 지적 59건 반영. REQ 26 · AC 27 수 불변(§C.0 26행 · §C.0a 합 27 유지) — AC-026 내부만 정밀화
...
next: "**M8만 남았고, 남은 것은 ① round15 재감사 ② 사용자의 라이브 세션 둘뿐이다.** 독립 코드 감사를 **네 번** 돌렸고 넷 다 FAIL이었으나 **round14는 이 사이클 최초로 치명 0건·fail-open 0건**이고 두 감사자 모두 GO/NO-GO에서 **\"차단 문구만 고치면 코드 축 GO\"**로 판정했다 — 그 반영이 끝났다(지적 누계 59건, 5,234 passed / 7 skipped, 회귀 0). 적대 감사자 뮤테이션 23건 중 21건이 잡혔고 **갈래 경계 뮤테이션까지 전부** 잡혔다. **남은 위험은 코드가 아니라 문서 축이었다** — amendment가 §F DoD 교차참조를 깨뜨렸고(정정 완료), **구 AC-026①(종단 생성 요구)이 삭제돼 제품 효능이 DoD 밖으로 나갔다**(의도된 결과이나 미기록이었다 — 고지 블록 신설). **M8 재정의는 완료**(v0.1.5, 사용자 승인 — 4관문 G1~G4 · REQ 26·AC 27 수 불변). 세션 준비물은 `M8-REDEFINITION-DRAFT.md` §3(P1~P5)이고 **미정은 P5 원복 절차 하나**다 — 사용자와 합의해야 한다. 세션에 **표시 문자열 판별 실험**(AC-026⑦)을 포함하고, **`unpatched` 미실측 가정**(`console_read.unpatched_count`)을 세션 전에 눈으로 대조하라. 코드 마일스톤 M1~M7 완료. 착수 전 §0 함정 9·10·11과 항목 2b·2c·2d, §E.2 round11~14 절의 [HARD] 패턴을 읽어라."
```
...
전부 잡혔다.** 도메인을 표로 열거한 것은 실제로 효과가 있었다. 수치 대조도 전 항목 일치로,
세 라운드 연속 지적됐던 계수 stale이 처음으로 0건이다.
