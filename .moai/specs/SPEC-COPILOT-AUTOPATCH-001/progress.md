...
| **E** | 미확인 신규 6건 — `U-19` 쇼파일명 판독 · `U-20` 콘솔 종료 원인 · `U-21` 편집기 열림의 관측 표지(**round24 후속에서 부분 답 — 프롬프트 문자열이나 서버가 못 읽음**) · `U-22` Patch 주소 빨간색 의미 · `U-23` 안전망 파일 식별 · **`U-24` 사람 트리거의 무엇이 다른가**(round24 후속 통제 실험: 목적지를 픽스처 계층에 맞춘 채 서버가 발화해도 0건) | 조사 |
...
>
> **[round24 후속 실측 2] 트리거 축은 목적지와 **별개**다 — 통제 실험으로 갈랐다.**
> 위 표지를 찾은 뒤, **목적지를 셋째 상태로 맞춘 채 서버가 발화**했다. 프롬프트가
> `Admin@ShowData/LivePatch/Stages/Stage 1/Fixtures>`임을 발화 **전후로 스크린샷 확인**했고,
> `Plugin 'CopilotPatch'`(6건 AddFixtures, 생성기 산출)가 `ok=true / OK`로 돌아왔다.
> 결과: 픽스처 **40 -> 40**. 즉 **목적지가 맞아도 서버 발화는 만들지 못한다.**
> 이전 두 축("사람 실행" · "편집기 열림")이 사실은 **독립**임이 이것으로 확정됐다 — 앞선
> L2 시험이 목적지를 한 계층 위에 둔 채 돈 것과 혼동될 여지를 이번 통제가 제거했다.
> 남은 물음은 *"사람 트리거의 무엇이 다른가"*이며 이 세션은 판정하지 않는다
> `[미확인]` **U-24**.
>
> **부수 발견 — `ShCuts` 토글.** 명령줄 우측 `ShCuts`(키보드 단축키) 표시가 **빨강**이면
> 콘솔이 키보드 입력을 받지 않는다. 오늘 사용자가 두 차례 실행했다고 했으나 Command Line
> History에 `Plugin ...` 항목이 **한 건도 남지 않은** 것이 이것으로 설명될 수 있다(확정은
> 아니다 — 사용자 조작을 관측하지 못했다). 세션 끝에 노랑(켜짐)으로 돌려 두었다.
> 또한 **합성 입력은 콘솔에 닿지 않는다**: `cliclick`·System Events 키 입력 모두 프롬프트에
> 한 글자도 넣지 못했다(클릭은 정상 동작). 그래서 이 검증의 마지막 칸은 조사자가 대신
> 수행할 수 없다.
