...
known_gaps:
  ...
  - "[CLOSED · round24 후속 실측] L2(패치 편집 세션 활성 상태에서 AddFixtures 실행) — **NEGATIVE**.
    실물 grandMA3 onPC 2.4.2에서 Patch 편집기를 열어 둔 채(모달 없음, Live Patch 표 표시 상태)
    `luagen`이 생성한 AddFixtures 플러그인을 Import·실행했다. 결과: 픽스처 40 -> 40,
    `Fixture 901`·`Fixture 902` 모두 `Illegal object`. **플러그인이 돌지 않은 것이 아니다** —
    같은 플러그인이 남긴 진단 매크로가 `ft=true mode=true pcall=true err=nil`이었다. 즉 타입·모드
    핸들을 얻었고 AddFixtures가 예외 없이 반환했는데도 아무것도 생기지 않았다. 이로써 REQ-AUTOPATCH-018
    (서버는 패치를 실행하지 않는다)의 가역성 조건이 소멸하고, 반자동 실행 모델이 확정 결론이 된다.
    부수 관측 2건: (1) 이미 찬 플러그인 슬롯에 Import하면 덮어쓰기 확인 UI 때문에
    `User Canceled Command`가 오는데 **실제로는 임포트가 완료돼 있었다** — 이 응답은 실패 근거로
    쓸 수 없다. (2) 모달 대화상자가 떠 있어도 OSC 명령과 상태 판독은 정상 동작한다."
