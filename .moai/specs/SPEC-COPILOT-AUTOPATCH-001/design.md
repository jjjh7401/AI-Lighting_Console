...
status: draft (v0.1.5, 2026-08-06) · Tier L · 설계 슬롯 5건 전부 종결 · 미결 0 · **plan-audit 1~10회차 + 독립 코드 감사 round11~23 지적 반영** `[라운드계수셀=23]`(**[round23 갱신]** **열세 라운드 전부 감사 FAIL**이며 round22는 **두 감사자 전원 FAIL/NO-GO · 「실사용 불가」**(치명 2·high 1·medium 1·low 2 · `progress.md` §E.2ad)였고, **round23은 FAIL / 릴리스 NO-GO이면서 M8 세션 재개는 GO**다(high 1 `R23-1` · medium 1 `R23-2` · low 2 · 뮤테이션 28건 중 26 KILLED = 92.9% 전체 / 96.3% 행동 · 하네스 약화 4점 공시 · `progress.md` §E.2ae). **round20에 이어 두 번째로 판정이 갈라졌다.** round20은 **감사 FAIL · 세션 GO(조건 3)**였다. **round21에서 `=21`로 올리지 않은 것은 그것이 반영이지 감사가 아니었기 때문이고, round22·round23은 감사이므로 올린다**(§E.2ac·§E.2ad·§E.2ae). **[round19 #1 정정]** 그 이전 판은 `round11~16`으로 **세 라운드 stale**이었다 — round15 #7이 같은 줄을 고치고도 형제 전파를 다시 놓친 것이며, 이 줄은 라운드 계수 셀 **여덟** 중 하나다) · §6.2 폐지(acceptance.md 단일 출처화). **[round15 #7]** 이전 판은 'plan-audit 1~9회차'에 멈춰 있어 round10 PASS와 코드 감사 5라운드가 반영 이력에서 빠져 있었다 — T13 버전 전파의 형제 미처리.
...
### §5.1 **[round23 R23-1 신설] 「말할 자격」과 「무엇을 해야 하는가」는 다른 층이다**

**설계 사실로 등재한다 — 미결 슬롯이 아니라 이미 확정된 구조다.**

타입·모드 라이브러리 판독은 두 층으로 답한다.

| 층 | 묻는 것 | 대표 | 정본 |
|---|---|---|---|
| 처분 층 | *"무엇을 해야 하는가"* | 절단 · 폐기 · 판독 실패 **세 축을 분리 유지** | `progress.md` §0 2d 세 축 표 `[round21]` |
| **자격 층** | *"이 목록에 대해 말할 자격이 있는가"* | **`_absence_unverifiable`**(round23 신설) | 본 절 |

**왜 자격 층이 따로 필요한가 — `None`의 두 뜻.**
`_library_incompleteness`가 돌려주는 `None`은 **두 가지 뜻**을 진다:
① **「전수임을 확인했다」**(불완전 근거가 없다) · ② **「전수인지 말할 근거가 없다」**
(선언 총계를 못 읽었다). **부재 가드 셋이 그 둘을 같게 읽고 있었다** —
`_absence_assertable`(점유폭 축) · `fixture_type_not_in_library` · `dmx_mode_not_in_library`.
그 결과 **같은 스냅샷이 갈래에 따라 반대로 말했다**: 확정 게이트는 막고(`library_incomplete`),
부재 가드는 **하드 스톱 + *"모드 열거를 전부 읽었고 절단도 없었다"*라는 거짓 문장**을 냈다.
**확정을 막는 것보다 강한 주장(부재 단정)이 더 느슨한 술어로 나간 것**이 `R23-1`이다.

**결정 — 복사가 아니라 분해.** 두 갈래는 **같은 질문이 아니다**: `_absence_assertable`은
**채널 수** 위의 부재 술어라 모드마다 `channel_count`를 읽어야 하고, 확정 게이트는 그것을
묻지 않는다. **공유하는 것은 「이 모드 목록이 전수인가」 하나뿐**이므로 그 하나를
**`_absence_unverifiable` 한 자리로 뽑고** 갈래 고유 전제는 각자에게 남긴다.
분해를 값으로 고정하는 **동치 게이트**가 이 분해가 다시 갈라지는 것을 막는다.

**따라오는 규율 둘.**
1. **부재를 단정하는 술어는 「전수임을 확인했다」를 자기 근거로 들어야 한다.**
   *"관측을 늘리는 것이 판정을 승격하지 않는다"*(round21)의 **자격 층 판**이다.
2. **사유 문장은 자기 근거를 말한다.** 그래서 사유문 3종을 *"모드 열거를 전부 읽었고"* →
   ***"선언된 모드 총계와 대조해 열거가 전수임을 확인했고"***로 고쳤다 —
   문장이 근거를 말하면 근거가 사라졌을 때 **문장이 먼저 틀린다.**

**[HARD] 이 층을 처분 축 셋으로 더 쪼개서 막으려 하지 마라.** 세 축 표는 *"무엇을 해야
하는가"*를 가르는 표이고 `R23-1`은 그보다 **앞선 층**에서 났다. 처분 축을 늘리면
round19 major#4가 고친 *"한 payload가 같은 후보에 모순된 말을 한다"*가 재발한다 —
round22 음성 결과 ②가 그 판단을 독립 검증했다. 전문·귀속은 `progress.md` **§E.2ae**.
