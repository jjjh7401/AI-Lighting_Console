...
status: draft (v0.1.0, 2026-08-05) · Tier L · 설계 슬롯 5건 전부 종결 · 미결 0
...
| # | 위험 | 완화 | 잔여 |
|---|---|---|---|
| R1 | FID 충돌로 엉뚱한 픽스처를 덮음 | 사용자 범위 강제 + (GO 시) 충돌 사전검사 + 배정 전수 나열 | 사용자가 범위를 틀리게 주면 막을 수 없다. 승인 화면에 그 의존을 명시 |
...
| R5 | 역산된 주소(`absolute_back_calculated`)로 엉뚱한 유니버스에 패치 | **슬롯 D** 참조 — 승인 화면에 근거 등급 노출 | 전제가 틀린 쇼파일에서는 근본적으로 막을 수 없다 |
