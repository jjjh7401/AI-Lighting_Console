...
status: draft (v0.3.0, 2026-07-26). 본 문서는 룩 라이브러리(연출 어휘 계층)가 얹힐 기존 코드베이스를 file:line 근거로 분석한다. **구현 코드는 제안하지 않는다 — 분석 전용.** 라이브 콘솔 조사는 이번 plan-phase 세션에서 수행하지 않았다(기존 SPEC들의 라이브 검증 결과와 룰북의 "validated" 표기를 재사용한다).
