...
status: draft (v0.3.2, 2026-07-26). 본 문서는 룩 라이브러리(연출 어휘 계층)가 얹힐 기존 코드베이스를 file:line 근거로 분석한다. **구현 코드는 제안하지 않는다 — 분석 전용.** 라이브 콘솔 조사는 이번 plan-phase 세션에서 수행하지 않았다(기존 SPEC들의 라이브 검증 결과와 룰북의 "validated" 표기를 재사용한다).

> **v0.3.2 — 본문 무변경. 스윕 결과만 기록한다.** v0.3.2의 표적 정정 2건(AC-008 ③ 검증 수단 → AST 스캔 / 미매핑 사유 `unaddressable` 등재)에 대해 본 문서를 훑은 결과, **고칠 하류 참조가 없었다.** 근거: (a) 본 문서가 `gate.screen()`을 언급하는 세 곳(§5.5 함의, §6 안전 불변식, §참조 표)은 **파이프라인의 사실 서술**이지 AC-008의 검증 수단 인용이 아니다 — 수단이 바뀌어도 서술은 그대로 참이다. (b) 본 문서는 미매핑 **사유 집합을 열거하지 않는다**(리그 형상 `rig_object`/`rig_section`과 `no` 키잉은 분석하되, 사유 어휘는 spec.md REQ-009가 정본이다). 버전 줄만 올리는 이유는 v0.3.1 개정이 실제로 이 문서를 건드렸기 때문에 **버전 정체가 "누락"인지 "무변경"인지 구분되지 않기 때문**이다 — 스윕이 이 문서에 도달했음을 남긴다.
>
