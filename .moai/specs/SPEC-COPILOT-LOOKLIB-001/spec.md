...
- **v1 룩 속성 범위**: 컬러·강도·빔(스트로브/줌 등)은 **구체값**으로; 포지션은 **역할 기반 추상**(백라이트, FOH 워시, 사이드 등)으로 표현하고 인스턴스화 시점에 사용자 리그의 그룹으로 매핑한다 — 하드 pan/tilt 값을 룩 데이터에 넣지 않는다 (사용자 확정 ①).
- **v1 내장 장르**: 워십, 록, 발라드, EDM 4종. 장르당 약 6~10룩으로 섹션 다이내믹스(잔잔함→클라이맥스)를 스팬한다 (사용자 확정 ②).
- **v1 범위**: 라이브러리 데이터 계층 + MA3 인스턴스화(콘솔에 프리셋/그룹 생성) + 자연어 매칭("웅장한 금색 코러스" 채팅 지시 → 룩 매칭·적용) **전부 v1** — 데이터 계층만이 아니라 완결된 사용자 대면 기능이다 (사용자 확정 ③).
- **스키마는 P1-1/P1-2의 공통 기반**: 섹션 다이내믹스는 순서 있는 축으로, 인스턴스화는 룩 단위/장르 묶음 단위 모두 표현 가능한 형상으로 설계한다 (제안서 §3, research.md §10).
- **안전 철학 계승**: AI는 초안 생성·사람이 확정(product.md §6). 룩 적용은 기존 게이트·승인 카드·LiveLock 제안 강등 플로우를 그대로 소비한다.

### ⚠️ 역할 매핑의 명시적 한계

역할→그룹 매핑은 사용자 쇼파일의 **그룹 명명 관례**(예: 'Wash', '워시', 'Back', '백라이트' 류 — 20_korean_terms.md의 showfile 어휘 클래스)에 기반한 휴리스틱이다. 이름 관례가 전혀 없는 리그에서는 역할이 매핑되지 않을 수 있으며, 그 경우 시스템은 **매핑 실패를 명시적으로 보고하고 해당 역할을 제외한 채 진행하거나 중단**한다 — 절대 임의의 그룹·픽스처를 추측해 대입하지 않는다(룰북 31:190-191 "NEVER invent a Group", 슬롯≠FID gotcha). 이는 의도된 트레이드오프이며 UI 응답에도 명시된다.

## B. 요구사항 (GEARS)

### B.1 룩 데이터 계층 (스키마 + 내장 라이브러리)

- **REQ-LOOKLIB-001** [Ubiquitous] — 룩 스키마 **shall** 다음 축을 정의한다: 아이덴티티(안정적 look id, 표시 이름, 한국어 별칭/무드 키워드), 장르, **순서 있는 섹션 다이내믹스 레벨**(잔잔함→클라이맥스), 속성 페이로드(컬러/강도/빔 구체값), **역할 기반 포지션 목록**, 선택적 무브먼트(페이저) 지정. 스키마는 명시적 `schema_version`을 가지며, P1-1(다이내믹스 축 소비)·P1-2(장르 묶음 인스턴스화)가 소비 가능한 형상이어야 한다(research.md §10).
- **REQ-LOOKLIB-002** [Ubiquitous] — 내장 라이브러리 **shall** 워십/록/발라드/EDM 4개 장르를 제공하고, 각 장르는 6~10개 룩으로 섹션 다이내믹스 스펙트럼을 스팬하며, 장르와 다이내믹스 레벨은 기계적으로 조회 가능한 축이다.
- **REQ-LOOKLIB-003** [Ubiquitous] — 룩의 속성 페이로드 **shall** 룰북에 라이브 검증된 커맨드 어휘로만 표현 가능한 값을 사용한다(`Attribute 'Dimmer'/'ColorRGB_R/G/B'/'Pan'/'Tilt'` + 검증된 페이저/MAtricks 문법 — 31_choreography_patterns.md). 미검증 attribute 이름은 라이브러리에 등장하지 않는다.
...
- **REQ-LOOKLIB-006** [Ubiquitous] — 포지션 역할 **shall** 스키마에 정의된 폐쇄 어휘(예: 백라이트, FOH 워시, 사이드, 사이크/샤막, 스페셜 등 — 정확한 집합은 M1 설계 산출물)로만 표현되며, 각 역할은 20_korean_terms.md의 showfile 어휘 클래스 관례와 정합하는 한국어/영어 명칭 별칭을 갖는다.
...
- **REQ-LOOKLIB-010** [Event-driven] — **When** 사용자가 룩(또는 장르 묶음)의 인스턴스화를 지시하면, the 시스템 **shall** 라이브 검증된 프로그래밍 패턴으로 커맨드 번들을 구성해(프리셋 저장 + Label; 산출물 확장 범위는 plan.md 결정 기록에 따름) **기존 `run_commands` → `gate.screen()` 경로로만** 실행한다 — 제2 실행 경로·신규 REST 실행 엔드포인트·OSC 표면 직접 import는 구성상 금지된다.
...
- **REQ-LOOKLIB-012** [Unwanted] — 인스턴스화 **shall not** 점유된 슬롯에 대한 `Store /Overwrite`를 기본 경로로 사용한다 — `Store /overwrite`는 블랙리스트 항목(blacklist.yaml:18)으로 승인 보류를 유발하며, 충돌의 기본 처리 방향은 안전(스킵/보류)이다. 구체 충돌 정책은 plan.md 결정 기록에 따른다.
- **REQ-LOOKLIB-013** [Event-driven] — **When** 인스턴스화가 완료되면, the 시스템 **shall** 생성된 오브젝트(프리셋/그룹/시퀀스)의 종류·번호·이름과 미매핑 역할·스킵된 항목을 사용자에게 요약 보고한다 — 부분 성공을 전체 성공으로 위장하지 않는다.
- **REQ-LOOKLIB-014** [Where] — **Where** 인스턴스화가 다량 계산 생성(생성형 Lua 플러그인 경로, 31:136-171)을 사용하면, the 경로 **shall** 기존 배포 파이프라인(pcall compile → destructive scan → 사람 리뷰 게이트, pipeline.py:5-12)을 전부 무변경으로 경유한다.
...
- **REQ-LOOKLIB-021** [Ubiquitous] — 기존 안전 불변식 전부 **shall** 무변경 유지된다: health gate, 문법 검증, 위험 분류(개방형 타깃 승인 보류 포함), deny-all 기본 승인 포트, 위험 커맨드 사전 쇼파일 백업 fail-closed, 미확인 이력 재승인·자동 재전송 금지 — 본 SPEC은 `gate.screen()` 파이프라인을 소비만 하고 수정하지 않는다.
...
- **미검증 전제 (ASSUMPTION 규율, EXECBODY-001 ASSUMPTION-12 다음 번호 계승)**:
  - **ASSUMPTION-13 (그룹 명명 관례 기반 역할 매핑 실효성)**: 실제 사용자 쇼파일의 그룹 이름이 역할 휴리스틱(한/영 관례 어휘)으로 유의미하게 매핑된다. **미검증** — M2 설계 + M6 라이브에서 실측하며, 실패 방향은 명시적 미매핑 보고(REQ-LOOKLIB-009)로 안전하다.
  - **ASSUMPTION-14 (`Store Preset`의 속성 선택 캡처 의미론)**: 프로그래머에 활성화된 값으로 `Store Preset <pool>.<slot>`을 실행하면 해당 풀 타입에 맞는 속성이 캡처된다는 00_grammar.md:67-68 레시피가 onPC 2.4.2에서 풀 타입별로 기대대로 동작한다. 레시피는 룰북에 있으나 31처럼 "validated live" 표기가 없으므로 **미검증으로 취급** — M6 라이브 검증 항목이며, plan.md M1이 검증 순서를 규정한다.
- **측정된 기준선**: 이번 plan-phase 세션 기준 브랜치 `feat/lighting-direction-features`, HEAD `81e2232`(제안서 커밋). run-phase 킥오프 시점에 신선한 pytest/vitest 기준선을 재측정한다(baseline-integrity 원칙).
...
- "이 리그로 버스킹 준비해줘" 일괄 팔레트 + 익스큐터 페이지 레이아웃 생성. 장르 묶음 인스턴스화 형상만 스키마에 예약하고 마법사 UX·페이지 레이아웃은 별도 SPEC.
...
| rig 데이터 형상 + 실번호 키잉 + 드릴다운 | `server/orchestrator/tools.py:65-88, 185-230, 233-265` |
