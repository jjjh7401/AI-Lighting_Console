...
20행이 나온다. 이 중 **콘솔에서 온 값**(`read_inventory` → `ConsoleFixture`)을 쓰는 자리만 아래 표에 든다.
...
| 안 | 자리 | 번짐 | 대가 |
|---|---|---|---|
| **A-1 판독 경계** | `server/prechk/inventory.py` — 읽는 즉시 번역해 하류 전체가 이름을 본다 | C1~C13 **동시 해결** | 공용 표면을 바꾼다. `FixtureTypes` 트리 판독이 1회 추가된다. 판독 실패·절단 시 처리를 설계해야 한다 |
