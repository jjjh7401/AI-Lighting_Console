...
| 안 | 자리 | 번짐 | 대가 |
|---|---|---|---|
| **A-1 판독 경계** | `server/prechk/inventory.py` — 읽는 즉시 번역해 하류 전체가 이름을 본다 | C1~C13 **동시 해결** | 공용 표면을 바꾼다. `FixtureTypes` 트리 판독이 **`read_inventory` 호출 지점 8곳 전부에** 추가된다(아래 근거 2). 판독 실패·절단 시 처리를 설계해야 한다 |
