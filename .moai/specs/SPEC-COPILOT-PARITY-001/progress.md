...
| **P5** | `mode_read.read_type_mode_widths` → `type_found` / `detail` | 모드 폭 판독 | **틀렸다(Y-1) → 고쳤다.** 전부-아니면-전무였다. `i` 없는 자식 **하나**가 그 타입 판독을 죽여 `mode_unresolved` → **픽스처가 안 패치된다.** 파장이 라벨보다 크다 | 같은 `PROTOCOL.md` 가 이 분기를 규탄. **P4 는 고쳤고 P5 는 안 고쳤다 — 형제 불일치가 내 코드 안에 있다** | **측정됨**(원문) · **영향 미측정** |
...
| **P13** | `lxseq/mapper._judge_console_read` | 계획 진행 여부 | **틀렸다(리뷰 #3) → 고쳤다.** 번역 상태를 안 봤다(`git grep` 0건). 판독 실패해도 계획이 서고 86행이 `fid_occupied` 로 나가며 그 라벨은 「다른 FID 로 다시 패치하라」다. **실행 취소가 없다** | 같은 함수가 판독 완전성은 이미 게이트한다 — **번역만 빠졌다** | **측정됨**(grep + 원문) |
| **P14** | `lxseq/mapper._occupancy_skip` → 3 라벨 | 건너뛰기 사유 | **맞다**(D2 수정 후) | — | **측정됨** — M1 통합 테스트 |

**집계**: 14행 · 측정됨 11 · 미측정 3(P6 는 손대지 않음, P10 · P12) ·
**이번 회차에 고친 것 3**(P2 · P4 · P11) → **다음 회차에 P5 · P13 도 닫아 틀린 채 남은 것 0**.
...
## §E.2 (이어서) — P5 · P13 (리드 확정 후 수정)

### P5 — 모드 판독도 자식별로 격하한다 (Y-1)

전수표가 잡은 **내 코드 안의 형제 불일치**를 닫았다: P4 는 자식별 격하로 고쳤는데 P5 는
전부-아니면-전무였다. 같은 `PROTOCOL.md` 규약이 두 자리에 걸리는데 한쪽만 정렬돼 있었다.

**파장이 라벨보다 크다** — 슬롯 없는 자식 하나가 그 타입 모드 판독을 죽이면
`mode_unresolved` 가 되어 **픽스처가 아예 안 패치된다.**

리드가 파장을 재서 확정했다: `read_type_mode_widths` 호출 2곳(`tools.py:3894` prechk ·
`:4521` lxseq), `TypeModeRead` 소비는 그 밖 0. **카드 밖 이용자는 하나이고 수정 방향이
그쪽에도 엄격히 유리하다**(나빠지는 소비자 없음).

`_usable_pairs` 를 세워 두 열거(타입 · 모드)에 같은 격하를 적용했다. 타입을 못 찾았는데
**버린 자식이 있으면 「목록에 없다」로 단정하지 않는다** — 부분집합에서 안 보이는 것과
전수 목록에 없는 것은 다른 사건이다(전용 대조군).

### P13 — 번역 실패가 점유 판정을 막는다 (리뷰 #3)

**표에서 가장 위험한 행이었다.** 타입 이름을 못 얻으면 「이미 패치됨」 비교가 성립하지
않는데도 계획이 섰고, 이미 있는 86행이 `fid_occupied` 로 나갔다. 그 라벨이 지시하는
다음 행동은 **「다른 FID 로 다시 패치하라」** — 같은 리그를 한 벌 더 만든다.
**이 앱에 실행 취소가 없다.**

`_judge_console_read` 가 번역 상태를 보게 했다. **좁게 걸었다**: 콘솔이 이름을 돌려주는
경로에서는 미번역이 0건이라 이 갈래를 아예 타지 않는다 — **핸들이 왔는데 이름을 못 얻은
경우에만** 막힌다. 사유는 `console_read_incomplete` 의 이유 문자열로 페이로드까지 간다.

리드가 「자료구조 안의 안전이지 조작자의 안전이 아니다」라고 한 그 자리다 — 사유가
페이로드에만 묻혀 있던 것을 **계획을 막는 힘**으로 바꿨다.

### 뮤테이션 — P13 은 양방향

| 뮤테이션 | 죽인 것 |
|---|---|
| MUT-K P13 게이트 제거(**안 막는** 방향) | `test_a_failed_translation_blocks_the_occupancy_verdict` **1건만** |
| MUT-L 번역 성공에도 차단(**과잉** 방향) | **16건** — 그중 과잉-차단 대조군 |
| MUT-M P5 격하를 전부-아니면-전무로 | **2건**(격하 · 부분집합 대조군) |

**MUT-L 이 핵심이다.** 「안 막는」 방향만 걸었으면 게이트가 모든 재실행을 막아도 초록이다.
리드가 두 방향을 지정한 이유가 이거였다.

복구: `shasum -a 256` `d1f4ade9…` / `f562d10f…` 대조 · `grep -c "MUT-"` 두 파일 `0`.

### 리뷰 #7 — 묶지 않았다 (리드 지시)

**분류 정렬(P5)과 구조 리팩터(판독 공유)를 한 커밋에 넣으면 무엇이 무엇을 깼는지
갈리지 않는다.** 문면만 고쳤다 — CHANGELOG 의 「질의 순증 0」은 **툴 수준에서 참이 아니다**:
툴이 호출당 타입 트리를 1회 더 읽고, `read_type_mode_widths` 가 확정 타입 수만큼 같은
루트를 이미 읽고 `_named_children` 으로 쌍까지 만든 뒤 버린다(`mode_read.py:234` 확인).

**순증 0 이 성립하는 것은 질의 예산 가드가 사는 `prechk` 경로까지다.** 이것은 리드가 (B)를
승인할 때 내가 「깨진 5건이 사는 자리에서 순증 0」까지만 말할 수 있다고 한정한 그 자리이며,
**내 한정이 옳았고 승인 문면이 넓었다**(리드 인정).

**후속 재료**: 진짜 판독 공유. `read_type_mode_widths` 가 이미 만드는 쌍을 버리지 않고
내보내면 툴 수준에서도 순증 0 이 된다 — **P5 가 닫힌 지금이 그때보다 자연스럽다**(두
열거가 이미 같은 `_usable_pairs` 를 쓴다).

### 측정

`9716 passed, 8 skipped, 0 failed` — 직전 `9712` 대비 **+4 = 신규 테스트 수 일치**
(P5 2 + P13 2). 봉쇄 구역 `5c988bd..HEAD` 빈 출력.

### 표의 정지 조건

**14행 (a)(b) 두 칸 충족 · 틀린 채 남은 행 0.** 이 뒤에 새로 나오는 자리는 표를 넓히지
않고 **「표가 놓친 축」**으로 기록한다 — t12 재료다.

그리고 이 표의 한계는 그대로다 — **열거의 근거는 오늘까지 발견된 축이지 축의 전수가
아니다. 표가 완성됐다는 것과 계열이 말랐다는 것은 다른 문장이다.**
