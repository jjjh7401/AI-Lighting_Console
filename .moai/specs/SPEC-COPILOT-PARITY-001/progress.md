...
| # | 자리 | 산출 | (a) 행동이 맞는가 | (b) 형제 대조 — **확인함 / 안 찾아봤다 / `—`(미기재)** | 측정 |
|---|---|---|---|---|---|
| **P1** | `mode_read.read_fixture_type_names` → `attempted` | 판독 도달 여부 | **맞다** — False 는 재조회를 지시하고 실제로 응답이 없었다 | **확인함** — `footprint.walk_mode_widths`(`:229` 이하)도 같은 축을 쓴다. **판단 같음** | **측정됨** — 거절/예외 갈래 테스트 2건 |
| **P2** | 같은 함수 → `whole_unconfirmed` | 전수 확인 불가 | **맞다** — 부정 결론만 보류, 도착한 쌍은 번역 | **확인함** — `footprint.py:317-331` 이 빈 목록에 `whole=False` 를 강제하며 이유를 주석에 적었고, 이번에 같은 판정으로 정렬 | **측정됨** — MUT-G 가 되돌리면 RED |
| **P3** | 같은 함수 → `shape_invalid` | 쓸 자식 0건 | **맞다** — 「판독 쪽을 고쳐라」 | **확인함** — `footprint._child_slots`(`:204` "or ``None`` if any child lacks one") + `:313` `whole = False`. 형제도 형태 불량을 「전수 아님」으로 매기고 멈추지 않는다. **판단 같음** ← **정정(감사 Z-1): 첫 판은 「없음」이라 적었고 그것은 확인하지 않은 부재였다** | **측정됨** — N-1 테스트 + MUT |
| **P4** | `mode_read._named_children` → 하나라도 나쁘면 `None` | 표 전체 폐기 | **P1~P3 경로에서는 더 이상 안 쓴다**(자식별 격하로 대체) | **확인함** — `console/lua/PROTOCOL.md:52-58` 이 `i` optional + 이름-only 격하를 established 로 규정 | **측정됨** — #2 테스트 |
| **P5** | `mode_read.read_type_mode_widths` → `type_found` / `detail` | 모드 폭 판독 | **틀렸다(Y-1) → 고쳤다.** 전부-아니면-전무였다. `i` 없는 자식 **하나**가 그 타입 판독을 죽여 `mode_unresolved` → **픽스처가 안 패치된다.** 파장이 라벨보다 크다 | **확인함** — 같은 `console/lua/PROTOCOL.md` 가 이 분기를 규탄. **판단 달랐다 → 이번에 맞췄다.** **P4 는 고쳤고 P5 는 안 고쳤다 — 형제 불일치가 내 코드 안에 있다** | **측정됨**(원문) · **영향 미측정** |
| **P6** | `footprint._listing_is_whole` | 전수 술어 | 이용자 다수의 **공유 술어** | **확인함** — 이 술어 자체가 형제들의 공통 기준선이다 | **손대지 않음** |
| **P7** | `footprint.walk_mode_widths` → `whole=False` 강제 | 빈 목록 | **기준선** — 이번에 P2 가 여기로 정렬 | **확인함** — 이 행이 기준선이므로 대조 대상이 자기 자신이다 | **측정됨**(원문 13줄) |
| **P8** | `inventory.translate_fixture_type` → 사유 5 | 미번역 사유 | **맞다** — 다섯이 각각 다른 행동을 지시 | **확인함** — `vwx/apply.py:520-535` 는 「이름이면서 핸들 형태」 모호성을 **거부**하는데 내 번역기는 **핸들 해석을 우선**한다 → **판단이 다르다(리뷰 #5)** | **측정됨**(원문) · **실기 영향 미측정** |
| **P9** | `inventory._name_handle_types` → 사유 선택 | 우선순위 | **맞다** — shape_invalid > unconfirmed > absent | **안 찾아봤다** — 사유 우선순위를 매기는 형제가 있는지 찾지 않았다 | **측정됨** — MUT-D·E·F |
| **P10** | `inventory.read_inventory` → `completeness` · `index_domain_unknown` | 판독 완전성 | **이 카드가 안 건드림** | **안 찾아봤다** — 범위 밖이라 대조하지 않았다 | **미측정** |
| **P11** | `tools` 의 `TypeNameRead` 생성(배선) | 경로 부재 시 산출 | **틀렸다(Y-2) → 고쳤다.** 아무것도 안 물었는데 `type_tree_unreadable`(재조회하라)을 냈다 | **확인함** — 인용한 형제가 내 코드를 금지하고 있었다. `tools.py:2696` *"nothing was queried, so nothing may be called unreadable."* 이제 `None`(→`no_type_table`) | **측정됨**(원문 대조) |
| **P12** | `vwx/apply.console_read_caveat` → caveat kind | 미판독 vs 절단 | **이 카드가 안 건드림** | **확인함** — `console_read_caveat` 자체가 형제다. `missing_count == 0` + 절단은 **막지 않는다**(수량 비교는 정확) — P2 는 같은 절단에서 **부정 결론을 보류**한다. **질문이 다르다**(픽스처 부재 vs 슬롯 부재)지만 **같은 절단 신호에 다른 판정**이다 | **미측정** — 두 판정이 실제로 충돌하는 입력이 있는지 안 쟀다 |
| **P13** | `lxseq/mapper._judge_console_read` | 계획 진행 여부 | **틀렸다(리뷰 #3) → 고쳤다.** 번역 상태를 안 봤다(`git grep` 0건). 판독 실패해도 계획이 서고 86행이 `fid_occupied` 로 나가며 그 라벨은 「다른 FID 로 다시 패치하라」다. **실행 취소가 없다** | **확인함(부분)** — 독스트링이 형제 호출부 둘(`tools.py` 의 `patch_fixtures` · `vwx/apply.py`)이 같은 잣대를 쓴다고 적고 그 판독-완전성 축은 같다. **번역 축은 안 찾아봤다** — 그 둘이 번역 상태를 보는지는 대조하지 않았다 | **측정됨**(grep + 원문) |
| **P14** | `lxseq/mapper._occupancy_skip` → 3 라벨 | 건너뛰기 사유 | **맞다**(D2 수정 후) | **안 찾아봤다** — 점유 라벨을 매기는 형제가 있는지 찾지 않았다 | **측정됨** — M1 통합 테스트 |
...
| MUT-L 번역 성공에도 차단(**과잉** 방향) | **16건** — 선택자 `uv run pytest server/tests/test_lxseq_tool.py -q` 기준(그 파일 34건 중). 그중 과잉-차단 대조군. **정정(감사 Z-2): 첫 판은 범위를 안 적었다** — 감사가 두 파일 기준 50건을 관측했고, **어느 쪽도 틀리지 않았다. 범위가 없었을 뿐이다** |
...
## §E.2 (이어서) — 5차 감사 지적 처분 (Z-1 · Z-2) · 3-phase close

5차 감사 **PASS 0.93**(이 카드 최고점). frontmatter 에 한계가 박혔다 —
`pass_scope: "지금 표에서 열린 것이 없다 — 더 없다가 아니다"`.

### Z-1 (주요) — 표가 자기 (b) 칸에서 이 카드의 결함을 저질렀다

`P3` 의 (b) 를 **「없음」**이라 적었는데 형제가 실재한다. **원문 확인:**
```
footprint.py:204  def _child_slots(payload) -> list[int] | None
                  """Pool slots …, or ``None`` if any child lacks one."""
footprint.py:313      if mode_slots is None: whole = False
```
형제도 형태 불량을 「전수 아님」으로 매기고 멈추지 않는다 — **판단이 같다.**

**코드 결과는 안 어긋났다. 어긋난 것은 표의 주장이다.** 그리고 그 주장이
**「확인하지 않은 부재를 사실로 적은 것」** — 이 카드가 다섯 회차 고쳐 온 바로 그것이다.
감사의 진단이 정확하다: **「없음」은 `slot_absent` 다.**

**처분 — 표를 넓히지 않고 이미 있는 칸을 갈랐다.** (b) 의 어휘를 셋으로:

| 어휘 | 뜻 |
|---|---|
| **확인함 — 형제 X(좌표), 판단 같음/다름** | 찾아봤고 결과가 있다 |
| **안 찾아봤다** | 대조하지 않았다 |
| `—` | 미기재 |

**공란과 「없음」은 정도가 다르다** — 공란은 미기재이고 **「없음」은 주장**이다.
14행 전부를 이 어휘로 다시 채웠고, 기계 검사로 어휘 밖 셀 **0** 을 확인했다.
그 결과 **「안 찾아봤다」가 4행**(P9 · P10 · P14 · P13의 번역 축) 드러났다 —
첫 판에서 `—` 로 가려져 있던 것이다.

### Z-2 (경미) — MUT-L 에 선택자가 없었다

「16건」에 **범위를 안 적었다.** 감사는 두 파일 기준 **50건**을 관측했다.
**어느 쪽도 틀리지 않았다 — 범위가 없었을 뿐이다.** 내가 실제로 돌린 선택자
(`uv run pytest server/tests/test_lxseq_tool.py -q`, 그 파일 34건 중 16건)를 적었다.

감사가 **세 번째 값을 일부러 안 만든 것**이 옳다 — 값이 하나 더 있어도 이 결함은 안
닫힌다. 닫는 것은 **선택자 한 줄**이다. (이 저장소가 「숫자엔 범위를 함께 적어라」로
이미 갖고 있던 규율이고, 내가 또 빠뜨렸다.)

### 기록만 (감사가 결함으로 안 올렸다)

**Z-3** — P13 게이트가 픽스처 단위 **전부-아니면-전무**다. 미번역 1대면 계획 전체가
막힌다. 사유 다섯 중 넷은 판독 실패라 전체 차단이 옳지만, **`slot_absent` 는 확인된
리그 사실**이고 그 한 대가 나머지 유효한 비교까지 막는다 — **이 카드가 두 회차에 걸쳐
세운 자식별 격하와 반대 모양이다.** 도달 조합의 실재를 **아무도 안 쟀고** 안전 방향이라
결함이 아니다. **다음 축 후보로 남긴다.**

**Z-4 — 리드 배차문 오류(산출물은 옳다).** 배차문이 「미측정 3(P8·P12·P10)」이라
적었으나 표는 「P6·P10·P12」이고 P8 은 「측정됨(원문)·실기 영향 미측정」이다.
**산출물이 옳고 전달이 어긋났다** — 같은 계열이 이번엔 배차문에서 났다. 산출물은
고치지 않았다.

### 감사가 마지막에 적은 것 (인용 — 지우지 않는다)

> 다섯 회차 동안 신설한 축 셋(⑧형제 대조 · ⑨인용 좌표 · ⑩표의 칸이 주장인지
> 미기재인지)이 **전부 남이 준 질문**이다. **여섯 번째 회차가 없으리라는 근거는
> 여전히 없다.**

**Z-6** — 리뷰가 잡은 넷이 ⑧·⑨ **밖**이었다. **「⑧·⑨가 얕은 게 아니라 축이 둘 더
있었던 것이다. 축을 둘 넓히고도 절반은 못 봤다.」**

그리고 감사가 자기 처방의 공을 좁게 잡은 것도 적는다 — **표가 값을 한 것은 P5 를
찾아서가 아니라(그건 4차에 Y-1 로 이미 보고됐다) 왜 그것이 네 회차까지 안 보였는지를
구조로 설명하고 같은 축의 나머지 13행을 함께 세운 것**이다.

### 3-phase close

`status: in-progress → completed`. **트리거는 리드가 읽은 5차 감사 판정**이며,
run 레인이 자기 산출물을 자기 판정으로 닫지 않았다.

[HARD] **`pass_scope` 를 함께 남긴다** — **「지금 표에서 열린 것이 없다」와 「더 없다」는
다른 문장**이고, 이 카드가 그 구분 때문에 다섯 번 돌았다.

**닫히지 않은 채 출하되는 것**(sync·후속이 지우면 안 되는 것):
AC 전건 통과 ≠ 실기 D2 해소 · 번짐 3건(C3 diff 거짓 경보 · C4 · C5 서류 인쇄, **원인
하나 — 대응표 미공급 경로**) · C6 · C7 미측정 · N-1 · #1 의 실기 도달성 미측정 ·
`read_inventory` 나머지 5곳 순증 미측정 · P8(리뷰#5) 실재 미측정 · P12 충돌 입력 미측정 ·
Z-3 도달 조합 미측정 · **진짜 판독 공유(리뷰#7) 후속** · **「안 찾아봤다」 4행**.
