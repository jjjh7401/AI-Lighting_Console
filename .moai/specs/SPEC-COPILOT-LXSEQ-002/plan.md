...
|---|---|---|---|
| 1 | **슬롯 번호의 권위가 시트인가 콘솔인가.** 시트는 GroupNo 1부터 18까지를 선언하는데 `build_group_write_plan` 은 요청 번호를 무시하고 `measure_empty_slots` 오름차순을 쓴다 | 빈 풀에서는 두 값이 우연히 같아 문제가 안 보인다. 점유가 하나라도 있으면 조용히 어긋나고, 곡 파일이 그룹 번호를 참조하므로 번호가 밀리면 쇼가 깨진다 | **A.4 열린 결정 ①** — Kickoff |
| 2 | **입력이 파일 하나인가 둘인가.** 멤버십을 콘솔에서 읽는 길은 픽스처 열거가 86에서 19로 잘려 막혀 있다(카드 t46) | 시트 종류 레지스트리는 파일 하나를 핸들러 하나로 보내는 모형이라, 인자 둘은 그 모형과 마찰한다 | **A.4 열린 결정 ②** — Kickoff |
...
| 항목 | 막는 대상 | 성격 | 부정 시 처리 |
|---|---|---|---|
| 열린 결정 ① 슬롯 권위 | M2 슬롯 대조 로직, M4 실기 절차 | **미해소**. Kickoff 답이 필요하다 | 답이 없으면 권고안(어긋나면 0배치와 대조표)으로 구현하고 progress.md에 기록 |
| 열린 결정 ② 입력 파일 수 | M3 툴 인자 집합, 레지스트리 행 모양 | **미해소**. Kickoff 답이 필요하다 | 답이 없으면 권고안(인자 둘)으로 구현 |
...
### A.4 열린 결정 2건 — 착수 승인(Kickoff) 때 감독께 올린다

리드 판정(2026-08-24): 두 결정 모두 저술을 막지 않으므로 지금 올리지 않고, plan에서 run으로 넘어가는 착수 승인 때 한 번에 올린다. 아래는 권고와 기각안이다.

**① 슬롯 번호의 권위 — 시트의 GroupNo 인가, 엔진이 잰 빈 슬롯인가**

사실관계. 시트는 GroupNo 1부터 18까지를 선언한다. `build_group_write_plan`(`server/groupgen/write.py:326`)은 `measure_empty_slots` 로 빈 슬롯을 오름차순으로 잡고 그것을 `select_group_slot` 에 요청한다 — 즉 **호출자가 번호를 지정할 통로가 없다.** 오늘 풀이 비어 있어(ASSUMPTION-80) 두 값이 우연히 같으므로 이 차이는 지금 보이지 않는다.

- **권고(가)**: 측정된 빈 슬롯 순열이 시트의 GroupNo 순열과 다르면 **계획을 내지 않고 대조표를 보고한다**(REQ-009). 기존 핸들러 0-diff이고, 조용한 재번호를 만들지 않는다. 곡 파일이 그룹 번호를 참조하므로 재번호는 쇼의 의미를 조용히 깨는 변경이다.
- 기각(나): 엔진이 잰 슬롯을 그대로 쓰고 매핑표만 보고한다. 성공률은 높으나 **번호가 밀린 것을 사람이 알아채야** 하고, 멤버십은 되읽히지 않으므로 사후 확인 수단이 약하다.
- 기각(다): `create_arrangement_groups` 에 슬롯 지정 인자를 뚫는다. 001의 「신규 툴 밖 0-diff」를 깬다.

**② 입력이 파일 하나인가 둘인가**

사실관계. 그룹의 멤버 FID를 알려면 「라벨에서 FID로」 표가 필요하다. 그 표의 출처는 둘 중 하나다. 콘솔에서 픽스처 이름을 읽어 접두를 뜯거나(001 결정 I가 이름을 「라벨에 FID」 꼴로 만든 이유가 이것이다), 패치 CSV를 다시 읽거나. **콘솔 경로는 오늘 막혀 있다** — 픽스처 열거가 86에서 19로 잘리고(카드 t46), 잘린 목록에서 멤버십을 만들면 REQ-GROUPGEN-024가 경고한 「18대짜리 19대 그룹」이 조용히 영속한다.

- **권고(가)**: 툴이 인자 둘을 받는다. 그룹 시트 바이트와 패치 시트 바이트. LXSEQ-001 plan.md의 E절 예약 문면 자체가 "본 SPEC의 FID 매핑표와 RIG GROUP 시트"라고 적어 두 입력을 전제한다. 절단에 영향받지 않는다.
- 기각(나): 콘솔 픽스처 이름에서 라벨을 읽는다. 카드 t46이 열리기 전에는 잘린 목록만 나온다.
- 기각(다): 그룹 시트만 받고 FID를 산술로 유추한다(FID_BASE에 순번). 추측이며 이 저장소의 실측 우선 원칙에 정면으로 어긋난다.

마찰 하나를 함께 올린다. 시트 종류 레지스트리는 **파일 하나를 핸들러 하나로** 보내는 모형이라, 인자 둘은 그 모형과 어긋난다. 002는 첨부 경로에서 그룹 시트를 받고 패치 시트는 툴 인자로 받는 비대칭을 택할 수 있으나, 그 모양을 못박는 것은 카드 t53의 입력이 되므로 여기서 결정하지 않고 감독 답을 받아 M3에서 확정한다.
...
- **Decision: serial** (권고).
- **사용자 접점(Kickoff)**: A.4의 열린 결정 2건(슬롯 번호 권위, 입력 파일 수)과 M4 라이브 세션 일정. 두 결정은 리드가 착수 승인 때 한 번에 올린다.
