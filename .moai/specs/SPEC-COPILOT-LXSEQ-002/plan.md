...
| 4 | **배치를 GroupNo 오름차순 1..16 과 17·18 로 나눈다.** 한 호출로 18개는 못 쓴다 | 실측 강제다. DEFAULT_GROUP_PLAN_CAP 이 16이고 툴이 max_plan_size 를 노출하지 않는다(spec.md A.2 둘째) | 결정 M — 닫힘 |
...
| **M** | 배치 분할 | **GroupNo 오름차순, 1..16 과 17·18.** 실측 강제다 — `server/groupgen/write.py:66` 의 DEFAULT_GROUP_PLAN_CAP 이 16이고 `server/orchestrator/tools.py:7101` 의 호출이 max_plan_size 를 넘기지 않아 18개는 GROUP_PLAN_TOO_LARGE 로 거부된다. 툴에 인자를 새로 뚫지 않는다(spec.md C.3) | M2, M3 |
