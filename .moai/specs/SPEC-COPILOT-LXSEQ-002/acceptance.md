...
**시나리오 5 — 번들이 예산 밖**: **Given** 선언된 바이트 예산을 ALL 의 실측 번들 길이보다 작게 설정한 트리, **When** preview 를 부르면, **Then** ALL 만 `line_over_budget` 으로 건너뛰고 목록에 실측 바이트 수가 적히며, 나머지 17개는 정상 계획된다.
...
**AC-LXSEQ2-011**. **Given** 빈 그룹 풀 단면과 실물 두 CSV, **When** 매퍼를 부르면, **Then** ① 배치가 정확히 2개다. ② 순서가 **GroupNo 오름차순**이라 1차가 1..16, 2차가 17·18이다. ③ 어느 배치도 `DEFAULT_GROUP_PLAN_CAP` 을 넘지 않는다. ④ 두 배치를 하나로 합치면 `guard_plan_size` 가 `GROUP_PLAN_TOO_LARGE` 를 던지고 16은 통과한다는 것을 같은 테스트가 함께 단언한다(분할이 필요하다는 근거의 비공허성). ⑤ **「기본 먼저」 순열을 엔진의 빈 슬롯과 짝지으면 시트 번호와 어긋나는 자리가 나온다**는 것을 함께 단언한다 — 이 대조가 통과해야 ②가 뜻을 갖는다. 무해했다면 순서 규칙 자체가 불필요하다.

**AC-LXSEQ2-012**. **Given** 선언된 줄 예산과 실물 입력, **When** 매퍼를 부르면, **Then** ① 각 그룹의 **가장 긴 명령 한 줄**의 인코딩 바이트가 보고에 실린다. ② 그 측정이 **추정이 아니라 실제 인코더**(`build_exec_request`)를 통과시킨 값과 같다 — 드리프트를 테스트가 잡는다. ③ ALL 의 줄이 **1201 바이트**로 관측되고, 실측 프레이밍 42 를 더해도 전송 상한 2048 미만이다. ④ **매퍼는 전송층을 임포트하지 않는다** — `server/lxseq/` 는 `server.bridge` 를 임포트할 수 없다(001이 세운 경계). 그래서 기본 예산은 프레이밍 여유를 뺀 값으로 **선언**되고, 「선언된 예산 + 실측 프레이밍 ≤ 전송 상한」은 양쪽을 임포트할 수 있는 **테스트가** 잰다. ⑤ **예산을 ALL 의 실측 길이보다 작게 낮추면 ALL 이 `line_over_budget` 으로 빠진다** — 안 빠지면 게이트가 공허하므로 이 항목이 게이트의 유일한 비공허성 증거다.

> **단위가 줄인 이유와, 빠짐이 계획 전체에 미치는 영향**: `run_commands` 가 한 줄씩 발화하므로 상한에 걸리는 것은 번들 합계가 아니라 최장 줄이다. 프레이밍은 전송층에 있고 매퍼는 그 층을 임포트할 수 없으므로, 매퍼는 체인 바이트만 재고 예산이 프레이밍 여유를 미리 뺀 값이다. 그리고 ALL 은 슬롯 1이므로 그것이 빠지면 남은 순열이 2..18 이 되어 REQ-009 의 어긋남이 발동하고 **배치가 0개**가 된다. 17개를 틀린 번호로 쓰느니 0개를 쓴다 — 결정 T 의 의도다. 대조로, **꼬리(슬롯 18)가 빠지면 순열이 안 깨져 17개가 계획된다.** 「어떤 빠짐이든 0배치」가 아니라 **앞쪽 연속이 깨질 때만** 0배치라는 것을 두 검사가 함께 잰다.
