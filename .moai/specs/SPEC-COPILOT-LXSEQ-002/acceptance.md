...
**AC-LXSEQ2-012**. **Given** 선언된 줄 예산과 실물 입력, **When** 매퍼를 부르면, **Then** ① 각 그룹의 **가장 긴 명령 한 줄**의 인코딩 바이트가 보고에 실린다. ② 그 측정이 **추정이 아니라 실제 인코더**(`build_exec_request`)를 통과시킨 값과 같다 — 드리프트를 테스트가 잡는다. ③ ALL 의 줄이 **1243 바이트**로 관측되고(선택 줄 1201 + 프레이밍 42) 전송 상한 2048 미만이다. ④ 기본 예산이 `MAX_PLUGIN_CALL_BYTES` 이하다. ⑤ **예산을 ALL 의 실측 길이보다 작게 낮추면 ALL 이 `line_over_budget` 으로 빠진다** — 안 빠지면 게이트가 공허하므로 이 항목이 게이트의 유일한 비공허성 증거다.

> **단위가 줄인 이유와, 빠짐이 계획 전체에 미치는 영향**: `run_commands` 가 한 줄씩 발화하므로 상한에 걸리는 것은 번들 합계가 아니라 최장 줄이다. 그리고 ALL 은 슬롯 1이므로 그것이 빠지면 남은 순열이 2..18 이 되어 REQ-009 의 어긋남이 발동하고 **배치가 0개**가 된다. 17개를 틀린 번호로 쓰느니 0개를 쓴다 — 결정 T 의 의도다. 대조로, **꼬리(슬롯 18)가 빠지면 순열이 안 깨져 17개가 계획된다.** 「어떤 빠짐이든 0배치」가 아니라 **앞쪽 연속이 깨질 때만** 0배치라는 것을 두 검사가 함께 잰다.
