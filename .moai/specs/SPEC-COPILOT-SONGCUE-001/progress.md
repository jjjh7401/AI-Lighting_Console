...
3. **F-3의 상한은 미실측이다.** `max_children = 24`는 소스에서 읽은 값이고 24를 넘는 풀에서 `truncated`가 실제로 어떻게 오는지는 재지 않았다(현재 시퀀스 17) **※ 2026-08-07 정정 — 픽스처 축에서는 그 뒤 실측됐다**: `.moai/specs/SPEC-COPILOT-GROUPGEN-001/progress.md:428`이 **39대 리그에서 반환 18 · `truncated: true`**를 관측했다. 즉 절단은 `max_children = 24`가 아니라 **페이로드 예산 `max_payload = 1900`이 먼저 문다**(`console/lua/copilot_responder.lua:33`, `:36-39`). **미실측으로 남은 것은 시퀀스 풀 축뿐이다.**.
...
### §F 개정 — PRESERVE 범위 확장 승인 (2026-07-29)

**사용자 승인으로 `spec.md`를 v0.1.0 → v0.2.0으로 개정하고 `console/lua/**`를 PRESERVE에서 제외했다.**

#### 오케스트레이터의 실수 — 먼저 적는다

트랙 B(응답기 프로퍼티 리더)를 설계할 때 **`plan.md §B M6`의 좁은 6파일 목록만 보고 `spec.md §C의 PRESERVE 정본 목록을 읽지 않았다.`** 정본에는 `console/lua/**`가 있었고, `spec.md`의 「Out of Scope — 콘솔측 Lua 변경」 절은 이 상황을 **정확히 예견해 닫아 두었다** — "응답기 확장은 그 자체로 별도 범위 결정이다". 즉 위반은 워커의 일탈이 아니라 **내가 위반을 지시한 것**이다. 발견한 것도 내가 아니라 M6 워커였다(broad-PRESERVE diff 보고).

**두 번째 실수**: 트랙 B를 "M4를 막는 임계경로"로 규정한 것은 **과장이었다.** SPEC의 원설계는 관측을 포기하고 한계를 상수로 명시하는 것(`PROPERTY_UNOBSERVED_NOTE`)이었고 M4는 실제로 그것을 구현했다. M4는 원래 막혀 있지 않았다. 병렬 2트랙의 정당화 근거 하나가 사후적으로 약해졌다는 뜻이며, 그래도 트랙 분리 자체는 유효했다(파일·언어가 실제로 겹치지 않았고 M1~M3과 동시 진행되었다).

#### 개정의 실질 근거 — 문서 간 모순

실수와 별개로, 이 개정에는 실질 근거가 있다. **`plan.md`와 `spec.md`가 서로 모순이었고 M0가 그것을 드러냈다**:

| 문서 | 지시 |
|---|---|
| `plan.md §B M0` | ASSUMPTION-22가 GO면 `TrigTime` 의미론을 **반드시 함께 측정할 것** |
| `spec.md §D` | 그 측정의 유일한 수단(응답기 확장)은 **범위 밖**, 한계 명시로 갈음 |

M0에서 ASSUMPTION-22가 **GO**로 나온 순간 두 지시는 동시에 만족될 수 없게 되었다 — 상태 조회는 name·class·slot만 주고 `List`·`Get`은 `OK`만 돌려주므로 응답기를 고치지 않으면 값을 볼 방법이 없다. **둘 중 하나는 반드시 어긋나며, 오케스트레이터가 측정을 택했고 사용자가 승인했다.**

#### 위반 범위 실측 — 정확히 한 항목

| PRESERVE 항목 | `git diff 38a6e7e..HEAD` |
|---|---|
| `server/looks/{schema,loader,roles,resolver,instantiate,matching}.py` | 빈 출력 |
| `server/looks/library/` · `server/safety/**` · `server/web/preview.py` | 빈 출력 |
| `server/rulebook/assets/v2.4.2/**` | 빈 출력 |
| **`console/lua/**`** | **4 files, +147 −6** ← 유일한 위반 |

`server/orchestrator/tools.py`의 금지 두 구간도 무침범이다 — 변경 hunk의 old 시작줄은 32 · 49 · 125 · 951 · 1222 · 1231이며 `:234-238`·`:524-569` 어디에도 걸치지 않는다(코디네이터가 `git diff -U0`으로 직접 확인).

#### 결합도 — 되돌릴 수 있었으나 되돌리지 않기로 했다

`server/looks/songcue_report.py`가 `child.get("cueNo", child.get("i"))`로 **폴백을 갖고** 있어 응답기를 되돌려도 SONGCUE는 동작한다. 즉 분리는 기술적으로 가능했고, 그럼에도 확장 유지를 택한 이유는 **AC-SONGCUE-018 측정 3**("큐 N개가 서로 다른 번호로 존재")이 `cueNo` 없이는 재조회로 성립하지 않고 F-4의 **쓰기성** 우회로로 후퇴하기 때문이다. 읽기 검증이 쓰기 프로브를 요구하는 것은 종단 검증 마일스톤에 바람직하지 않다.

#### 개정이 남기는 의무

`REQ-SONGCUE-017`의 **한계 명시 의무는 폐지되지 않았다.** 재조회 경로는 여전히 `CueFade`·`TrigType`을 노출하지 않으며, `prop`은 **별개 경로**다. 두 경로를 뭉뚱그려 "확인했다"고 적는 것은 계속 금지이고, M7이 이 구분을 지키는지 확인한다.
