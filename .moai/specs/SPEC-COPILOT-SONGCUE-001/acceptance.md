...
- 비고: 실행되지 않은 분기는 **축별로** `skip` 사유를 명시한 채 **남긴다 — 삭제하지 않는다**.
  ...
  2개는 `skip`으로 남는다. 후속 SPEC이 게이트를 다시 열 때 출발점이 된다(AC-BUSKWIZ-012 비고의 계승). **※ 2026-08-07 정정** — 두 축 모두 M0에서 **GO로 닫혔다**(`progress.md` M0 측정 3: ASSUMPTION-20 타임코드 실재 · 측정 4: ASSUMPTION-22 `TrigType` `'Follow'`/`'Time'` 양쪽 OK). `skip`으로 남은 것은 **부정 분기**이며 "다시 열" 대상이 아니다 — **회귀 방지용 보존**이다.
