...
> **※ 소급 정정(2026-08-07)** — 위 그림의 *"돌아오지 않는 것: 커맨드 · CueFade · TrigType"* 중 **`TrigType`은 이제 돌아온다.** 응답기 v1.5.0의 `prop` 동사로 `TrigType`·`TrigTime`이 판독 가능해졌고(`.moai/specs/SPEC-COPILOT-SCENE-001/spec.md:230`), 그 개정은 본 SPEC의 M0가 집행했다. 그림은 **v1.5.0 이전** 상태의 기록으로 보존한다 — 돌아오지 않는 것으로 남은 것은 **커맨드 본문 · `CueFade` · 큐의 내용** 셋이다.
