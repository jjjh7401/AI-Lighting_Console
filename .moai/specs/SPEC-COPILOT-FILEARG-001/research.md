...
문서 상태: draft (v0.2.0, 2026-08-23 — §7 갭 4건 닫힘 · §9 리드 실측 추가) · Tier M · 본 문서는 **저장소 정적 조사**의 기록이다. 라이브 콘솔 관측은 0건(M5가 사용자 수행 앱 실기 1회를 소유). 근거 등급: `[코드]`(이 워크트리에서 직접 읽음) · `[리드]`(칸반 리드가 이 워크트리에서 읽어 확정해 넘긴 좌표) · `[문서]`(타 SPEC 아티팩트) · `[미확정]`.
...
| Vectorworks **자신의** 서명이 vwx fixture 7종의 변형을 견디는가 | 열 계약(`server/vwx/columns.py`)이 실물 변형을 얼마나 관대하게 받는지는 재지 않았다. 결정 I의 회귀 위험 크기가 여기에 달렸다 | **M1이 실측**. 견디지 못하면 안전판 `REQ-FILEARG-018` |
