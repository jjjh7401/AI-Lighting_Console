...
문서 상태: draft (v0.8.0, 2026-08-23 — PASS-WITH-DEBT 0.87 채무 정리) · **Tier L · 통과 임계 0.85** · 칸반 카드 t10 · 분할 **A(판별기)**
...
9. **한 방향만 훑지 마라 — 부정 훑기와 긍정 훑기는 둘 다 필요하다.**
   ...
   - **계열 인식**: 이 카드에서 같은 계열이 **네 번째**다 — `columns.py` 오귀속 · `2859-2860` 좌표 · `§G.x` 포인터. 셋 다 **"문서가 옮겨간 것을 가리킨다"**는 한 가지 형태다. 새 결함으로 보이면 먼저 이 계열인지 의심하라.
