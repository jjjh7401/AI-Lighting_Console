...
**행이 드는 것은 열 목록이 아니라 술어다**(결정 K). 그리고 그 술어는 **신원**을 묻는 것이어야 한다(결정 L) — 이유는 §G.1이 소유한다.
...
### G.1 우리는 다른 물음에 답하는 함수를 판별자로 썼다 — 관대함이 아니라 축이 다르다
...
### G.2 왜 좁은 쪽(신원)을 골랐는가 — 두 후보를 실측했다
