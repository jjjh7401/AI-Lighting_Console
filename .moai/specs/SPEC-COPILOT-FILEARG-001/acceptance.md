...
문서 상태: draft (v0.9.0, 2026-08-23 — B 저술이 드러낸 교정 4건: 핸들러 태그 쌍(결정 M) · AC-023 태그별 분기 · README 경로 교정 · base..HEAD 22개소) · **Tier L · 통과 임계 0.85** · AC **20건**(전부 오프라인 · 그중 AC-021은 조건부). 본 문서는 spec.md의 요구를 관측 가능한 Given-When-Then 기준으로 전개한다. 요구(GEARS)는 spec.md가 소유하며 여기서 되풀이하지 않는다.
