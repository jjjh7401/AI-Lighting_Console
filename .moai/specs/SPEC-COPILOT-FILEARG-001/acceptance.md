...
문서 상태: **in-progress** (updated 2026-08-24 — M1 실행됨) · 직전 개정 (v1.0.0, 2026-08-24 — M1 구현 반영: AC-021 ③ `^상태: 발동함` 앵커 · AC-009 ②③을 B로 이관하고 ①④ 번호 유지) · **Tier L · 통과 임계 0.85** · AC **20건**(전부 오프라인 · 그중 AC-021은 조건부). 본 문서는 spec.md의 요구를 관측 가능한 Given-When-Then 기준으로 전개한다. 요구(GEARS)는 spec.md가 소유하며 여기서 되풀이하지 않는다.

> **참조 규약**: 정본(spec.md · 본 문서)은 줄번호로 인용하지 않고 안정 토큰만 쓴다. `파일:줄`은 코드·타 SPEC 아티팩트에만 쓴다.
>
> **검증 명령 규약**: 아래 명령은 워크트리 루트에서 실행한다. 테스트 파일이 아직 없는 시점에는 그 AC가 미충족이라는 뜻이며, "명령이 없어서 건너뜀"은 PASS가 아니다. **카드를 닫을 때 각 AC의 증거는 로컬 실행 출력이다**(spec.md §C) — 이 저장소의 `test.yml`은 **PR이 열려야** 돌므로 카드 안에서는 AC를 판정해 주지 않는다. PR을 연 뒤에는 CI가 독립 증거로 **더해지지만**, 로컬 출력을 **대체하지 않는다**.
...
| ② 신원과 사용성의 분리 | "무엇인가"와 "패치를 뽑을 수 있는가"를 섞지 않는다 | 축을 섞으면 술어가 **양방향으로** 틀린다(`design.md` §1) |
...
> **분할 기록(v0.5.0).** 이 기준은 두 요구를 동시에 섬기고 있었다 — `REQ-005`의 **순서** 속성(판별하려고 시험 파싱하지 않는다)과 `REQ-010`의 **표시** 속성(업로드 직후 네 필드를 보인다). 통째로 한쪽에 주면 다른 쪽이 증거를 잃으므로 **둘로 쪼갰다**. **A는 순서 단언을 가진다**(ID 유지 · 범위 축소). **표시 단언은 `SPEC-COPILOT-SHEETPIPE-001`이 자기 번호로 가져간다** — A는 B의 번호를 인용하지 않는다.

> **[HARD] v1.0.0 범위 정정(G-6) — ②③은 A가 소유한 코드로 만족시킬 수 없다.** v0.9.0까지 이 AC는 ②③으로 "종류가 정해진 **뒤에야** 파서가 **정확히 한 번** 돈다(호출 계수 1) · 행 수는 그 파싱에서 파생된다"를 요구했다. 그런데 ①은 **판별이 파서를 아예 부르지 않을 것**을 요구한다 — 그리고 A가 소유한 코드는 **판별기가 전부**다. **파서를 부르는 주체(바이트를 나르는 층)가 A 안에 없으므로, A의 범위에서 ①과 ②③은 동시에 성립할 수 없다.** M1은 이 모순을 만나 "판별 중 호출 0회를 단언하고, 종류가 정해진 뒤 파서를 한 번 불러 보였다"로 읽어 넘겼다(`progress.md` §E.2 G-6) — 그 읽기는 합리적이었으나 **②③이 재고 있는 성질은 A의 것이 아니다**.
>
> **처분**: ②③은 **`SPEC-COPILOT-SHEETPIPE-001`의 대응 기준으로 넘긴다**(B가 자기 번호로 소유한다 — A는 B의 AC 번호를 인용하지 않는다). A에 남는 것은 **①④, 즉 순서 단언뿐**이다. 아래 Then 절이 그것이다.

**Given** 유효한 패치 CSV 바이트, **When** 판별기를 부르면, **Then** ① 판별이 끝나기 **전에는** 어떤 종류의 실제 파서도 호출되지 않는다(후보별 시험 파싱 0회) · ④ 판별 과정에서 `MissingColumnsError`가 발생하지 않는다(예외를 제어 흐름으로 쓰지 않는다 — §F 속성 B).

> **번호를 ①④로 유지하는 이유**: ②③을 지우고 ①②로 다시 매기면 `progress.md` §E.2의 M1 판정 기록(①~④로 적혀 있다)이 가리키는 대상이 어긋난다. **비어 있는 번호는 이사 간 자리**이며, 이 SPEC은 REQ 번호에서도 같은 규약을 쓴다(`spec.md` §B 서두).
...
**③의 검증 토큰 — 행 머리 앵커(M1 결함 D-1 교정)**:

```
grep -n "^상태: 발동함" .moai/specs/SPEC-COPILOT-FILEARG-001/plan.md
```

`plan.md` §D.1a의 **기계 판독 상태 줄** 하나만 이 앵커에 걸린다. **N/A일 때 빈 출력**이고, 폴백이 실제로 발동해 그 줄이 `발동함`으로 갱신됐을 때만 한 행을 낸다.

> **왜 앵커여야 하는가 — 같은 결함이 두 번 났다.** v0.2.0은 이 자리를 `grep -n "발동함"`(앵커 없음)으로 고치면서 "이미 있는 문장을 grep해 오늘도 통과하는 검사"를 없앴다고 적었다. 그런데 **§D.1의 지시문 자체가 그 토큰을 인용**하고 있어서(`… 이 문단을 "발동함"으로 갱신하고 …`), 폴백이 발동하든 아니든 grep은 **언제나 한 행을 냈다** — M1이 실측으로 잡았다(`progress.md` §E.2 D-1). **토큰만 바꾸고 인용을 남긴 것이 원인**이며, 결함은 한 겹 아래에서 그대로 되풀이됐다.
>
> **일반형**: **검사가 문서를 grep 하면, 그 문서가 검사의 토큰을 인용하는 순간 검사가 죽는다.** 토큰을 바꾸는 것만으로는 안 닫힌다 — **인용하는 쪽도 함께 봐야 한다.** 앵커를 인용 불가능한 형태(행 머리 고정 등)로 두는 것이 근본 처방이다. 산문은 토큰을 문장 가운데서 인용하지 행 머리에 두지 않으므로, `^` 앵커는 인용과 상태 선언을 구조적으로 가른다.
>
> **부수 규칙**: `plan.md`에서 `상태: ` 로 시작하는 줄은 **§D.1a 하나뿐이어야 한다**. 둘이 되는 순간 앵커가 다시 애매해진다.

**N/A 판정은 이 grep 하나에 기대지 않는다.** M1 실측 기준 실질 근거는 셋이다 — 위임 술어가 페이로드 **10/10을 흡수**했고(교차 분류표), 레지스트리에 **폴백 행이 없으며**, `pytest -k "declared_fallback"`이 **0건 수집**이다. 앵커 grep은 그 셋과 **일치하는지 확인하는 교차 검증**이지 단독 증거가 아니다.
...
### AC-FILEARG-023 — 핸들러가 해소되지 않는 행은 판별 시점에 후보에서 빠진다 (REQ-FILEARG-008 · 감사 B4 · 결정 M)

> **v0.9.0 정정.** v0.8.0까지 이 기준은 "**등록 툴 집합**에 없으면 제외"라고만 적혀 있었다. 그런데 `vectorworks` 행의 핸들러는 **툴이 아니라 세션 메서드**다(실측: `grep -c 'name="upload_vectorworks_export"' tools.py` → **0**, `grep -c 'def upload_vectorworks_export' session.py` → **1**). 문면 그대로 읽으면 **`vectorworks` 행이 매 판별마다 제외되어 결정 I가 무효**가 된다. 그래서 검사를 **태그별로** 가른다 — 핸들러 열이 두 종류를 담기 때문이며, 태그가 없으면 존재 확인이 애초에 불가능하다(`spec.md` §G.0).

**Given** 레지스트리, **When** 각 행의 핸들러 `(kind_tag, name)`을 해소하면, **Then** 태그가 지정하는 레지스트리에서 확인된다:

| `kind_tag` | 확인 대상 | 오늘 등록된 행 |
|---|---|---|
| `tool` | `TOOL_NAMES`에 `name`이 있다 | `patch` → `import_lxseq_patch` |
| `session_method` | `ChatSession`(`server/web/session.py:3405`)에 `name` 속성이 있고 호출 가능하다 | `vectorworks` → `upload_vectorworks_export`(`:9655`) |

**Given** 주입한 표에 ① 태그 `tool`인데 `TOOL_NAMES`에 없는 행 · ② 태그 `session_method`인데 `ChatSession`에 그런 속성이 없는 행 · ③ **태그를 선언하지 않은** 행이 각각 있을 때, **When** 아무 바이트나 판별하면, **Then** ⓐ 세 행 모두 **판별 시점에** 후보에서 제외된다 — 일치 계수에 들어가지 않는다 · ⓑ 제외 사실이 **오류로 보고**된다(조용한 무시가 아니다) · ⓒ 그 오류가 판별 결과에 실려 올라와, 나중에 실행 시점에 발견되기 **전에** 드러난다 · ⓓ 정상 행들의 판별은 그 오류와 무관하게 그대로 진행된다 · ⓔ **오늘 등록된 두 행은 어느 것도 제외되지 않는다**(`vectorworks`가 제외되면 결정 I가 무효라는 뜻이므로, 이것이 이 AC의 회귀 방지선이다).

**왜 판별 시점인가**: 핸들러 부재를 실행 시점에 발견하면 운영자는 파일을 올리고, 기다리고, 시켜 본 뒤에야 안다. 판별 시점에 걸면 업로드 응답에서 바로 안다.

**뮤테이션**: 검사를 태그와 무관하게 **`TOOL_NAMES` 하나로만** 되돌리면 ⓔ가 **빨개져야 한다** — 그것이 v0.8.0의 상태이며, `vectorworks` 행이 매번 제외된다.

**검증**: `uv run pytest server/tests/test_sheets_registry.py -q -k "handler_resolution"`
...
> **[HARD] ③은 ②를 증명하지 않는다(감사 A3).** ②가 열거한 트리 중 `server/lxseq/` · `server/vwx/` · `server/orchestrator/` · `server/web/`(트리 전체 — `preview.py` **한 파일만** 가드에 있다) · `ui/` · `server/paperwork/` · **`server/prechk/`**는 **`_PRESERVE_PATHS`에 없다**. 실측: 그 트리들은 가드 base 대비 **수만 줄 규모로 바뀐 상태**에서도 스위트가 `41 passed`였다 — 가드가 그 트리들을 **보지 않기 때문**이다. 그러므로 ③은 자기 열 항목에 대한 검사일 뿐이며, **그 밖의 모든 트리에 대해서는 ①의 직접 diff가 유일한 검사다.** ①은 건전하고, 과대 주장이었던 것은 ③이다. 여기서도 구분은 같다 — **"가드가 초록이다"와 "그 가드가 이 트리를 본다"는 다른 문장이다.**
>
> **`server/safety/`가 이 목록에 없는 것은 누락이 아니라 판단이다.** 그 트리는 `_PRESERVE_PATHS`가 아니라 **별도 상수 `_SAFETY_DIR`**(`test_overlap_preserve.py:186`)가 따로 지킨다. 즉 ②의 나머지 트리와 달리 `server/safety/`는 저장소 가드의 시야 **안**에 있으므로, 위 "가드가 보지 않는다" 목록에서 의도적으로 제외했다.
>
> **여기에 절대 수치를 박지 않는 이유(감사 D2).** 이 문장의 논증은 **"그 트리들이 크게 바뀌었는데도 스위트가 초록이었다"**이며, 바뀐 줄 수의 **크기는 논증을 전혀 지지하지 않는다**. 게다가 그 수치는 **재현되지 않는다** — 비교 대상이 `origin/main`이고 이 보드가 커밋하는 동안 `origin/main`이 움직이므로, 같은 명령이 몇 시간 안에 수백 줄씩 다른 값을 낸다(리드와 감사자가 같은 두 base로 재서 각각 다른 값을 얻었고, **누구도 틀리지 않았다**). 그러므로 이 자리에는 수치를 쓰지 않는다. 굳이 수치를 남겨야 한다면 **두 ref와 명령을 함께** 적어야 하며(§D 계수 규약의 [HARD] — "숫자만 적고 그 숫자를 만든 명령을 적지 않으면 틀린 명령을 부른다"), v0.7.0의 이 문장은 **같은 문서 안에서 그 규약을 스스로 어겼다**.

**[HARD] 실행 순서 — 커밋 → 게이트 → 보고.** 취향이 아니라 **정확성 조건**이다. `test_overlap_preserve.py`는 고정 base와 **`HEAD`**를 diff한다 — 실측 `grep -c '\.\.HEAD' server/tests/test_overlap_preserve.py` → **22개소**(예: `:427` `body = _git("diff", f"{base}..HEAD", ...)` · `:446` · `:456`). 그러므로 **작업 트리의 미커밋 변경은 그 진단의 시야 밖**이고, **커밋 전 초록은 운이 아니라 구조적으로 보장된 거짓 신호**다. 오늘 이 보드에서 두 번 표면화했고(t20 · t31), t31에서는 동결 PRECHK 문서의 **실제 위반**을 이 가드가 잡았으나 **커밋 뒤에야** 잡았다(미커밋 상태에서는 `41 passed`). 저장소 메모리의 "게이트는 미추적 파일을 못 본다"와 같은 계열이다.
>
> **개수 정정 기록(v0.9.0).** v0.6.0~v0.8.0은 이 자리를 "세 곳"으로 적었고, B 저자는 **다른 세 곳**을 짚어 정정했다. **둘 다 부분이었다** — 각자 자기가 쓴 좁은 패턴(`{base}` · `{_PRECHK_BASE}` · `{_OVERLAP_BASE}`)이 찾아낸 부분집합만 본 것이고, 넓은 패턴으로 세면 **22개소**다. 지목된 줄은 **누구 것도 틀리지 않았다**; 틀린 것은 "그게 전부"라는 문장이었다. 결론은 약해지지 않고 **강해진다** — 이 조건이 걸린 자리가 셋이 아니라 스물둘이므로 놓치기는 더 어렵다.
...
**[HARD] 전제 확인 — 뮤테이션 전에 대상이 깨끗한지 먼저 잰다(감사 D4).** 이 뮤테이션은 `server/web/preview.py`가 **가드 base 대비 그 밖의 변경이 없을 때만** 판별력을 가진다. 이미 더러운 `preview.py`를 물려받은 실행은 **뮤테이션을 걸기도 전에 빨간 상태**이고, 그 빨강을 자기 뮤테이션의 성과로 읽으면 아무것도 검증하지 못한 채 초록 도장을 찍는다.

```
0) git diff --stat <가드 base>..HEAD -- server/web/preview.py   # 뮤테이션 **전에** 실행
```

**빈 출력이어야 뮤테이션을 시작한다.** 비어 있지 않으면 이 뮤테이션은 **판별력 없음(N/A)**으로 기록하고, 그 사실과 실제 출력을 `progress.md`에 남긴다 — 뮤테이션을 강행해 "빨개졌다"고 적지 않는다.
...
**[HARD] 인프라 단정은 확인 명령을 달고, 고칠 때는 전수로 훑는다.** 「…가 없다 / 안 쓴다 / 하나뿐이다」류 **인프라 단정**은 만들어지는 순간부터 **조용히 만료된다** — 참이던 문장이 거짓이 될 때 아무 신호도 나지 않는다. 그래서 둘을 함께 못박는다.

① **단정할 때는 확인 명령을 함께 적는다.** 이 건에서는 `ls .github/workflows/`다. 명령이 없으면 다음 사람은 그 문장이 **언제 기준으로 참이었는지** 확인할 방법이 없다.
② **고칠 때는 정본 한 곳이 아니라 전수로 훑는다.** 이 건에서는 `grep -rn '테스트 CI' *.md`다. 인프라 사실은 여러 문서에 복제되며, **한 곳만 고치면 나머지가 조용히 남는다**.

> **[HARD] 다만 그 훑기 패턴으로 「다 고쳤다」를 닫지 마라 — 주제 토큰은 고친 문장에도 남는다.** `테스트 CI`는 낡은 단정에도, 그것을 고친 문장에도, 이 규약 문단에도 들어 있다. 그래서 `grep -rn '테스트 CI'`는 **후보를 찾는 데 쓰고**, 닫을 때는 **단정 형태**로 다시 훑는다 — 이 건에서는 `grep -rn '테스트 CI가 없'`이며, 남은 히트는 **눈으로 판독해** 인용된 이력인지 살아 있는 단정인지 가른다. 넓은 패턴은 후보 수집용, 좁은 패턴은 종료 판정용이다. 이것은 이 문서가 `AC-FILEARG-021` ③에서 이미 두 번 겪은 것과 **같은 결함**이다 — 토큰만 보는 검사는 그 토큰을 **설명하는 문장**에도 걸린다.

**실례(이 카드에서 실제로)**: `.github/workflows/test.yml`이 들어오면서 "이 저장소에는 테스트 CI가 없다"가 만료됐고, 코드 레인이 `progress.md`를 고쳤을 때 같은 SPEC 안에 사본 **셋**(`spec.md` §C · `plan.md` · `acceptance.md`)이 남아 있었다. **그 계열 문장을 처음 적어 넣은 라운드가 곧 그 규칙의 첫 위반 라운드였다** — 지시가 한 곳으로만 내려갔기 때문이다. **"한 곳 고쳤으니 됐다"로 끝내지 않게 하는 것이 이 규약의 목적이다.**
