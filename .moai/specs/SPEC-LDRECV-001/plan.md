...
| M3 공유 programmer 중재자 | 022 | 신규 `server/director/programmer_arbiter.py`; 기존 `server/safety/gate.py` **EXTEND**(새 public 메서드 `execute_preapproved` + 공유 private 스테이지에 lock 획득/해제 추가 + `screen()`에 `arbitrate: bool = True` 키워드 매개변수, design.md §2.5); `server/orchestrator/tools.py`·`server/web/session.py`·`measurement/runner.py` — **확정: 수정하지 않는다**(design.md §2 — lock 이 `gate.py` 안에 있어 세 호출부 모두 그대로 둔다, `arbitrate` 기본값 `True` 이므로 이 셋은 키워드를 넘기지 않고도 계속 직렬화된다); `server/web/panel.py`(SHOWUI 소유) **최소 1줄**만 `arbitrate=False` 로 명시 호출(design.md §2.5 — REQ-SHOWUI-013 과의 충돌 해소, M3 착수 후 발견) | **director/chat/import**(panel 제외, design.md §2.5) 모든 shared mutation 이 하나의 lock 을 거침, 충돌 `TARGET_BUSY`, 오래 대기시켜 낡은 승인을 실행하지 않음. panel 은 REQ-SHOWUI-013 대로 chat/director 진행 중에도 busy 없이 통과 | 아니오 | 권장(누락 시 race 가능) |
...
| `gate.py` | 이 SPEC 만 쓴다(새 public 메서드 `execute_preapproved` 추가 + 공유 private 스테이지에 중재자 lock 획득/해제 추가 + `screen()`에 `arbitrate: bool = True` 키워드 매개변수 추가, §2.0-가·나, M3, design.md §2.5). **`screen()` 의 관측 가능한 입출력 계약(결정·사유·감사 로그)은 바뀌지 않는다**(회귀 0, 기존 gate 테스트 스위트로 확인) — 다만 내부 구현에 lock 스테이지가 하나 늘어난다(§2.0-나에서 정정). 충돌 없음 |
| `panel.py` (SHOWUI 소유, 신규 발견 — design.md §2.5) | M3 착수 후 전체 회귀에서 발견한 REQ-SHOWUI-013 충돌 해소를 위해 **최소 1줄**만 수정: `PanelRuntime.fire()`의 `self._gate.screen([command])` → `self._gate.screen([command], arbitrate=False)`. 이 SPEC 이 panel.py 를 쓴 유일한 이유이며, 그 밖의 panel.py 로직은 건드리지 않았다 |
