...
### AC-AXISCORE-006 — 링 오독 2종의 정정

**When** 2겹 링 리그가 좁은 반지름 간격을 가지면, the 분류 **shall** `concentric`을 답한다.

- 대상 요구: REQ-AXISCORE-002 · 004 · ASSUMPTION-79
...
- 전제 확인: ASSUMPTION-79가 NEGATIVE(= 이 리그의 정답이 `concentric`이 아니다)로 판정되면 이 AC는
  `low_confidence` 기대로 다시 쓰이며, 그 경우에도 오늘의 `depth_rows`는 오답으로 남는다.
...
- **통과 판정 (스윕 B · 평면 격자)**: `cols ∈ {3,4,6,10}` × `trims ∈ {2,3,4}` × `zgap ∈ {0.5,3.0,8.0}` ×
  `xpit ∈ {0.5,2.0}` → **72 리그**. 판정이 `zgap`·`xpit`의 **절대 크기에 의존하지 않는다**(같은
  `cols×trims`면 네 스케일 조합이 같은 답). ⚠ 이 스윕의 기대값은 `plan.md` §D의 미결 항목
  *"평면 격자에서 lateral과 vertical 중 무엇이 이겨야 하는가"*(ASSUMPTION-77)의 결정에 종속된다 —
  결정 전에는 **일관성만** 단정하고 승자는 단정하지 않는다.
...
- CI 축약: `plan.md` §D의 미결 항목 *"252 리그 스윕의 CI 축약 기준"*이 결정한다. 축약하는 경우에도
  **전수 스윕 스크립트는 저장소에 남고**, 축약 근거를 테스트 docstring에 적는다.
...
- **통과 판정**: `uv run --frozen pytest -q` → `4716 passed · 7 skipped` + 신규 테스트분, `0 failed`.
  M2 게이트와 M3 게이트에서 **각 1회**만 실행한다(1회 97.56s).
...
| AC-AXISCORE-006 | REQ-002 · 004 · ASSUMPTION-79 | 결함 정정 |
