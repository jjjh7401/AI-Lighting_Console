...
0.333으로 vertical 승 — 전순서 개입 없음). 나머지 29건에는 정답이 없다. 사람 결정이 필요하다
(`plan.md` §D `[NEEDS CLARIFICATION: 평면 격자에서 lateral과 vertical 중 무엇이 이겨야 하는가]`).

또 하나: `_golden_bilateral`(리그 #8, **고정 없음**)이 오늘 `concentric`에서 N1 `depth_rows`로 바뀐다
— depth 0.200 대 concentric 0.200 **정확한 동점**을 전순서가 깬 결과다.
