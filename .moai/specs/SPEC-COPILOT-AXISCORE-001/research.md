...
(depth가 `max_gap`으로 채점됨)와 §3.3(분할 품질 항 없음)이다.

**골든 커버리지 문제의 정확한 형태 — 백분율이 아니라 1 미터다.** 코퍼스의
`_golden_concentric`(6@**r=2.0** + 12@r=5.0)은 `concentric`으로 통과하고, 위 오독 리그는
6@**r=3.0** + 12@r=5.0이다. **두 리그의 유일한 차이는 반지름 간격 3.0 m 대 2.0 m** — 즉 이 SPEC 계열이
존재하는 이유인 창립 오독이 **초록색 골든 픽스처에서 반지름 1 미터 떨어진 곳에 있고, 코퍼스는 그
사이를 들여다보지 않는다.** 이것이 "증상이 난 리그를 고정했을 뿐 원인을 고정하지 않았다"
(`GROUPGEN progress.md:778-779`)의 검증 가능한 형태다 — 스윕을 믿어야 성립하는 0.8%라는 숫자와 달리,
이 문장은 두 리그를 직접 돌려 확인할 수 있다. 문서에서는 이 표현을 우선한다.

[검증됨 · 부모 에이전트 재측정 2026-08-05] 두 버킷 서명(`[1,2,2,4,2,2,1]` · `[1,2,2,2,4,2,2,2,1]`)이
본 문서 기록 그대로 재현됐고, `test_topology_naming_seam.py:47-49`의 `two_ring_concentric`은 내측
r=2.0이라 **오독 대상이 아니다.** AC-AXISCORE-006의 픽스처는 반드시 내측 **r=3.0**으로 지어야 한다 —
r=2.0으로 지으면 이미 옳은 동작을 상대로 **공허하게 통과**한다.
...
> **⚠ 위 "포화" 줄의 불변성 주장은 조건부다 — 무조건으로 읽지 말 것.**
>
> 본 절의 초안과 `plan.md` §B.2는 `sep`이 *"같은 축 위 두 길이의 비이므로 축 좌표 ×k에 불변"* 이라고
> 적었다. **그 문장은 `within_bucket_span == 0`일 때 거짓이고, 그것이 정상 케이스다** — `rows.py`가
> 스스로 그렇게 적는다(§3.1). 스프레드가 0이면 같은 축 위에 비를 만들 두 번째 길이가 없어 분모가
> `SPATIAL_ROW_NOISE_SPAN`이라는 **절대** 하한으로 떨어지고, 포화 아래에서 `sep`은 여전히 미터 비례다.
> 포화 `min(…, 1.0)`은 임계 **위쪽에서만** 그것을 가린다.
>
> 정식 조건절은 REQ-AXISCORE-001에 들어갔다(같은 결함 유형의 선례 2건도 그곳에 기록). 실측 경계와 두
> 모집단의 분리는 아래 §9.2를 볼 것.
...
## 9. 구현 후 실측 (2026-08-05 · 워크트리 `axiscore`)

§1~§8은 착수 전 조사다. 이 절은 **착지 후** 같은 워크트리에서 다시 잰 값이며, 앞선 기록과 다른 곳은
명시한다.

### 9.1 기준선 재현 — 전부 일치

착수 전 §3.1·§5.1·§5.2·§5.3의 숫자를 **하나도 빠짐없이 재현**한 뒤에 코드를 건드렸다.

| 측정 | 기록값 | 재현값 |
|---|---|---|
| `three_rows_two_trims` depth 대 vertical | 60.00 / 80.00 | **60.00 / 80.00** |
| `m6_mirror_flat` concentric 대 lateral | 20.00 / 0.75 | **20.00 / 0.75** (27배 역전) |
| `golden_concentric` depth `max_gap` 대 `min_boundary_gap` | 1.830 / 0.670 | **1.830 / 0.670** |
| `rings 6@3.0+8@5.0` 판정 · 버킷 | `depth_rows` `[1,2,2,4,2,2,1]` | **동일** (max_gap 2.598 / min_bnd 0.9375) |
| `rings 6@3.0+12@5.0` 판정 · 버킷 | `depth_rows` `[1,2,2,2,4,2,2,2,1]` | **동일** (max_gap 2.500 / min_bnd 0.0981) |
| `golden_vertical_levels` z×0.1 | `lateral_split` ⚠ | **`lateral_split`** — 잠재 결함 1 라이브 확인 |
| 배제 규칙만 삭제 시 위상 코퍼스 | 1 failed | **1 failed · 59 passed** |
| 스윕 A 판정 변경 (규칙만 삭제) | 126 / 252 | **126 / 252** |
| 스윕 B · C 판정 변경 (규칙만 삭제) | 0 · 0 | **0 · 0** |
| 405 리그 분류 비용 | ~0.03초 | **0.033초** — D4 유지 |

**조사 기록은 신뢰할 수 있다.** 위 표에서 어긋난 항목은 0건이다.

### 9.2 스케일 불변성의 실측 경계 — 그리고 스윕을 어디까지 넓혀도 되는가

405 리그 × 4 축(x · y · z · 전축) × 5 배율(0.1 · 0.5 · 2 · 10 · 100) = **8100 조합**.

| 모집단 | 조합 | 판정 변동 | 무엇인가 |
|---|---|---|---|
| **포화 구간 안** — 모든 분할 축이 포화 | **5510** | **0** | REQ-AXISCORE-001이 보증하는 범위 |
| **포화 구간 밖** | 165 | 165 | 보증 밖. `sep`이 아직 미터 비례인 구간 |
| **절대 하한이 분할을 다시 자름** | 2163 | — | **불변성 질문이 아니다** |

**세 번째 행을 두 번째와 합치지 말 것.** 분할이 바뀐 리그는 "같은 리그를 다른 단위로 잰 것"이 아니라
**다른 리그**다. 좌표를 충분히 줄이면 경계 갭이 `SPATIAL_ROW_NOISE_SPAN` 아래로 내려가 검출기가 애초에
다른 버킷을 만들고, 그 절대 하한은 `rows.py:52-61`이 *"의도적으로 **절대** 하한"* 이라고 적은 물리 상수다
(5 cm 리깅 공차). 두 모집단을 합치면 2328/8100이 되어 성질이 실제보다 훨씬 나빠 보인다.

**스윕 범위의 상한 — 다음 사람에게**: 배율을 더 넓히면(예: ×0.01) 세 번째 모집단이 급격히 커지고,
"판정이 바뀌었다"가 무더기로 나온다. **그것은 회귀가 아니다.** 절대 하한을 건너간 스윕은 두 개의 다른
리그를 하나로 착각해 재는 것이므로, 불변성 단정은 **분할 형상이 보존된 조합에만** 걸어야 한다.
`test_scaling_an_axis_cannot_change_a_verdict_the_score_no_longer_measures`가 정확히 그렇게 구현돼
있고(형상이 바뀐 조합은 세어서 제외, 포화 여부는 공개 점수로 판정), `exercised > 400` 하한이 그 제외가
테스트를 공허하게 만들지 못하도록 막는다.

포화 여부는 **공개 출력만으로** 판정할 수 있다: 포화하면 점수가 정확히 `min(bucket)/n`이 된다(길이가
전혀 남지 않은 개수 비). 테스트가 `_axis_buckets` 내부를 재구현하지 않아도 되는 이유다.

### 9.3 판정 변경 전수 (AC-AXISCORE-013) — 열거되지 않은 변경 0건

정규화 전 base(규칙 있음) 대 정규화 후(규칙 없음), 405 리그 전수 비교:

| 스윕 | 리그 | 변경 | 내용 |
|---|---|---|---|
| A (행×트림) | 252 | **0** | 규칙을 지웠는데도 판정이 하나도 안 바뀐다 = **규칙이 잉여였다는 증거 그 자체** |
| B (평면 격자) | 72 | **30** | 18건 `lateral`→`vertical` · 12건 반대 — §7.3·D1 그대로 |
| C (2겹 링) | 81 | **2** | 링 오독 2종 → `concentric` (D2가 인도로 확정한 결함 수정) |

합계 **32건**, 전부 §7.3·§5.3이 예고한 세 무리 안이다. **예고 밖 변경 0건.** 고정 19 리그 쪽 1건
(`_golden_bilateral` `concentric`→`depth_rows`, depth 0.200 대 concen 0.200 정확한 동점을 전순서가 깬
결과)까지 합쳐 **33건**이 전수다.

### 9.4 D1이 결정대로 구현됐다는 증거

스윕 B 72 리그를 정규화 점수로 분해하면:

| 구간 | 리그 | 선택 |
|---|---|---|
| `lateral` == `vertical` (정확한 동점) | **12** | 전부 `lateral_split` — 전순서 발화 |
| `lateral` > `vertical` (점수) | 6 | `lateral_split` |
| `vertical` > `lateral` (점수) | 54 | `vertical_levels` |
| **전순서가 더 높은 점수를 덮은 리그** | **0** | — |

마지막 행의 **0**이 핵심이다. `lateral` 기본값을 무조건 선호로 구현했다면 54건이 전부 넘어가고
`_golden_vertical_levels`가 RED가 됐을 것이다(그 리그는 `lateral` 0.250 대 `vertical` 0.333으로
**점수로** vertical이 이긴다). D1이 **동점 전용 기본값**으로 착지했음이 이 숫자로 확인된다.
