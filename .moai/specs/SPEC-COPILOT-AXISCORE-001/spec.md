...
| 하는 일 | 더 높은 점수의 후보를 **제거** | 대등한 후보 중 **선택** |
| vertical이 더 높으면 | 그래도 진다 | **이긴다** |
...
| **평면 격자에서 어느 축이 맞는가** | **NO** | ASSUMPTION-77 — 사람 결정 |
| **동점 전순서가 배제 규칙의 정당한 후계인가** | **NO** | ASSUMPTION-78 — 사람 결정 |
| **새로 `concentric`이 된 링 2종이 옳은가** | **조건부** | ASSUMPTION-79 — 도메인 확인 |
...
- **ASSUMPTION-77 (평면 격자 판정)** — y에 깊이가 없는 `cols × trims` 격자에서 `lateral`과 `vertical`은
  ...
  `research.md` §7.3). 그 변경이 운영상 수용 가능하다. **미검증 — 사람 결정.** 코퍼스가 고정하는 것은
  `_golden_vertical_levels` 1종뿐이고 후보 N1은 그것을 맞힌다(0.250 대 0.333, 전순서 미개입).
- **ASSUMPTION-78 (전순서의 정당성)** — "동점일 때 depth를 먼저 본다"가 "depth가 분할하면 vertical을
  ...
  순서라는 사실이다. **미검증 — 사람 결정.** NEGATIVE면 전순서를 정규화 항으로 대체해야 하고, 그러면
  ASSUMPTION-76을 다시 측정해야 한다.
- **ASSUMPTION-79 (링 오독 정정의 정당성)** — 오늘 `depth_rows`로 나오는 `6@3.0+8@5.0` ·
  ...
  같은 형상이고 코퍼스가 `concentric`으로 고정한다 — 반지름 간격만 다르다. NEGATIVE면 `concentric`
  검출기 자체의 문제이며 본 SPEC 밖이다.
...
| 252 리그 행×트림 스윕 무회귀 | 매개변수화 스윕 | **현상 결속** |
