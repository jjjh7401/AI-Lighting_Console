...
- **REQ-AXISCORE-001** [Ubiquitous] — 각 축 점수 **shall** 무차원이며, **분리도가 포화 구간에 있는 한**
  그 축 좌표의 균일 스케일 변환에 불변이다. 구체적으로: 모든 분할 축의 최소 경계 갭이
  `SPATIAL_ROW_GAP_RATIO × SPATIAL_ROW_NOISE_SPAN` 이상이면, 어떤 축의 모든 좌표에 양의 상수 `k`를
  곱해도 그 축의 점수와 `classify()`의 최종 선택이 변하지 않는다. 이는 §A.1 원인 1을 닫는 기계 검증
  가능한 불변식이며, 오늘은 `within_spread <= 0.05` 구간에서 거짓이다(`research.md` §3.1).

  **조건절은 장식이 아니라 실측된 경계다.** 버킷 내 스프레드가 0인 리그 — `rows.py`가 스스로 정상이라
  적는 경우 — 에는 같은 축 위에 비를 만들 **두 번째 길이가 존재하지 않는다.** 분모는 `rows.py`의 절대
  하한으로 떨어지고, 포화 아래에서 점수는 여전히 미터 비례다. 이를 없애려면 새 기준 길이를 발명해야
  하는데 그것은 **REQ-AXISCORE-008이 금지한다** — 즉 이 조건은 식의 결함이 아니라 정규화가 도달할 수
  있는 한계다. 임계는 항상 **유도식으로** 적는다(`GAP_RATIO × NOISE_SPAN`); 숫자로 적으면 둘 중 하나를
  튜닝하는 순간 문서가 거짓이 된다.

  **실측 (구현 후 · 405 리그 × 4 축 × 5 배율 = 8100 조합)** — 두 모집단을 절대 합치지 말 것:

  | 구간 | 조합 | 판정 변동 |
  |---|---|---|
  | 포화 구간 **안** (보증 대상) | 5510 | **0** |
  | 포화 구간 **밖** (보증 밖) | 165 | 165 |
  | 절대 하한이 **분할 자체를 다시 자름** | 2163 | 불변성 질문이 아님 — 다른 리그다 |

  세 번째 행을 두 번째와 합치면 성질이 실제보다 훨씬 나빠 보인다. 분할이 바뀐 리그는 "같은 리그를 다른
  단위로 잰 것"이 아니라 **다른 리그**이고, 그 절대 하한은 `rows.py`가 의도적으로 소유한 물리 상수다.

  **이 조건절은 되풀이되는 결함 유형의 세 번째 사례다** — *조건부 보증을 무조건으로 적는 것.*
  `REQ-SPATIAL-024`는 약한 의미에서만 참이었고, `REQ-WRITEGATE-005`는 `Cmd()` 리터럴만 덮으면서
  "배포 대상 Lua 소스" 전체를 주장했으며, 본 REQ의 초안이 같은 일을 했다(무조건 스케일 불변).
  **일회성이 아니라 패턴이다.** REQ를 적는 다음 저자는 문장을 확정하기 전에 *"이 문장이 거짓이 되는
  입력이 존재하는가"* 를 먼저 물어야 한다 — 세 번 다 그 질문 하나로 잡혔을 것이다.
