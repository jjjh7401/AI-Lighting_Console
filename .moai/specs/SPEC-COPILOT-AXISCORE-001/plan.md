...
> **base 이동 예정 — 머지 후 rebase 필요**: `SPEC-COPILOT-WRITEGATE-001`의 PR #26이 머지되면
> `origin/main`이 WRITEGATE를 포함하도록 이동한다(배치 결정 4 · 사용자 2026-08-05). 그 시점에 이
> 브랜치는 **rebase**하고 위 base 해시(`b1a630e`) 표기를 갱신한다. 파일 무교차이므로 충돌은 예상되지
> 않으나, `acceptance.md` AC-AXISCORE-011의 `git diff --stat <BASE>..HEAD` 기준점이 base에 걸려 있으므로
> rebase 전 측정값은 재확인 대상이다.
...
**M0(결정 게이트)는 닫혔고 접었다.** §D의 미결 항목 4건이 2026-08-05에 전부 결정됐다
(사용자 2건 · 에이전트 2건). M0의 유일한 내용이 그 게이트였으므로 **남은 작업이 0건**이고, 빈
마일스톤으로 남기지 않고 **M1로 접는다** — 착수 지점은 M1이다. 게이트의 산출(결정 표 ·
ASSUMPTION-77/78/79 상태 갱신)은 §D와 `spec.md` §C.3에 이미 반영돼 있다. 마일스톤 번호는 `acceptance.md`
와 §E의 교차 참조를 보존하기 위해 **M1~M5 그대로** 둔다(번호를 당기지 않는다).
...
2. **행×트림 스윕** — `rows∈{2,3,4,5} × cols∈{3,5,8} × trims∈{2,3} × ypitch∈{1,3,6} × zgap∈{0.5,2,4,10}`.
   **252건 전수를 상시 실행한다**(§D D4) — 축약도 `slow` 마커도 없다. 실측: 288 리그가 `classify` +
   `analyze_spatial_records` 전 경로로 **0.017초**(0.06 ms/리그)이므로 252건은 약 **0.02초**다. 스윕
   B(72) · C(81)도 같은 기준으로 전수 상시다.
...
착수 전에 열려 있던 미결 항목 **4건은 2026-08-05에 전부 닫혔다.** 아래는 결정만이 아니라
**그 결정을 필요하게 만든 분석**까지 남긴다 — 결정문만 남으면 6개월 뒤에 왜 그렇게 골랐는지 읽을 수 없다.

| # | 항목 | 결정 | 결정자 | 동작 변경 |
|---|---|---|---|---|
| **D1** | 평면 격자에서 `lateral` 대 `vertical` | **`lateral`(좌우) 우선** | 사용자 (2026-08-05) | 72 리그 중 **30건** 판정 변경 · ASSUMPTION-77 닫힘 |
| **D2** | 오늘 `depth_rows`로 나오는 2겹 링 2종의 정답 | **`concentric`** — 오늘의 `depth_rows`가 오독 | 사용자 (2026-08-05) | 링 2종 정정 = **본 SPEC이 인도하는 잠재 결함 수정** · AC-AXISCORE-006 · ASSUMPTION-79 닫힘 |
| **D3** | 동점 전순서가 배제 규칙의 정당한 후계인가 | **정당하다** — N1 전순서 확정 | 에이전트 (2026-08-05) | 없음(N1 현행 유지) · ASSUMPTION-78 닫힘 |
| **D4** | 252 리그 스윕의 CI 축약 기준 | **축약하지 않는다 · 전수 상시 실행** | 에이전트 (2026-08-05) | M4 항목 2 · §F 변경 · `slow` 마커 없음 · §E 위험 5 소멸 |

### D1 — 평면 격자의 기본값은 `lateral`이다 (사용자 결정)

**무엇이 열려 있었는가**: y에 깊이가 없는 `cols × trims` 격자에서 두 축은 **대칭적 가설**이다. 오늘은
물리적 갭이 큰 축이 이기고, 그래서 같은 리그도 `zgap`을 바꾸면 판정이 뒤집힌다. 정규화 후 **30 / 72
리그**의 판정이 바뀐다(18건 `lateral`→`vertical`, 12건 반대 — `research.md` §7.3). 코퍼스가 고정하는
것은 `_golden_vertical_levels` 1종뿐이고 후보 N1은 그것을 맞힌다. **나머지 29건에는 코퍼스가 말해주는
정답이 없었다.** 선택지는 셋이었다: (a) 최소 버킷 몫으로 결정 + 동점은 전순서(N1 현행) (b) 평면 리그에서
`vertical`을 항상 선호 (c) 둘 다 확신이면 `low_confidence` 반환.

**결정 — 사용자 (2026-08-05)**: **`lateral`(좌우) 우선.** 선택지 (a)를 채택한다.

**근거(사용자 문면)**: 사람은 조명 버드를 기본적으로 **"없어진 열"**로 읽고, 무대 언어("오른쪽부터
웨이브")가 좌우 중심이므로 `lateral`이 기본값으로 자연하다. 그리고 이 기본값은 **간격 크기와 무관하게
안정적**이다 — 오늘의 결함(같은 리그가 `zgap` 하나로 뒤집힘)을 정확히 그 축에서 닫는다.

**식에 어떻게 착지하는가**: 이 결정의 실체는 §B.2 전순서에 이미 들어 있는 **`lateral_split` >
`vertical_levels`** 항이다(`depth_rows > lateral_split > concentric > vertical_levels`). `lateral`
우선은 **동점 구간의 기본값**이지 더 높은 점수를 덮는 규칙이 **아니다.** 후자로 읽으면 두 가지가 동시에
깨진다: (i) `_golden_vertical_levels`가 RED가 된다 — 그 리그는 `lateral` 0.250 대 `vertical` 0.333으로
**점수로** vertical이 이기고 코퍼스가 그것을 고정한다(`research.md` §7.3, 전순서 미개입); (ii) 배제
규칙을 좌우 방향으로 뒤집어 다시 세우는 것이므로 REQ-AXISCORE-008(**"배제 규칙을 다른 형태로 대체하지
않는다"**) 위반이다. 즉 D1은 D3과 **같은 성질의 물건**이다 — 동점에서만 발화한다.

**동작 변경**: 스윕 B 72 리그 중 **30건**. 그중 코퍼스가 고정하던 1건(`_golden_vertical_levels`)은
...
**무엇이 열려 있었는가**: `6@r=3.0 + 8@r=5.0` · `6@r=3.0 + 12@r=5.0`이 오늘 `depth_rows`로 분류된다 —
버킷이 각각 `[1,2,2,4,2,2,1]` · `[1,2,2,2,4,2,2,2,1]`, 즉 링의 y 투영 artefact를 행으로 읽는 **이 SPEC
계열의 창립 오독과 같은 형상**이다(`research.md` §5.3). 정규화 후에는 `concentric`이 된다. 물어야 했던
것: 안쪽 반지름 3.0 · 바깥 5.0의 2겹 링이 정말 `concentric`인가, 아니면 두 링이 너무 가까워
`low_confidence`가 정직한 답인가.

**결정 — 사용자 (2026-08-05)**: **`concentric`.** 정규화 결과가 맞고 오늘의 `depth_rows`
(버킷 `[1,2,2,4,2,2,1]`)는 **오독**이다.

**근거(사용자 문면)**: 이 형상은 이 SPEC 계열의 **창립 오독과 같은 형상**이며, 정규화가 이를 고치는 것은
부작용이 아니라 **이득**으로 기록한다.

**동작 변경**: 링 2종이 `depth_rows` → `concentric`(버킷 `[6,8]` · `[6,12]`). 이것은 본 SPEC이 인도하는
**잠재 결함 수정**이며 `acceptance.md` **AC-AXISCORE-006**이 고정한다 — 뮤테이션 표적은 골든 픽스처가
아니라 **정규화 그 자체**다(정규화를 되돌리면 두 리그가 `depth_rows`로 돌아가 RED). ASSUMPTION-79의
"NEGATIVE면 `low_confidence` 기대로 다시 쓴다" 분기는 **닫혔다.**

### D3 — 동점 전순서는 배제 규칙의 정당한 후계다 (에이전트 결정)

**무엇이 열려 있었는가**: 본 SPEC은 배제 규칙을 지우고 그 자리에 **동점일 때만 발화하는 축 전순서**를
남긴다. 그것이 정책을 이름만 바꿔 옮긴 것 아닌가가 문제였다. 대안은 depth 선호를 정규화 항으로 흡수하는
것이었고, 그러면 ASSUMPTION-76을 처음부터 다시 측정해야 했다.

**결정 — 에이전트 (2026-08-05)**: **정당하다.** N1의 전순서를 그대로 확정한다.

**근거** — 둘이다.

1. 그 전순서는 **오늘의 우연한 동점 처리 그 자체**다. `scored.sort`가 안정 정렬이므로 `contenders`
   삽입 순서(`topology.py:464-470`)가 이미 동점을 그렇게 깨고 있다. 본 SPEC의 변경분은 **우연을
   문면화한 것**이지 새 정책이 아니다.
2. 성질이 **결정적으로 다르다**. 배제 규칙은 **더 높은 점수의 후보를 제거**한다 —
   `three_rows_two_trims`에서 **80점 `vertical`을 빼서 60점 `depth`를 당선**시킨다. 전순서는 **정확히
   동점인 후보 중에서만** 고른다. `vertical`이 더 높으면 여전히 `vertical`이 이긴다 —
   `_golden_vertical_levels`(0.250 대 0.333)가 그 증거다.

이 구분은 `spec.md` §A.4 비교표에 네 행(발화 조건 · 하는 일 · vertical이 더 높을 때 · 오늘 이미
쓰이는가)으로 명시돼 있다.

**동작 변경**: 없다. N1 현행이 그대로 확정이다. ASSUMPTION-78 닫힘.

### D4 — 252 리그 스윕은 축약하지 않고 전수 상시 실행한다 (에이전트 결정)

**무엇이 열려 있었는가**: 전수 스윕이 CI에서 매번 돌리기에 크다고 **가정**했고, 대표 부분집합으로 줄이면
다시 "형상에 고정"되는 것 아닌가가 문제였다. 선택지는 (a) 전수를 `slow` 마커로 분리하고 CI는 부분집합
(b) 전수를 그대로 돌리되 리그 생성을 경량화 (c) 경계값 + 무작위 시드 고정 샘플. 이 항목은 **실측이
없어서** 열려 있었다 — 필요한 것은 도메인 판단이 아니라 숫자였다.

**결정 — 에이전트 (2026-08-05)**: **축약하지 않는다. 전수 상시 실행.** `slow` 마커도 쓰지 않는다.

**근거 — 실측이 가정을 부순다** (부모 에이전트가 이 워크트리에서 측정):

| 측정 | 값 |
|---|---|
| `classify` + `analyze_spatial_records` 전 경로로 288 리그 분류 | **0.017초** (= **0.06 ms / 리그**) |
| 현재 `test_topology.py` 48건 전체 | **0.04초** |
| 전체 스위트 | 약 **90초** |

252 리그는 0.06 ms × 252 ≈ **0.02초** — 전체 스위트의 **0.02 %**다. 스윕 A(252) + B(72) + C(81) 405
리그를 전부 더해도 **0.03초** 규모다. **축약할 근거가 존재하지 않는다.** 반대로 부분집합으로 줄이는 것은
§E 위험 3(*"테스트가 다시 형상에 고정된다"*)을 **계획이 스스로 자초하는** 일이다 — 이 저장소가 같은
이유로 같은 결함을 두 번 놓친 기록이 있다(`GROUPGEN progress.md:778-779`).

**동작 변경**: M4 항목 2와 §F가 바뀐다 — 스윕 A · B · C는 **전수가 상시 단위 테스트**이고, "대표
부분집합 + 축약 근거 docstring + 전수는 재현 스크립트로 보존" 경로는 **폐기**된다. 이 결정으로 §E 위험
3의 표면이 줄고 **위험 5는 소멸한다.** 4건 중 ASSUMPTION 없이 **실측만으로** 닫힌 유일한 항목이다.
...
### 위험 5 — CI 시간 (**닫힘 · 실측으로 소멸**)

**징후**: 스윕이 스위트 시간을 눈에 띄게 늘린다.

**판정**: 이 위험은 **실측으로 소멸했다**(§D D4). 288 리그 분류가 **0.017초**(0.06 ms/리그)이고 현재
`test_topology.py` 48건 전체가 **0.04초**, 전체 스위트가 약 **90초**다(이전 측정 97.56s). 스윕 A+B+C
405 리그를 전부 상시 실행해도 **0.03초** 규모이므로 축약하지 않는다 — 따라서 축약했을 때 되돌아왔을
**위험 3(형상 재고정)의 재유입도 없다.**
...
| **변형 스윕(상시)** | 위와 **같은 명령** — 스윕 A(252) · B(72) · C(81)이 일반 테스트로 포함된다(§D D4) | 405 리그 **전수** GREEN · `slow` 마커 없음 · 추가 소요 약 **0.03초** |
...
전체 스위트는 **M2 게이트와 M3 게이트에서 각 1회**만 돌린다(약 90초 · 이전 측정 97.56s). 변형 스윕은
그와 달리 **단위 단계에 상시 포함**된다 — 실측 비용이 0.03초 규모라 분리할 이유가 없고, 분리하면 §E
위험 3이 되살아난다(§D D4).
