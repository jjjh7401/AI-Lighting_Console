...
> ⚠️ **2026-08-24 하향 — 이 절의 판정은 「원리적 불가」가 아니라 「미측정」이다.**
> 위 제목과 본문은 **한 글자도 고치지 않았다**(그때의 기록이다). 다만 이 판정이
> 내려진 표면에는 `introspect` 가 없었고, 그 전제는 만료됐다 —
> `SPEC-COPILOT-RESTORE-001/readability-survey.md` §A.2 가 지적하고 §A.5 가
> **미측정**으로 하향했다. 재측정은 그룹이 있는 쇼파일을 기다린다(라운드 2 실측:
> `DataPool/Groups childCount 0` — 물을 대상이 없었다).
>
> **이 절을 인용하는 곳이 이 저장소에 20곳 더 있다**(전수표:
> `SPEC-COPILOT-RESTORE-001/overclaim-survey.md`). 그 20곳에는 표식을 달지
> 않았다 — 여기가 생산 지점이고, 점-수정으로 흩으면 계열이 안 마른다.
>
> **결론은 바뀌지 않는다** — 미측정도 백업할 수 없다는 점에서 불가와 같으므로
> REQ-GROUPGEN-022(점유 슬롯 금지)는 그대로 강제된다.
