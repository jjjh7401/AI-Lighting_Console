...
#### A.4-1 REQ-009·REQ-010 의 층 오지정 (t75)

문면은 「**매퍼**가 기존 `Store Preset` 빌더를 재사용한다」였다. 그런데 그 빌더는
`server/web/session.py` 에 있고, 매퍼가 그것을 임포트하면 **순환**이 난다:
...
번역이 툴 층에 있을 때만 만족된다. **AC 의 검사가 옳고 REQ 의 산문이 층을 잘못 적었다.**
...
- **REQ-LXSEQ3-009** `[Ubiquitous]` The **툴 층** **shall** 기존 `Store Preset` 빌더를 재사용한다(§A.2-3).
  🔴 **매퍼가 아니다**(t75 정정 — §A.4-1). 빌더는 `server/web/session.py` 에 있고 매퍼가 그것을
  임포트하면 순환이 난다. 매퍼는 **무엇을 어느 슬롯에 넣을지**까지만 정하고, 명령 문형은 툴 층이 만든다.
