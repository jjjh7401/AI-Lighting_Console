...
| # | 무엇 | 왜 측정하지 않았나 |
|---|---|---|
| **G1** | **페이지·익스큐터 저작 문법의 존부**(ASSUMPTION-16의 실체) | 결정적 테스트가 쇼파일 쓰기를 요구하고, **그 결과가 v1 판정을 바꾸지 못한다**(게이트가 측정 2로 이미 닫힘). ~~후속 SPEC이 익스큐터 축을 열 때 최우선 측정 항목이다~~ → **2026-08-07 정정: 이 갭은 닫혔다.** `.moai/specs/SPEC-COPILOT-PRECHK-001/progress.md:297`이 쓰기 세션에서 `Store Page 2` · `Label Page 2 'PRECHK Probe'`를 발화해 `DataPool/Pages` childCount 1→2와 `prop … Name` 재조회를 확인했다(ASSUMPTION-28 **GO**). **남은 blocker는 G2 하나이며 G2가 선행이다** — G1이 GO여도 G2가 부정인 한 축은 서지 않는다 |
| **G2** | **`Assign Preset ... At Executor <n>`의 실제 효과** | 긍정 검증에 "실재하는 빈 익스큐터"가 필요한데 측정 2가 그 식별 불가를 확정했다. **파싱된다는 사실만 기록**하고 효과는 열어 둔다. **※ 2026-08-07 정정 — 두 축이 갈렸다**: ① **효과는 그 뒤 확인됐다** — `.moai/specs/SPEC-COPILOT-PRECHK-001/progress.md:299`가 `Assign Preset 4.1 At Executor 202`로 대상 익스큐터의 Name이 `'Ballad Yellow Red'`→`'금빛 코러스'`로 **재조회 변경**됨을 관측했다. ② **대상 식별은 여전히 부정 실측이다** — "미검증"이 아니라 **"대상 식별 부정 실측"**이 정확한 문면이다. 즉 **문법도 효과도 있고 막힌 것은 대상 선택 하나**이며, 사용자가 번호를 명시 지정하는 축소형은 `.moai/specs/SPEC-COPILOT-FXLIB-001/spec.md:98`이 이미 출하했다 |
...
- **G2가 후속 SPEC의 출발점을 바꾼다.** plan-phase는 ASSUMPTION-19의 기본 기대값을 "부정"으로 적었으나 실측은 **파싱됨**이었다. 익스큐터 축을 다시 여는 SPEC은 "문법이 없다"가 아니라 **"문법은 있으나 효과와 대상 식별이 미검증"**에서 출발해야 한다. 이 정정을 여기 남긴다.
  - **※ 2026-08-07 재정정 — 한 단계 더 좁혀졌다.** PRECHK M0가 문법(ASSUMPTION-28 GO)과 효과(`progress.md:299` 재조회 변경) 양쪽을 닫았다. 정확한 출발점은 이제 **"문법은 파싱되고 효과도 확인됐으나 대상 식별이 부정 실측되어 자동 배치에 도달할 수 없다"**이다. 미검증이 아니라 **부정 실측**이며, 그 차이가 후속의 착수 여부를 가른다.
...
honest_residuals:
  ...
  - "익스큐터 축 G1(페이지·익스큐터 저작 문법 존부) 미측정 — 후속 SPEC 최우선 항목 ※ 2026-08-07 정정: G1은 PRECHK M0(progress.md:297, ASSUMPTION-28 GO)가 닫았다. 남은 blocker는 G2(빈 익스큐터 식별)이며 그것은 부정 실측이다"
