...
| 그룹 | 건수 | 판정 | 처방 |
|---|---|---|---|
| codesign 부재 | 5 | **4건 진짜 macOS 전용 · 1건은 ⓒ로 닫힌다** | ⓑ+ⓒ 혼합. **ⓐ(러너)는 안 간다** — 재본 결과 비용 대비 안 덮는다(§2.4) |
...
### 1.1 🔴 무엇이 틀렸는지 정확히 갈라 둔다

**그 테스트는 틀리지 않았다.** 「도구가 없는 것은 봉인이 깨졌다는 증거가 아니다」는 옳은 추론이고,
codesign 없는 환경에서 크래시하지 않고 넘어가는 것도 옳은 동작이다. 그 검사는 제 일을 한다.

**틀린 것은 그 파일의 초록을 봉인 커버리지로 세는 것**이다.

이 구분이 중요한 이유: 이건 「검사가 부족하다」가 아니라 **「검사가 있는데 그것이 no-op 통과를 세고 있다」**이고,
**훨씬 나쁜 종류다.** 부족한 검사는 없다는 게 보이지만, no-op 을 세는 검사는 **있는 것처럼 보인다.**
그래서 처방도 다르다 — 그 테스트를 고치는 게 아니라, **세는 쪽의 장부를 고쳐야 한다**(§4).
...
### 2.4 🔴 ⓐ(macOS 러너)는 **안 간다** — 비용 결정을 미룬 게 아니라 재본 결과다

**칸반 리드 결정(2026-08-25).** 근거는 비용이 아니라 **덮는 범위의 비대칭**이다.

러너를 추가해도 검사 대상은 **합성 번들**이다. 이 클래스의 테스트들은 `_signable_app(tmp_path)` 로
만든 임시 번들을 쓴다. 실제 배포 산출물을 보는 검사(`:590` `test_a_built_bundle_carries_a_bootable_sidecar`)는
**빌드가 있는 트리에서만 도는 별개 게이트**이고, CI 는 앱을 빌드하지 않으므로 러너를 붙여도 그 검사는 여전히 skip 이다.

그리고 서명이 실제로 문제되는 순간은 **로컬 macOS 에서 배포본을 만들 때**다. 그 순간은 CI 밖이다.

⇒ **비용을 쓰고도 문제되는 순간을 안 덮는다.** 그러면 그건 방어선이 아니라 **방어선처럼 보이는 것**이고,
이 문서가 §1.1 에서 진단한 병과 **같은 병**이다. 러너를 붙이면 「macOS 검사가 CI 에 있다」가 장부에 올라가고,
그 장부는 여전히 실제 배포본을 안 덮는다.

**이 문단은 나중에 누가 러너를 붙이러 올 때 막으라고 남긴다.** 뒤집으려면 위 비대칭을 반박하면 된다 —
예컨대 CI 가 실제로 `.app` 을 빌드하게 만들면 전제가 바뀐다. 그때는 이 판정도 다시 재야 한다.
...
남는 5건(codesign 4 + launcher 1)은 ⓑ로 명시 고정한다. ⓐ는 가지 않는다(§2.4).**
...
6. **ⓐ(macOS 러너)의 비용은 끝내 재지 않았다.** 그런데 §2.4 의 판정은 **비용에 기대지 않는다** —
   근거가 「덮는 범위의 비대칭」이라서, 러너가 공짜여도 결론이 같다. 즉 이 Gap 은 **판정을 흔들지 않는다.**
   ⚠️ 뒤집으려면 비용을 재는 게 아니라 **비대칭을 반박**해야 한다(예: CI 가 실제 `.app` 을 빌드하게 만든다).
