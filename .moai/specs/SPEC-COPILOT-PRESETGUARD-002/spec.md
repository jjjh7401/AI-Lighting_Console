...
- **REQ-PG2-005** [Unwanted] — the 개정 **shall not** 기존 안전 동작을 퇴행시킨다:
  ...
  경로 유지.
- **REQ-PG2-006** [Event] — WHEN 덮어쓰기 확인 카드의 답이 새 시작 번호를 담을 때
  (`"2.31부터 저장해줘"`·`"31번부터"` — `_BASIC_POSITIONS_START` 문법), the 흐름
  **shall** 그 답을 승낙도 거절도 아닌 **재지정**으로 읽어, 기존 대상 구간을 덮지 않고
  새 시작 번호로 점유 검사부터 재진입한다(새 구간이 점유면 새 카드). 번호가 없는 그 외
  문장은 계속 `unrecognised`다 — 재지정은 숫자+구간 문법이 있을 때만 성립한다.
  (실사용 실측 2026-08-16: 운영자가 카드 안에서 "2.31부터 저장해줘"로 방향 전환을
  시도했고, fail-closed 거절 후 흐름이 끝나 새 요청을 처음부터 다시 써야 했다.)
