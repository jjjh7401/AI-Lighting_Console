...
| REQ | 요구사항 | 근거 |
|---|---|---|
| REQ-LDDESIGN-062 | **When** 포지션(`pos`)이 이전 큐와 달라지면, MIB 검사는 **SHALL** 그 변화 직전 무버가 이미 꺼져 있던 시간을 확인하고, 그 시간이 이동 시간(plan.md §F 잠정값 — 이동 1.5초 + 정착 0.5초, M8 콘솔 프로브로 실측치 교체 예정) 이상이면 `dark`(어두운 채 이동 완료)로, 부족하면 `mark`(Mark 큐로 앞당겨 삽입 가능)로, 무버가 켜진 채라면 `live`(경고)로 판정한다. | 문서 §5.5, `tracking-timing-mib-20260921.md` |
