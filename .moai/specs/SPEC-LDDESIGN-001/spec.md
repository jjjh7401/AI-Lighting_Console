...
| 2026-09-23 | **REQ-004·REQ-030 예외 구절(t439).** 감독 결정 인용: 「음악 스타일마다 다르니 하나로 고정은 무리」 → 후렴 회차 색 고정은 기본값(modulate)으로 두고, 감독이 곡마다 고르는 `per_chorus`(Q2B)는 회차마다 색을 바꾸는 예외로 둔다. 해당 곡의 G7(후렴 주색 동일)은 n/a. 곡별 스위치 정식화(분할 큐 주·보조색 맞바꾸기를 `palette_mode`로 끌어올리기·UI)는 카드 t445. |
...
| REQ-LDDESIGN-030 | **The** 후렴(Chorus) 구간 전체는 **SHALL** 동일한 주색을 유지한다(Final Chorus의 클라이맥스 색 전환은 예외) — 이는 기본값이며, 곡별 `palette_mode`/`color_usage` 선택(`per_chorus` 등)은 예외다(그 곡의 G7은 n/a) — 8곡 실측 전부 후렴 주색 동일(`apply-candidates-20260921.md` C1 표). | `apply-candidates-20260921.md` C1, G7 정의 |
