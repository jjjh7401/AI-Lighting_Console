...
## §0 인수인계 — 여기서 시작한다 (2026-08-03 최초 작성 · **동일자 run-phase 완료 시점으로 갱신**)
...
**상태**: **run-phase M1~M7 + 감사 소인 완료 (2026-08-03).** base `origin/main` = `3176900`, 작업 브랜치 `spec/introspect-001`. 응답기는 라이브 배포·동작 중이며 **버전은 `console/lua/copilot_responder.lua`의 `M.VERSION`이 정본**이다. ASSUMPTION **46~49 TRUE**(M1) · **50 확정**(M3 소스 고정 + M6 라이브) · **51·52 관측 범위 내 판정**(M7). 결정은 `plan.md` §F, 마일스톤별 증거는 §E.2, 감사 이력은 `.moai/reports/{plan-audit,sync-audit}/`가 정본이다.

> **이 절에 변동 수치를 적지 않는다(2026-08-03, plan-audit review-2 D4 · review-3 재지적 소인).** 커밋 수·테스트 통과 수·버전 리터럴·AC 개수를 여기 박아두면 **다음 커밋 하나로 즉시 거짓이 된다** — 실제로 두 번 그렇게 됐다(§0을 M8 커밋 직전에 갱신해 v1.6.0·4295로 남았고, 게이트 테스트에 박아둔 `1.6.0` 리터럴은 1.6.1에서 깨졌다). 수치가 필요하면 **정본을 보라**: 테스트 수는 `uv run pytest server/tests/ -q`, 버전은 `M.VERSION`, AC 개수는 `acceptance.md`, 커밋은 `git log 3176900..HEAD`. 이 SPEC의 중심 명제("관측하지 않은 것을 단정하지 않는다")는 자기 문서에도 적용된다.

**이 SPEC의 한 줄**: *"후보를 소진했다"는 "그 정보가 없다"를 함의하지 않는다* — 코디네이터가 Executor 핸들에 후보를 전수로 찍고 실패한 뒤 Sequence 핸들에서 `CurrentCue`를 찾은 사건이, 그 비약을 구조적으로 불가능하게 만들 도구를 요구했다. (카운트 표기는 `research.md` §1의 "카운트 약칭 정리"를 따른다 — 이름까지 열거된 후보는 21종이고 `Index`·`Fader`는 그 밖의 2건이다.)
...
- **다음 단계**: **PR**. M1~M7과 감사 소인이 모두 닫혔다. sync-audit review-1의 Critical(AC-INTROSPECT-004의 **런타임** 교차 대조 부재)은 응답기에 구현·라이브 재배포됐고 sync-2가 하네스·라이브·뮤테이션 2종으로 **종결 확인**했다. plan-audit의 D1~D6도 소인됐다. 마지막 재감사 사이클의 판정은 `.moai/reports/`의 최신 보고서를 볼 것.
- **run-phase 미해소 없음**: `props` 상한(ASSUMPTION-50)은 16으로 확정돼 M3 산술 테스트와 M6 라이브가 닫았고, ASSUMPTION-51·52는 M7이 **관측 범위를 명시해** 판정했다. M1 프로브의 쇼파일 증거 폴백은 "불허 + 불필요"로 닫혔다.
...
> **이 절은 plan-phase 시점의 동결 스냅샷이다 — 갱신하지 않는다.** 아래 수치(코드 변경 0 · 열린 질문 2 · ASSUMPTION 46~52 미해소)는 *그 시점의 사실*이며 현재 상태가 아니다. 현재 상태는 §0과 §E.2가 나른다. 감사 시 이 절을 현재 상태로 읽지 말 것(plan-audit review-2 D4 소인).
