...
- **REQ-INTROSPECT-024** [Unwanted] — 본 SPEC **shall not** LLM 대면 툴을 추가한다. 닫힌 툴 집합의 크기는 **본 SPEC에 의해 변하지 않는다**(델타 0). v1의 소비자는 개발자·에이전트용 진단 경로이며, 쇼 진행 중 모델이 쇼파일을 훑는 표면을 열지 않는다. (기록된 결정 — §F D-4.) **절대 개수를 이 요구에 고정하지 않는다**: 집합은 다른 SPEC이 정당하게 늘린다(base 시점 18 → `main` 머지 후 22, 2026-08-03 SPATIAL/GROUPGEN). 고정하면 남의 정당한 추가가 본 SPEC의 요구 위반으로 읽힌다 — 버전 리터럴에서 겪은 것과 같은 함정이다.
...
- 자기진단 능력을 모델의 툴로 등재하는 것. 닫힌 툴 집합은 **본 SPEC에 의해 늘지 않는다**로 유지된다(REQ-INTROSPECT-024, 결정 D-4). v1 소비자는 개발자·에이전트용 진단 경로다.
...
| **D-4** | **LLM 툴을 추가하지 않는다.** 닫힌 툴 집합에 본 SPEC의 추가 0 (본 SPEC 기준 델타 0 — 절대 개수는 다른 SPEC이 늘린다: base 18 → main 머지 후 22). | v1 소비자는 발견을 수행하는 개발자·에이전트다. 쇼 진행 중 모델이 임의 핸들을 훑는 표면을 여는 것은 별개 결정이며 별개 SPEC에서 다룬다. 툴 개수는 어느 쪽이든 AC로 고정된다. |
