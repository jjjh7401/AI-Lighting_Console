...
상한값 자체는 열린 질문이다(plan.md §F). 참고 지점: 코디네이터의 실사용은 **22종**이었으므로, 16이면 2회 요청, 24면 1회다.
...
> **plan-phase에서 이 절은 비어 있다.** run-phase M1이 판정 결과를 여기에 접어 넣는다(EXECBODY-001 design.md §5와 동일한 형식).
...
1. **`introspect` 절단분은 회수할 수 없다.** `state`는 슬롯별 개별 조회라는 우회로가 있지만, 열거는 이름을 모르면 물을 수 없다. `total`이 누락 사실을 드러낼 뿐이다(§4.2).
...
4. **클래스 단위 일반화는 미검증이다.** 같은 클래스의 다른 인스턴스가 같은 필드 집합을 신고하는지는 ASSUMPTION-51이며 M7에서 판정한다. 판정 전까지 "Executor는 이런 필드를 갖는다"는 진술은 **관측한 그 인스턴스에 대해서만** 참이다.
