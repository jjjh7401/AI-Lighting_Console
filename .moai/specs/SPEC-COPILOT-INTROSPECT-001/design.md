...
3. **LLM 툴 미등재** — 닫힌 툴 집합에 본 SPEC의 추가 0 (본 SPEC 기준 델타 0 — 절대 개수는 다른 SPEC이 늘린다: base 18 → main 머지 후 22)(REQ-INTROSPECT-024). v1의 유일한 소비자는 개발자 CLI이므로, 판독된 값이 모델 컨텍스트로 흘러 들어가는 경로가 **구조적으로 없다.**
