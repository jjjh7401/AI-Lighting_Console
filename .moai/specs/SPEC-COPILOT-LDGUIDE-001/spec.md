...
- **REQ-LDG-017** [Ubiquitous] — the 가이드 **shall** 모든 능력 주장 문장에 증거 원장의
  행 id를 `data-ev` 속성으로 부착한다(예: `<span data-ev="E-17">`). 이것이 REQ-LDG-007의
  "주장 ↔ 원장 행 대응"을 기계적으로 확인 가능하게 만드는 유일한 장치다 — 부착 규약이
  없으면 대응 관계는 원리적으로 아무도 검증할 수 없다.
...
- **REQ-LDG-013** [Event] — WHEN 어떤 위험의 근거가 증거 원장에 귀속될 때, the 가이드
  **shall** 그 경고를 해당 단계 절 안에 배치한다(문서 끝 안전 절로 몰지 않는다).
  ...
  (REQ-LDG-008 승계). 아래 4건은 **하한이 아니라 조사 후보**다 — 4건 전부를 싣는 것이
