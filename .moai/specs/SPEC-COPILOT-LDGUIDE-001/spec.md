...
- **REQ-LDG-004** [Unwanted] — the 가이드 본문 **shall not** SPEC ID·내부 모듈 경로·
  ...
  현업 어휘이므로 제한 없이 쓴다.
- **REQ-LDG-005** [Ubiquitous] — 도구 식별자(`get_rig_context` 등)는 본문에서 배제하고,
  문서 말미의 **참조 부록** 한 곳에만 대조표로 둔다. 기존 가이드에 이미 존재하던
  목록 절의 유용성을 보존하되(REQ-LDG-010), 본문 서술은 감독 언어로 유지하기 위한 분리다.
...
- **REQ-LDG-009** [Ubiquitous] — 수치 주장(도구 수·이펙트 수·문서 종류 수) **shall** 개정
  시점의 실측값과 일치한다. §A.1의 "22 vs 33" 재발을 막기 위해, 수치는 증거 원장의
  측정 명령과 함께 기록한다.
...
- **REQ-LDG-013** [Event] — WHEN 어떤 단계가 저장소에 기록된 **되돌릴 수 없는 위험**을
  포함할 때, the 가이드 **shall** 그 경고를 해당 단계 절 안에 배치한다(문서 끝 안전 절로
  몰지 않는다). 최소 다음 4건을 다룬다:
...
- **REQ-LDG-015** [Unwanted] — the plan 세션 **shall not** 백로그를 직접 등록한다.
