...
- **REQ-SPATIAL-024** [Ubiquitous] — 좌표 기록 **shall** 게이트 단일 관문을 경유한다 — **스크리닝·감사·승인이 모두 선행하지 않고서는** 콘솔에 도달하는 기록 경로가 존재하지 않는다. **커맨드라인 경로에 대해** 세 절이 전부 충족된다(SPEC-COPILOT-WRITEGATE-001). **단일 관문·스크리닝**: 모든 기록이 `gate.screen()`을 지난다. **감사**: 모든 기록이 감사 로그에 남는다 — §E.2.20이 54건 전부를 감사 로그만으로 재구성할 수 있었던 것이 그 증거다. **승인**: 폐쇄 집합 항목 **`"Set Fixture"`**(`server/safety/blacklist.yaml` v2)가 `Set Fixture <fid> Pos[xyz] '<v>'`를 `risky=True` · `category="blacklisted"` · `matched_entry="Set Fixture"`로 분류하므로 **사람의 승인 카드 없이는 실행되지 않으며**, 같은 경로에서 규칙 ③ 쇼파일 백업도 발동한다(`gate.py:362-365`).
  - ⚑ **범위를 명시한다 — 이 보장의 기전은 커맨드 문면 매칭이다.** 그러므로 (a) **응답기 신규 동사는 이 보장에 포함되지 않는다**: 응답기측 기록 동사는 `Set Fixture` 문면을 싣지 않아 `_match_blacklist`에 도달조차 하지 않는다. v1은 응답기 무변경(§E.2.5 D-1=A)이라 그 채널이 **비어 있어서** 오늘 참인 것이고, 채널이 생기면 참이 아니다. 초안이 이 절을 *"커맨드라인 경로든 응답기 신규 동사든"* 무조건 보장으로 적은 것은 **약한 읽기로는 참, 강한 읽기로는 미획득**인 문장 — 이 요구가 고쳤다는 결함과 **정확히 같은 형태**였다. (b) 배포 플러그인의 **Lua 직접 대입**, (c) **`Set Layout … 'PositionX'`** 는 오늘도 무승인 경로다(WRITEGATE `spec.md` §D).
  - ⚑ **삭제 금지 교훈**: v0.3.0까지 이 문장은 *"확인 없이 콘솔에 도달하는 경로는 없다"*가 **감사 의미로는 참, 승인 의미로는 거짓**인 두 갈래로 읽혔고, **그 모호함이 §E.2.20 결함 2(무승인 좌표 기록 54건)를 놓친 문면상의 원인**이었다. 요구 문장이 두 뜻을 허용하면 **그중 약한 뜻이 구현의 알리바이가 된다.** 위 범위 명시가 그 교훈의 적용이며, 한 뜻으로 읽히게 하되 **좌표 기록 일반이 아니라 `Set Fixture` 커맨드라인 형태로 한정**한다 — 넓게 적어 다시 알리바이를 만들지 않기 위해서다.
...
| **좌표 기록의 승인 카드** | **YES(기계) — 카드가 요청된다 / 라이브 렌더 미검증** (SPEC-COPILOT-WRITEGATE-001) | `Set Fixture <fid> Pos[xyz] '<v>'`가 폐쇄 집합 항목 `"Set Fixture"`(`server/safety/blacklist.yaml` v2)에 걸려 `risky=True` · `category="blacklisted"`로 분류되고 규칙 ③ `before_risky_execution()`도 발동한다(`gate.py:362-365`) — 단위 테스트로 **기계 검증되며 뮤테이션 필수**(AC-SPATIAL-031). ⚠ **기계 검증의 범위를 정확히 적는다**: 단위 테스트가 증명하는 것은 `screen()`이 채워진 `ApprovalRequest`로 `request_approval`을 **호출한다**는 것 — 카드가 **요청된다**는 사실이다. **화면에 렌더된다는 관측은 없다**(WRITEGATE ASSUMPTION-68 = `SKIP: CONDITION_NOT_MET`, 라이브 M2 의도적 미실행). 이 칸에 "뜬다"를 기계 증거로 적는 것은 **이 표 자신이 결함으로 규정한 행위**이며(acceptance.md §D), §E.2.20의 교훈이 바로 *채널 수준의 긍정 신호는 관측된 효과가 아니다*였다. **이 행은 v0.3.0까지 NO였다**: 당시 `Set Fixture … Pos*`가 `safe`로 분류돼, 라이브에서 요청하지 않은 좌표 기록 **54건**이 무승인 통과했다(§E.2.20 결함 2). 같은 턴의 `Go+ Page 1.202`는 정상적으로 카드를 띄웠다 — **게이트는 건강했고 좌표 기록만 그물을 통과했으며, 그 구멍이 닫혔다** |
