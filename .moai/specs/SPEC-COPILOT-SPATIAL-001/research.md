...
1. **회전축 연출** — rot* 기록·활용. ~~M0가 회전 판독까지 확인한 경우에만 성립.~~
   **※ 2026-08-07 정정 — 선행 조건 충족, 착수 가능.** M0 P4b가 `rot*` 축의 1:1 대응을 이미 확인했고
   (`progress.md:156`), `SPEC-COPILOT-WRITEGATE-001`이 `Set Fixture … Rot*`를 **risky로 분류**해
   승인 흐름까지 확보했다(`.moai/specs/SPEC-COPILOT-WRITEGATE-001/spec.md:139-140`).
