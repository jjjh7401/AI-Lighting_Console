...
`PRECHK_RIG_SECTIONS`에 `"fixture_types"`를 **추가하지 않는다.** 그 가드는 `create_macro=True` 분기 안에만 있어 추가하면 같은 오버라이드 누락이 인자에 따라 다른 결과를 낸다. 그리고 `server/tests/test_prechk_tool.py:895-905`·`:907`이 누락 섹션 **집합**을 메시지로 단정하므로 추가하면 그 두 테스트가 깨진다.
