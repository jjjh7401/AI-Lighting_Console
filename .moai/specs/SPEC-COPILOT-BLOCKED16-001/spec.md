...
> **2026-08-23 갱신 — 위 인용의 전제는 뒤집혔다(인용문 자체는 원문 보존).**
> 인용은 PRECHK-001 `progress.md:644` 의 원문이므로 고치지 않는다. 다만 그 안의
> "응답기는 프로퍼티명을 열거할 수 없으므로"는 **현재 응답기(v1.6.1)에서 참이 아니다** —
> `introspect` 판독기가 픽스처 한 대에서 108개를 열거한다(카드 t4 실측, 읽기 전용).
>
> **결론은 살아남는다**: `introspect` 에 페이징이 없어 108개 중 28개에서 끊기므로
> (`truncated: true`), 「이 프로퍼티는 없다」는 부재 증명은 지금 표면으로 여전히
> 성립하지 않는다. 이유가 「열거 수단이 없다」에서 「열거가 절단된다」로 바뀌었을 뿐이다.
>
> 상세: `disposition.md` § 2026-08-23 갱신 · 절차: `docs/runbooks/console-read-paths.md` ·
> 나머지 절반(쓰기 필요): `docs/runbooks/fake-real-parity-method.md` §5 W-2.
...
| # | backlog 원문 | 정정 |
|---|---|---|
| 정정-1 | 2단계 "장비 채널폭을 직접 읽지 못해" | 모드별 폭은 열거된다. 막히는 것은 픽스처↔모드 **조인**이며, 그 조인조차 "전량 반증"이 아니라 **현행 읽기 표면 한정 미성립**이다. **미측정 경로는 I-14 1건**이며, I-15는 OVERLAP-001로 **출하 완료**다(§A.3.1). 초판은 여기에 "2건 잔존"이라 적었고 §A.3.1이 그것을 반박한 채로 남아 있었다 — 정정본은 1건이다 |
| 정정-2 | 6단계 "페이드 값 판독 경로가 확인되지 않아" | **확인됐다.** Cue 레벨 `CueFade`는 반증, **Part 레벨 `CueInFade`는 `5.0`으로 판독 확정**(CUETIME-001 M1, 2026-08-14 라이브). **주소 형식도 실측 기록이 있다**(`.claude/skills/ma3-spatial-pointing/SKILL.md:167`). 남은 것은 그 형식이 **타 SPEC 생성 큐와 다중 Part 큐로 일반화되는가**뿐이다 — 초판의 "주소화뿐"은 §A.4.2가 스스로 "과장이다"라 적은 문면이었다(§A.4) |
...
```bash
sed -n '3547,3557p' server/web/session.py
```

> **좌표 정정 [HARD]** — 초판은 `3546,3555`였다. 그 창은 **주장의 근거 앞에서 닫힌다**:
> 대상 부재 시의 `return None`은 **3557행**이고, 창 안 3548행의 `return None`은
> **동사 부재** 경로여서 이 주장의 근거가 아니다. 좌표는 해소되고 주장도 참인데 그
> 좌표가 근거를 담고 있지 않은 형태 — `plan.md` §B.9 술어 ②가 잡는 계열이며 run 단계
> 실측으로 적발했다(`probe.md` §3.1).
