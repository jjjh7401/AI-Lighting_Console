...
| 0.9.0 | 2026-08-18 | sync 감사 반영. **AC-008에 문서 전체 앵커 무결성 축 신설**(F-4) — 종전에는 `id="aiming-non-tool"`을 파괴해도 AC 13건이 전부 통과했다. 초판 신설 축이 *도착지 소실*만 보고 *참조 소실*은 요구조차 하지 않아(감사 delta G-1, 형태 A 실증) **필수 앵커를 리터럴로 하드코딩**했다 — AC-006이 데이터 유도를 `split()` 하드코딩으로 닫은 것과 같은 교정. `comm`+프로세스 치환을 POSIX 루프로 교체(G-2). **AC-007 `cited` 계수를 이식 가능한 형태로**(F-11) — `grep -coE`는 구현에 따라 매치 수(ugrep)와 행 수(GNU/BSD)로 갈린다. AC 수·판정 축 불변(`AC-COUNT` 13). |
| 0.8.0 | 2026-08-18 | run 단계 review 지적(P2-1) 반영. **AC-007 `cited` 할당 신설** — 선언만 되고 할당이 없어 `(cited=)` 빈 값을 출력하던 분모를 실제로 세도록 고치고 빈-입력 가드를 붙였다. AC 수·판정 축 불변. |
...
# 분모를 먼저 센다. v0.7.0까지 `cited`는 **어디에서도 할당되지 않아** 출력이 `(cited=)`였다 —
# 판정 축은 정상이었으나 "몇 건을 봤는가"가 빈 값이었다(review P2-1). 규약 3의 빈-입력 가드도 겸한다.
# `grep -coE`는 grep 구현에 따라 매치 수(ugrep)와 행 수(GNU/BSD)로 갈린다 — 이 맥에서는
# 매치 수라 값이 맞지만 다른 환경에서 분모가 조용히 줄어든다(감사 F-11, sync 세션 정정).
# `-o | wc -l`은 어느 구현에서도 매치 수다.
cited=$(LC_ALL=C grep -oE '[A-Za-z0-9_./-]+\.(md|py):[0-9]+' "$D" | wc -l | tr -d ' ')
...
# (4) 문서 전체 앵커 무결성 — 신설 절의 링크가 도착지를 갖는가.
#     개별 앵커 1건만 검사하면 다음 편집이 **다른** 앵커를 죽일 때 13건 전부 초록이다(감사 F-4).
#     빈-입력 가드가 있는 이유는 규약 3과 같다 — 추출이 깨지면 "깨진 앵커 0건"이 되어
#     통과가 부재의 증거로 둔갑한다.
refs=$(LC_ALL=C grep -oE 'href="#[^"]+"' "$G" | LC_ALL=C sed 's/href="#//;s/"$//' | LC_ALL=C sort -u)
ids=$(LC_ALL=C grep -oE 'id="[^"]+"' "$G" | LC_ALL=C sed 's/id="//;s/"$//' | LC_ALL=C sort -u)
[ -n "$refs" ] || { echo "FAIL 앵커 참조 0건 — 추출이 깨졌다(규약 3)"; exit 1; }
# [A] `for r in $refs`는 **zsh에서 단어분할을 하지 않아 1회만 돈다**(이 저장소의 셸은 zsh 5.9).
#     그러면 $r이 16줄 blob이 되고 개행이 grep에서 대체(OR)로 해석돼 **항상 매치** —
#     반응은 하는데 판별력이 0인 형태다(감사 H-1, 셸 3종 실증). `while read`는 세 셸에서
#     동일하게 행 단위로 돈다. `-F`로 정규식 메타문자도 봉쇄한다.
broken=$(printf '%s\n' "$refs" | while IFS= read -r r; do
  [ -n "$r" ] || continue
  printf '%s\n' "$ids" | LC_ALL=C grep -qxF "$r" || printf ' %s' "$r"
done)
n=$(printf '%s\n' "$refs" | wc -l | tr -d ' ')
# 분할 재발 가드 — 분할이 다시 깨지면 n이 1이 되어 fail-closed로 떨어진다.
[ "$n" -ge 2 ] || { echo "FAIL 앵커 $n건 — 루프가 안 돌았을 수 있다(분할 가드)"; exit 1; }
[ -z "$broken" ] || { echo "FAIL 도착지 없는 앵커:$broken"; exit 1; }
# (5) 발견 경로 자체를 요구한다 — (4)만으로는 **링크를 지우면 통과한다**(감사 delta G-1, 실증).
#     `refs`는 데이터에서 유도되므로 없어진 링크는 요구되지 않는다 — AC-006 [HARD] 주석이
#     「관측되지 않은 게이트는 요구되지 않는다」로 이미 문서화한 계열이다. AC-006이 두 축을
#     `split()`으로 하드코딩해 닫은 것과 같은 교정: 필수 앵커를 리터럴로 요구한다.
#     D-4는 「만드는 일이 아니라 드러내는 일」이고(disposition.md D-4 행), 드러내는 수단이 이 링크다.
#     발견 경로는 「문서 어딘가」가 아니라 **부록 이전 구간**에 있어야 한다 — 부록 안에
#     자기링크를 두면 단계 3에서 사라져도 통과한다(감사 H-3, 실증). FAIL 문면이 검증
#     범위를 넘어서면 그 자체가 과대주장이다.
pre=$(LC_ALL=C awk '/id="appendix-tools"/{exit} {print}' "$G")
printf '%s' "$pre" | LC_ALL=C grep -qF 'href="#aiming-non-tool"' \
  || { echo "FAIL 단계 3 → 부록 발견 경로 소실 (REQ-B16-005 R)"; exit 1; }
echo "PASS 앵커 ${n}건 전량 해소 · 발견 경로 존재"
```

기대: 두 줄 — `PASS 비도구 절 1 · 발화 조건 1 · 도구 표 33행` · `PASS 앵커 16건 전량 해소 · 발견 경로 존재`.
