...
> 마일스톤 **M0~M8** (**M0 실행 완료**), 결정 등록부 **11건(A~K) 전부 해소**, 열린 결정 **0건**, clarification 마커 **0건**. 정본 토큰 계약: REQ **21건** · AC **24건** · ASSUMPTION **5건(41~45 — 41·42는 판정 후 moot)** · 라이브 세션 **2회(1회 소진)**. 마일스톤별 `- **AC**:` 줄은 `acceptance.md §C.0a`와 1:1이며, 합 **24 · 중복 0 · 누락 0**이다.
>
...
### M0 — 라이브 프로브 (cycle_type=none — 측정 세션, 코드 변경 0) 【**실행 완료 2026-08-01 · 정리 잔여 1건 — AC-SCENE-019 미완결**】
...
- **⚠️ 미이행 — 정리 의무**: 프로브가 만든 시퀀스 **191·192·193·194·195·196·197**이 쇼파일에 잔존한다. **`Delete`는 블랙리스트이므로 툴 경로로 지울 수 없다** — 사용자가 GUI에서 직접 삭제하고 그 사실을 progress.md에 기록한다. **이 항목이 닫히기 전에는 AC-SCENE-019가 완결이 아니다.**
...
### 라이브 세션 회계 (결정 A·B의 비용 — **1회 소진, 1회 잔여**)
