...
| 후보 | 내용 | 성질 |
|---|---|---|
| **① 상류 어휘 확장 SPEC** | FXLIB 어미 목록에 `하는`(하다-용언 관형형), LOOKLIB 별칭에 `파란` 추가 → **표제 문장 폴백 해소**. 두 상류가 PRESERVE라 별도 SPEC이 필요하다. `find_fx`·`find_scene`이 **같은 문장에 같은 답**을 내는 것이 인수 기준이 되어야 한다. **→ 제안서 준비 완료: `docs/proposals/2026-08-02-upstream-vocabulary-extension-proposal.md`** — 편집 지점 실측(fx `_ENDINGS`:163 · scene `_ENDINGS`:97 동일 사본 · looks는 어미 축 자체가 없음 · `파란` 자산 0건), 닫힌 집합 고정 테스트 0건 확인, plan-phase 결정 4건 사전 정리, 인수 기준 스케치 포함. **다음 세션은 이 제안서에서 시작하면 된다** | 후속 SPEC (가장 실익 큼) |
