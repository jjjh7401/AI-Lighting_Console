...
| 후보 | 내용 | 성질 |
|---|---|---|
| **① 상류 어휘 확장 SPEC** | FXLIB 어미 목록에 `하는`(하다-용언 관형형), LOOKLIB 별칭에 `파란` 추가 → **표제 문장 폴백 해소**. 두 상류가 PRESERVE라 별도 SPEC이 필요하다. `find_fx`·`find_scene`이 **같은 문장에 같은 답**을 내는 것이 인수 기준이 되어야 한다 | 후속 SPEC (가장 실익 큼) |
