...
| 0.2.0 | 2026-08-01 | manager-spec | **개정 — D1 폐기 후 옵션 D 채택.** 사유: **M0 라이브 프로브가 D1을 무너뜨렸다**(정본 기록 `progress.md §E.2`). ① ASSUMPTION-41(`/CueOnly` 접수)은 **기계 채널 소진** — 날조 플래그 `/CueOnlyy`가 `ok` + 저장까지 되어 `ok`도 재조회도 비변별로 판명. ② ASSUMPTION-42는 A/B 대조에서 **A=B**(둘 다 이월) — 후속 프로브가 **큐 생성이 실질 append-only**임을 실측해, 룰북이 정의한 `/CueOnly`의 보정 대상(“다음 큐”)이 저장 시점에 **존재할 수 없음**을 설명했다. ⇒ **`/CueOnly` 완전 제거**, 트래킹 정책을 **속성 집합 균일화 + 미주장 속성 전수 열거**(옵션 D)로 대체. 근거 조사: `.moai/reports/scene-uniform-attribute-set-proposal.md`(gitignore — 저장소에 없다). 반대로 ③ ASSUMPTION-44(룩+fx 결합) · ④ ASSUMPTION-45(충돌 승자)는 **사람 GUI 관측으로 GO** — D2/D3는 개정 없이 **강화**됐다. 동반 반영: plan-audit iter-1(PASS 0.91) minor 결함 6건 해소. 토큰 변동 REQ 20→**21**(REQ-SCENE-021 신설) · AC 22→**24**(AC-SCENE-023, AC-SCENE-024 신설) · ASSUMPTION 41·42 **moot**. **plan-artifact 해시 변경 ⇒ plan-audit 재실행 강제.** |
...
| 0.2.5 | 2026-08-01 | manager-docs | **머지 전 독립 리뷰 반영 — 출하 코드 결함 P1 3건 수정 + 테스트 강화 + 감사 지적 6건 소인. 요구·인수·마일스톤·정책 무변경**(REQ **21** · AC **24** · Out of Scope **16** · 접두 행 **8** — 토큰 변동 0). PR #10을 열고 **읽기 전용 리뷰어 3 + 독립 sync-audit 1**을 병렬로 붙였다(`gh pr checks` 0건 저장소의 유일한 관문이 사람 리뷰이고, PRECHK PR #7이 2721 전건 통과 상태에서 P1 4건을 낸 선례가 있다). **P1 3건 전부 3794 그린 상태에서 살아 있었다**: ① `compile_scene`이 fx의 `_refuse_unemitted_axes`를 **우회**해 fx 경로가 거부하는 자산을 **선언된 축을 버린 채 수락**했다(두 리뷰어 독립 검출) ② `SceneMatch.fallback`이 `kind == FALLBACK`이라 **축은 잡혔는데 조합 씬이 없으면 성공 형상으로 나갔다**(출하 자산 axis id 8개 중 6개) ③ 미확인 (a) 주장이 **`기계 확인됨:` 표제 아래** 렌더돼 표제와 바로 아래 줄이 서로를 반박했다. 셋 다 오케스트레이터가 **직접 재현한 뒤** 원천에서 고쳤고, **고친 직후 379 passed·죽은 테스트 0건**이라 스위트의 공허함이 실증됐다 — 같은 커밋에서 테스트 **117건**을 보강하고 재주입으로 **15/6/8건 사망**을 확인했다(sha256 원복 IDENTICAL). 신규 상수 2종: `NO_SCENE_COMPOSES_AXES`(다섯 번째 폴백 사유) · `INVALID_TRIGGER_TIME`(컴파일 경계의 음수 `trig_time` 거부). 감사 지적: **`tools.py` 줄 앵커 7계열 전건 부패**(하나가 `plan.md §A.5` PRESERVE 좌표 — **심볼 앵커로 교체**) · **M8 AC 판정 2건에 폐쇄 어휘 절 부재**(신설) · `research.md` 개정 고지의 *"§3 영향 없음"* 오류 · 무범위 "ruff 클린" · **§E.4 자신의 감사 4축 중 3축이 거짓**(전건 정정). **PRESERVE diff 빈 출력** 유지. pytest **3927 passed / 5 skipped** · vitest **223 passed**. |
...
- **REQ-SCENE-015** [Ubiquitous] — **값 라인 충돌 가드 (1급 요구 — 경계 2중, FXLIB REQ-FXLIB-011 계승)**:
  ...
  - **(b) 지시 턴 경계 (교차 호출 — 실행 결과 시점)**: dedupe의 실제 경계는 번들이 아니라 **지시 턴 전체**다(`server/orchestrator/runner.py` 가 `ExecutionContext(executed_ok=…)`를 다음 호출로 전달; 판정 주석 원문 "either **in a prior tool call** … or earlier in THIS bundle" — `server/orchestrator/tools.py`의 `run_commands` 내부 dedupe 판정 루프 주석). the 툴 **shall** 실행 결과의 커맨드별 outcome을 검사해 **비면제 라인에 `skipped_already_executed`가 1건이라도 있으면 해당 컴파일을 성공으로 보고하지 않고** 교차 호출 충돌을 명시적 실패로 보고한다.
...
| dedupe 판정 + 면제 3종 + 축적 경계 | `server/orchestrator/tools.py`의 `_PROGRAMMER_STATE_COMMANDS` 상수 · `run_commands` 내부 dedupe 판정 루프 · `runner.py` ExecutionContext — `[코드]` (**심볼 앵커** — 줄 앵커는 이 SPEC이 `tools.py`를 고치면서 실제로 썩었다. sync-phase 감사 지적) |
| 툴 핸들러 = `run_commands`의 **caller** (제2 실행 표면 금지) | `server/orchestrator/tools.py`의 `run_commands` 정의 + 그 클로저를 호출하는 핸들러들(`instantiate_look` · `prepare_songcue` · `instantiate_fx` · `compile_scene`) — `[코드]` (**심볼 앵커**) |
