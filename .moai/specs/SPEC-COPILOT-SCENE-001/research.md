...
`.moai/reports/handoff/TEMPLATE-병렬웨이브-파이프라인.md:76-81`(gitignore — 저장소에 없다)의 HARD 불변식 6항을 본 SPEC 문맥에서 점검했다. **전부 이행 가능**하며 매핑은 plan.md §F.4가 소유한다(§F.4는 이 **템플릿 6항**에 v0.2.0 개정이 신설한 뮤테이션 재료 규율 1항을 더해 **7항**으로 이행한다 — 개수 차이는 그 신설분이다). 특기 사항 2건:
