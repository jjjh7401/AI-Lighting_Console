...
- **REQ-MUSICSYNC-014** [Ubiquitous] — the 업로드 경로 **shall** 오늘의 로컬 WebSocket 위 base64 프레임만 쓰며, Tauri capability(`src-tauri/capabilities/default.json`: no http, no websocket, no upload)를 **개정하지 않는다.** 새 전송 수단(HTTP 멀티파트·파일 시스템 경로·별도 소켓)은 이 SPEC 밖이다.
  - ⚠️ **2026-09-14 개정** — 이 REQ 는 원래 세 가지(전송 수단 불변 · 원본 상한 · 초과 거절 문구)를 한 줄에 묶고 있었고, 그래서 상한만 고치려는 변경이 capability 방어선까지 건드리는 것처럼 읽혔다. 상한은 **REQ-MUSICSYNC-026**, 프레임 분할은 **REQ-MUSICSYNC-027** 이 정본으로 갈라졌다. 구 본문의 「원본 8 MiB」는 **만료**다 — 아래 026 을 읽어라.
- **REQ-MUSICSYNC-026** [Ubiquitous] — the 업로드 경로 **shall** 상한을 **디코드된 원본 64 MiB**(`67108864` 바이트) 하나로 두며, 초과하는 파일은 **상한 수치를 명시한 한국어 사유**로 거절한다. base64 **문자열 길이** 상한 `((64MiB+2)//3)*4` 은 그 64 MiB 에서 **파생된 값**이지 별도 상한이 아니다 — 두 개의 상한처럼 읽히면 나중에 한쪽만 고쳐진다. 거절 문구에 사용자 페이로드는 한 바이트도 들어가지 않는다(서버가 쓴 고정 문장 + 서버가 가진 숫자만).
  - 근거: 구 상한 8 MiB 에서 감독 곡 31.5MB(= 30.0 MiB)가 **어느 경로로도 올라가지 않았다**. 64 MiB 는 그 곡에 여유를 두고 5분대 무손실 WAV(약 50MB)까지 덮는다. **다운샘플·모노 변환으로 상한을 우회하는 처방은 이 SPEC 밖이다** — `bf16adc` 가 「품질을 거의 안 잃는다」를 반증했다(Cut and Run BPM 107.67→144.23, 34% 오차; 증거 `.moai/reports/threshold-widen-20260914/transcode_compare9.json`).
- **REQ-MUSICSYNC-027** [Event-driven] — **When** 원본이 한 WebSocket 프레임에 담기지 않는 크기이면, the 업로드 경로 **shall** 곡을 **분할 프레임 셋**(시작 · 조각 · 종료)으로 나누어 보내고 서버가 조립한다. 조각 하나의 base64 문자열 길이는 **4 MiB 이하**이며, 이 값은 `uvicorn` 의 기본 `ws_max_size` = `16777216`(16 MiB) 안에 확실히 들도록 잡힌 값이다 — `server/web/serve.py:572` 의 `uvicorn.run(app, host=..., port=...)` 은 `ws_max_size` 를 **주지 않으므로** 기본값이 실효 프레임 천장이다.
  - 조립 중인 부분 바이트는 **세션 첨부가 아니다**: `sha256` · 바이트 수 고지 · 교체 고지 · 이전 분석 무효화는 모두 **종료 프레임을 받아 조립이 끝난 뒤 단 한 번** 일어난다(REQ-MUSICSYNC-013 의 3층 통과는 그대로). 조각이 하나라도 빠지거나 순서가 어긋나거나 선언한 총 길이와 조립 결과가 다르면, the 서버 **shall** 한국어 사유로 거절하고 **아무것도 보관하지 않는다.**
