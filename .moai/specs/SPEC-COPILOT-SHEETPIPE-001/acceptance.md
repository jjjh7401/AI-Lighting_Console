...
**전제 — §A.4 ①이 (나)로 확정되면 이 AC의 전제가 무너진다.** A의 `vectorworks` 행이 사라지면 "판정이 `vectorworks`"라는 가지가 도달 불가가 된다. 그 경우 이 AC는 **N/A로 기록하지 않고**, 무엇이 그 자리를 대신하는지를 `progress.md`에 적은 뒤 재작성한다 — 도달 불가를 통과로 세면 회수가 조용히 일어난다.
...
② 대상 툴 이름은 **레지스트리 행이 지목한 것**이며 핸들러 안에 하드코딩돼 있지 않다 — 행의 `target`을 읽는다.
...
| 그 종류의 대상 툴이 등록 툴 집합에 없다 | `no_target_tool` | 어느 종류의 어느 대상이 없는지 |
...
**[HARD] 실행 순서 — 커밋 → 게이트 → 보고.** 취향이 아니라 **정확성 조건**이다. `test_overlap_preserve.py`는 고정 base와 **`HEAD`**를 diff하므로(본 세션 실측 — `:427` · `:456` · `:464`), 작업 트리의 미커밋 변경은 그 진단의 **시야 밖**이고 **커밋 전 초록은 구조적으로 보장된 거짓 신호**다. 저장소 메모리의 "게이트는 미추적 파일을 못 본다"와 같은 계열이다.
...
1. **AC 11건 전부 PASS.** 조건부 AC는 **없다** — `AC-SHEETPIPE-005`는 §A.4 ①이 (나)로 확정될 때 **N/A가 아니라 재작성**이다(도달 불가를 통과로 세면 기능 회수가 조용히 일어난다).
