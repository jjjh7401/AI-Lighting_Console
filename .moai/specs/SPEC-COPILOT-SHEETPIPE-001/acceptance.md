...
문서 상태: draft (v0.3.0, 2026-08-24 — 채무 D2·D3·D6 반영: AC-005 6항 · AC-001 통과 조건표) · **Tier M · 통과 임계 0.80** · 칸반 카드 t10 · **분할 B(전달경로)**
...
**[HARD] 통과 조건 — "기재가 있을 것"은 통과 조건이 아니다.** 실패를 정직하게 적은 기록도 "기재"는 있다. 항마다 **무엇이 참이어야 통과인지**를 명시한다:

| 항 | 통과 조건 (전부 참이어야 PASS) | 실패 시 |
|---|---|---|
| ① | `server/sheets/`의 판별 진입점 · 레지스트리가 **존재**하고, A의 테스트 실행 결과가 **초록**이다 | **B는 M0에서 멈춘다** — 순서 위반이므로 우회하지 않는다 |
| ② | 좌표 11건이 `grep`으로 재측정됐고, **어긋난 것이 0건**이거나 **어긋난 것이 전부 정정돼 본문에 반영**됐다 | 미정정 좌표가 하나라도 남으면 FAIL |
| ③ | 등재 7지점이 세 명령으로 확인됐고, **6·7번이 첫 명령에 걸리지 않는다는 사실**이 기록됐다 | 7지점 중 하나라도 미확인이면 FAIL |
| ④ | 기준선 2종이 **이 워크트리에서** 측정됐고, 기존 실패가 있으면 **원인별로 나뉘어** 적혔다 | 이월값을 적었거나 원인 미분류면 FAIL |
| ⑤ | A의 태그 재정이 A 쪽에 **반영됨**으로 확인됐다 | **미반영이면 FAIL이 아니라 M1 진입 차단** — B에서 대신 구현하지 않고 A로 올린다 |

**"확인했으나 결과가 나쁘다"는 PASS가 아니다.** ①이 "A 없음"으로 확인됐다면 그 기록은 성실하지만 이 AC는 **FAIL**이며, 그 상태로 M1에 들어가지 않는다. 이 표가 없으면 그 구분이 사라진다.

**검증**: `progress.md` §E.1에 ①~⑤가 **명령·출력과 함께** 있고, 위 표의 통과 조건이 항마다 **충족**되어 있을 것.
...
**왜 이것이 따로 서는가.** `AC-SHEETPIPE-002`가 전부 초록이어도 이것은 빨갈 수 있다 — 슬롯은 제대로 채워지고, 안내도 뜨고, 운영자 화면은 정상인데 **툴이 보는 것만** 어긋나기 때문이다. 뷰를 쓴 사람이 그 위험을 **미리** 적어 두었다(`43b1b04`, docstring `server/web/session.py:3353-3364`) — **사고 기록이 아니라 예견**이므로, 이 AC가 서는 근거는 "한 번 당했다"가 아니라 **두 형태가 한 `build_toolset` 호출에 동시에 살아 있다는 실측**이다(`server/web/session.py:3515` 필드 직접 · `:3525` 뷰 경유).
...
③ 첨부 버튼은 **하나**이고 비이미지 가지가 보내는 프레임은 **하나**다 — `ui/src/App.tsx:629-635`의 **분기**가 두 갈래로 남아 있고 세 번째 프레임 종류가 생기지 않았다. **⑤와 충돌하지 않는다**: ⑤가 손대는 것은 `uploadVectorworksExport` 본문의 **문자열**이고 ③이 지키는 것은 `routeAttachment`의 **분기**다 — 서로 다른 함수다.
...
⑤ **종류를 단정하는 문구가 넷 다 중립화됐다**(`spec.md` §G.1이 정본) — `ui/src/useCopilotSocket.ts:336`(전사) · `ui/src/App.tsx:551`(확장자) · `:555`(크기) · `:559`(판독 실패). 넷 모두 **판정 전에** 나가므로 종류를 단정할 수 없다. **`:555`와 `:559`는 오늘의 `patch`에서도 도달한다** — 8 MiB 넘는 패치 CSV, 판독에 실패한 패치 CSV. `:551`은 오늘 미도달이지만 **면제 사유가 아니다**(§G.1). 검증: `grep -c 'Vectorworks' ui/src/App.tsx ui/src/useCopilotSocket.ts`가 **0**일 것 — 오늘 이 값은 넷이다.

⑥ **유산 프레임 주석이 기계로 확인된다** — `routeAttachment` 옆 주석이 앵커 토큰 `SPEC-COPILOT-SHEETPIPE-001`을 달고 있고, `grep -c 'SPEC-COPILOT-SHEETPIPE-001' ui/src/App.tsx`가 **1 이상**이다(오늘 이 값은 **0**이므로 판별력이 있다). **이 검사가 단언하는 것과 하지 않는 것**: 주석이 **있다**는 것만 단언하고 주석의 **뜻**은 단언하지 못한다 — 그래도 아무것도 걸지 않는 것보다 낫다. **기대값이 주석에만 있는 검사는 아무것도 단언하지 않으므로**, 최소한 그 주석의 존재는 기계에 건다.

**[HARD] ②가 이 카드에서 가장 되돌리기 쉬운 항이다.** `REQ-SHEETPIPE-003`을 프레임 전체로 넓혀 읽으면 ②가 사라지고, 그것은 **오늘 살아 있는 기능의 회수**다. ①~⑥ 중 하나라도 빨가면 그 넓은 읽기를 한 것이다.

**뮤테이션(필수)**: `:555`를 원래 문구로 되돌리면 ⑤가, `routeAttachment` 옆 주석을 지우면 ⑥이 **빨개져야 한다**.
