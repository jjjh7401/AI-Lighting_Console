...
문서 상태: **in-progress** (updated 2026-08-24 — M1+M2 실행됨) · 직전 개정 (v0.4.0, 2026-08-24 — run 이후 문면 정정 3건: AC-005 ⑤ 검증 명령 · 뮤테이션 원장 8건 · 계수 규약 일곱 줄) · **Tier M · 통과 임계 0.80** · 칸반 카드 t10 · **분할 B(전달경로)**
...
⑤ **종류를 단정하는 문구가 넷 다 중립화됐다**(`spec.md` §G.1이 정본) — `ui/src/useCopilotSocket.ts:336`(전사) · `ui/src/App.tsx:551`(확장자) · `:555`(크기) · `:559`(판독 실패). 넷 모두 **판정 전에** 나가므로 종류를 단정할 수 없다. **`:555`와 `:559`는 오늘의 `patch`에서도 도달한다** — 8 MiB 넘는 패치 CSV, 판독에 실패한 패치 CSV. `:551`은 오늘 미도달이지만 **면제 사유가 아니다**(§G.1).

**검증 — 인용부호에 앵커를 건 형태로 잰다.**
...
**앵커가 있는 이유 — 식별자와 문구는 다른 것이고, 이 항이 지키는 것은 문구다.** 앵커 없는 `grep -c 'Vectorworks'`는 위에 열거한 문구 넷뿐 아니라 `sendVectorworksExportUpload` · `buildVectorworksExportUpload` · `uploadVectorworksExport` · `vectorworksUploadError` 같은 **식별자**까지 함께 센다. 인용부호가 그 둘을 가르는 유일한 기계적 표지다.

**측정 (2026-08-24 · 두 ref에서 각각 실측):**

| | 문면 그대로 | 따옴표 앵커 |
|---|---|---|
| `origin/main` (구현 전) | `App.tsx` 13 + `useCopilotSocket.ts` 6 = **19** | 3 + 1 = **4** ← 이 항이 말한 "넷" |
| `02b5b56` (구현 후) | 12 + 5 = **17** | 0 + 0 = **0** ← 이 항이 말한 "0" |

**이 항의 두 숫자를 동시에 내는 것은 앵커 형태뿐이다.** v0.3.0까지 적혀 있던 앵커 없는 명령은 어느 ref에서도 넷도 0도 내지 않는다 — 명령과 기대값이 **둘 다 적혀 있었는데 서로 맞지 않았다**. 일반형은 `spec.md` §C.0 **규약 7**이 든다.

**[HARD] 그리고 그 명령은 만족시킬 수도 없었다.** 구현 후에도 남는 17 중 하나인 `buildVectorworksExportUpload`는 **정의가 `ui/src/protocol.ts:503`에 있고**, 그 파일은 `plan.md` §C가 **"열지 않는 것"**으로 못박은 자리다(`REQ-SHEETPIPE-009` · `AC-SHEETPIPE-011` ②). 즉 앵커 없는 명령을 0으로 만드는 유일한 길은 그 이름을 바꾸는 것이고, 그러려면 **같은 SPEC의 경계 요구를 어겨야** 했다. **인수 기준과 경계가 서로 배타적이면 틀린 쪽은 기준이다** — 경계는 이 카드가 지키기로 한 약속이고, 기준은 그 약속이 지켜졌는지 재려고 나중에 적은 자다.
...
**[HARD] 계수 규약** — `spec.md` §C.0이 정본이며 **일곱 줄** 전부 여기에도 걸린다. 특히:
...
4. **뮤테이션 원장 — 8건.** `AC-003` 뷰 제거 · `AC-004` 업로드 직후 실행 · **`AC-005` ⑤ `:555` 문구 되돌리기** · **`AC-005` ⑥ 앵커 주석 삭제** · `AC-006` 통 이름 제거 · `AC-009` 스키마에 바이트 인자 추가 · `AC-010` 빈 슬롯에 빈 바이트 호출 · `AC-011` 경계 침범(커밋 뒤 판정). 각각 해당 테스트를 **실제로 죽이는 것을 실측**하고 `progress.md`에 적는다.
   - **v0.3.0까지 이 줄은 "6건"이었다.** 빠뜨린 둘은 `AC-SHEETPIPE-005`가 자기 본문에서 **필수**로 요구하는 바로 그 둘이었다 — 원장이 **필수 행을 조용히 떨어뜨린 것**이며, 이는 정정 1(그 AC의 검증 명령이 자기 대상을 잘못 셌다)과 **같은 형태가 한 층 위에서 되풀이된 것**이다 — 세는 자와 세어지는 것이 어긋났고 **어느 쪽도 소리를 내지 않았다**.
   - **[HARD] 이 계수에는 정의 앵커가 없다 — 그래서 드리프트했다.** REQ·AC는 `grep -c`가 낼 수 있는 정의 앵커를 갖지만(이 절 머리의 계수 규약) 뮤테이션은 갖지 않는다. 실측(2026-08-24):
     ```
     grep -c '^\*\*뮤테이션(필수)\*\*' acceptance.md  ->  6   # 필수 헤더 수
     grep -c '^\*\*뮤테이션'           acceptance.md  ->  7   # AC-011은 다른 문구를 쓴다
     ```
     **두 패턴에 `^` 앵커가 붙은 이유**: 앵커 없는 형태는 **이 코드 블록 자신을** 함께 세어 6이 아니라 7을 낸다. 계수 명령을 문서에 적는 순간 그 문서가 코퍼스의 일부가 된다 — 적고 나서 **다시 재야** 알 수 있다.

     **어느 명령도 8을 내지 않는다** — `AC-SHEETPIPE-005`의 헤더 **하나가 뮤테이션 둘**을 담기 때문이다. 그러므로 이 수는 명령이 아니라 **각 AC의 뮤테이션 절을 전수로 읽어** 낸다. 기억으로 옮겨 적으면 이번처럼 줄어든다.
