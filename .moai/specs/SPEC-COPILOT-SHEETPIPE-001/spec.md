...
| 버전 | 날짜 | 작성자 | 변경 |
|---|---|---|---|
| 0.4.0 | 2026-08-24 | manager-spec | **run 이후 문면 정정 3건 — 문서 전용 · 코드 변경 0 · REQ 9 · AC 11 불변.** **정정 1 — `AC-SHEETPIPE-005` ⑤의 검증 명령이 자기 기대값과 어긋났고, 만족조차 불가능했다.** 문면의 `grep -c 'Vectorworks' …`는 `origin/main`에서 **19**(13+6) · `02b5b56`에서 **17**(12+5)을 낸다 — AC가 적은 "넷"도 "0"도 아니다. 두 숫자를 **동시에** 내는 것은 **인용부호 앵커** 형태뿐이며(각각 **4** · **0**), 4구간 측정표를 ⑤에 실어 다음 사람이 검산하게 했다. 게다가 맨 명령을 0으로 만들려면 식별자 `buildVectorworksExportUpload`를 바꿔야 하는데 그 **정의는 `ui/src/protocol.ts:503`** — `plan.md` §C가 "열지 않는 것"으로 못박은 파일이다. **인수 기준과 경계가 서로 배타적이면 틀린 쪽은 기준이다.** §G.1의 측정 명령도 같은 앵커 형태로 정렬했다. 일반형은 §C.0 **규약 7**로 등재 — *검사의 기대값은 적기 전에 실제 코퍼스에 대고 잰다*(이 카드에서 **세 번째** 사례다: "빌더 둘"→실측 하나 · "등재 4지점"→실측 7 · 이 명령). **정정 2 — `plan.md` §E의 "B는 열리지 않는다"는 참이 아니었다.** `origin/main:server/sheets/registry.py:99-104`의 `SheetKindRow`가 드는 것은 `kind` · `predicate` · `handler` · `passthrough_args` **넷뿐**이며 유효 `action`을 말하는 열도 행 계수 훅도 **없다**(`passthrough_args`의 `"action"`은 *전달된다*는 뜻이지 *어떤 값이 유효한가*가 아니다). `server/sheets/**`가 B에게 읽기 전용이므로(`REQ-SHEETPIPE-009`) 구현은 종류-키 표 둘(`SHEET_KIND_ACTIONS` · `_SHEET_ROW_COUNTERS`)을 B 쪽에 들 수밖에 없었다 — 새 종류는 **각 표에 한 줄씩**을 요구하고, 빠뜨리면 앞은 `action`을 명시한 호출을 **전부 거절**하고 뒤는 행 수를 **"미상"으로 떨어뜨린다**(두 실패는 모양이 다르다). 처분은 카드 **`t51`**(A에 `valid_actions` 열 + 행 계수 훅)이며 **`t18` 앞에** 놓였다 — **종류가 하나뿐인 지금 모양을 정하면 도출이 아니라 추측**이므로 두 번째 종류(LXSEQ-002 GROUP)를 기다린다. **미룬 것이지 잊은 것이 아니다.** `spec.md` §D의 형제 문장에도 한정을 달았다. **정정 3 — 뮤테이션 원장이 문서 안에서 갈렸다(6 vs 8).** `acceptance.md` §D 4는 **6**을 열거했고 그 목록은 `AC-SHEETPIPE-005`가 자기 본문에서 **필수**로 요구하는 둘(⑤ 문구 되돌리기 · ⑥ 앵커 주석 삭제)을 빠뜨렸다. **8**로 맞췄다 — run이 실제로 돌린 수(`progress.md` §E.2.5)와 같다. **필수 행을 조용히 떨어뜨린 원장은 정정 1과 같은 형태가 한 층 위에서 되풀이된 것**이다. 이 계수에 **정의 앵커가 없다**는 사실도 함께 적었다(앵커 붙인 두 명령의 실측 **6** · **7** — 어느 것도 8이 아니다. 헤더 하나가 뮤테이션 둘을 담기 때문이며, 앵커를 빼면 그 명령이 **자기 자신을** 센다). **부수 정정**: `acceptance.md` §D의 "`spec.md` §C.0 … **다섯 줄**"이 v0.3.0의 규약 6 추가 이후 고아였다 → **일곱 줄**. |
...
1~6은 A가 다섯 라운드에 걸쳐 값을 치르고 얻은 규약이며, B는 **첫 판부터** 지킨다. **7은 B가 자기 값을 치르고 얻었다** — 이 SPEC의 인수 기준 하나가 스스로 만족 불가능하게 적혀 있었고, 그 결함이 이 카드에서 **세 번째 같은 형태**였다.
...
7. **검사의 기대값은 적기 전에 실제 코퍼스에 대고 잰다.** 규약 1은 *"숫자 옆에 그 숫자를 만든 명령을 적으라"*고 요구하는데, 그 둘을 **둘 다 적고도 서로 맞지 않을** 수 있다 — 규약 1의 사각지대다. `AC-SHEETPIPE-005` ⑤가 그 자리였다: 명령과 기대값("오늘 이 값은 넷")이 나란히 적혀 있었으나 **그 명령을 그 트리에 대고 돌린 적이 없었고**, 실제 값은 **19**였다(정정과 4구간 측정표는 `AC-SHEETPIPE-005` ⑤가 든다). **이 카드에서 같은 형태가 세 번 나왔다** — ① `REQ-SHEETPIPE-008`의 "빌더 둘"(실측 **하나**) · ② §G가 물려받은 "등재 4지점"(실측 **7**) · ③ 이 명령. 셋 다 **적기 전에 한 번 돌려 보면** 잡혔다. 그러므로 기대값을 적는 순간 그 명령을 **그 자리에서 실행하고 출력을 함께 싣는다**. 돌려 보지 않은 기대값은 인수 기준이 아니라 **가설**이다.
...
- 그룹 · 프리셋/FX · 시퀀스 시트를 읽는 코드도, 그 종류의 대상 툴도 만들지 않는다. B가 만드는 래퍼는 **한 종**이고, 그것은 **레지스트리가 지목한 대상**을 부르므로 후속 SPEC이 행을 더하면 **래퍼 자신은** 그대로 둔 채 동작한다. **다만 래퍼 옆의 종류-키 표 둘은 함께 늘어난다** — A의 행에 유효 `action` 열도 행 계수 훅도 없기 때문이며, 그 처분은 `plan.md` §E가 든다.
...
**측정 명령 — 인용부호에 앵커를 건다** (사유는 `AC-SHEETPIPE-005` ⑤가 든다: 이 표가 세는 것은 **문구**이고, 앵커 없는 형태는 **식별자**까지 함께 센다):

```
grep -nE "[\"'\`]Vectorworks" ui/src/App.tsx ui/src/useCopilotSocket.ts
```

앵커 없는 `grep -n 'Vectorworks' …`는 이 표의 넷 말고 **식별자 15행**을 더 낸다(`origin/main` 기준 합계 19). 이 표를 검산할 때 그 형태를 쓰면 넷을 찾지 못한다.
