...
### §F.2 개정 2회차 — PRESERVE 에서 `server/looks/instantiate.py` 를 뺀다 (2026-09-11)

> **본 절은 종료된 SPEC 의 PRESERVE 선언을 사후에 좁힌 이 저장소의 첫 기록이다.** 선례
> (`SPEC-COPILOT-SONGCUE-001/progress.md` §F 개정 절, 2026-07-29)는 **그 SPEC 이 살아
> 있을 때** 자기 `spec.md` 를 v0.1.0 → v0.2.0 으로 올려 `console/lua/**` 를 뺐다. 게이트
> 모듈(`server/tests/test_overlap_preserve.py`) 독스트링이 그 차이를 명시적으로 열어
> 두었다: 「⚠️ 그때는 그 SPEC 이 **살아 있었다.** 닫힌 SPEC 의 선언을 사후에 고치는
> 경우는 이 선례가 덮지 않는다 — **그 판단은 이 게이트 밖이다**.」 게이트가 판단을 자기
> 밖에 두었으므로 판단자는 감독이며, **감독(jaihyun)이 2026-09-11 명시적으로 승인했다.**
> 뒤에 오는 카드는 이 절을 선례로 읽게 된다 — 그래서 무엇이 승인됐고 무엇이 승인되지
> **않았는지**를 아래에 갈라 적는다.

#### 개정 내용 — 정확히 한 항목

`plan.md` §A.5 PRESERVE 재확인 표의 첫 행에서 중괄호 묶음이 좁아졌다.

| | 개정 전 | 개정 후 |
|---|---|---|
| 첫 행 경로 | `server/looks/{schema,loader,roles,resolver,instantiate,matching}.py` | `server/looks/{schema,loader,roles,resolver,matching}.py` |
| 룩 파일 수 | 여섯 | 다섯 |
| 방침 문구 | `PRECHK는 룩 계층 소비자가 아니다. 변경 0건` | 동일(불변) |

행을 지우지 않았고 나머지 여섯 행(`server/looks/library/` · `server/web/preview.py` ·
`console/lua/**` · 룰북 · `tools.py` 두 구간 · `server/safety/**`)은 손대지 않았다.
개정 사유 전문은 `plan.md` §A.5 의 개정 절에 있다.

#### 요청자와 근거

**카드 t348** — 리그의 어느 장비도 조정할 수 없는 축(예: Zoom 없는 리그의 `Zoom`)을
쓰려는 룩을 프리셋 저장 전에 보류한다. 그 보류는 `_plan_stores` 가 이미 가진 사유 사다리에
rung 하나(`axis_absent`)를 더하는 모양이고, **그 사다리는 이 파일에만 있다.** 대안은 두
번째 보류 메커니즘을 만드는 것이었고 기각했다: 같은 질문에 보류 경로가 둘이면 어느 쪽이
발사됐는지 보고가 답할 수 없다.

#### 무엇이 부정되지 **않았는가**

- **PRECHK 의 역사적 사실은 그대로다.** PRECHK run-phase 는 이 파일을 실제로 건드리지
  않았고(변경 0건), 그 기록은 손대지 않았다.
- **`spec.md status` · AC 17건 · REQ 20건 · Out of Scope 는 하나도 바뀌지 않았다.**
  SONGCUE 처럼 `spec.md` 버전을 올리지도 않았다 — 개정 대상이 `plan.md` §A.5 한 행이고,
  §E.4 가 이미 「목록 불변이면 버전을 올리지 않는다」는 판단을 기록해 두었다.
- 좁아진 것은 **이 선언이 앞으로 누구를 구속하는지** 하나다.

#### 🔴 이 승인이 덮지 **않는** 범위 — 같은 파일을 잠그는 두 번째 선언

`server/looks/instantiate.py` 를 드는 선언은 **둘**이며, 본 개정은 하나만 좁혔다.

| 선언 | 출처 | 게이트 | 본 개정 |
|---|---|---|---|
| PRECHK §A.5 첫 행 | `SPEC-COPILOT-PRECHK-001/plan.md` §A.5 | `test_overlap_preserve.py` `_PRESERVE_PATHS` | **좁혔다** |
| `REQ-SONGCUE-021` | `SPEC-COPILOT-SONGCUE-001/spec.md:182` | `test_songcue_bundle.py` `_PRESERVE_LOOK_FILES`(여섯 파일) | **건드리지 않았다** |

실측(2026-09-11, 워크트리 `.claude/worktrees/t348`): 두 게이트 모두 `origin/main`
(`bfaa560`)에서 초록이고, t348 의 head 에서 **둘 다** 빨갛다. 즉 두 번째 게이트는 낡은
잔여물이 아니라 살아 있는 잠금이며, 그 선언 역시 **종료된 SPEC** 의 것이다. 그것을 좁히는
판단은 별도 승인 사안이고 이 절이 대신하지 않는다.

#### 뒤에 오는 카드가 이 절에서 읽어야 할 것

1. 잠긴 파일이 필요하면 처방은 **게이트 예외가 아니다.** 선언 층으로 가고, 닫힌 SPEC 이면
   감독 승인을 받는다. 게이트 우회(`SKIP_MOAI_PREPUSH=1`)·`_PRESERVE_PATHS` 직접 수정은
   경로가 아니다.
2. **한 파일이 여러 선언에 잠겨 있을 수 있다.** 하나를 좁혔다고 푸시가 통과하지 않는다.
   착수 전에 `grep -rn '<파일 경로>' server/tests/` 로 잠금을 **전수** 세라.
3. 승인 기록은 무엇이 승인됐는지와 함께 **무엇이 승인되지 않았는지**를 적는다. 범위를
   적지 않은 승인은 다음 카드가 넓게 읽는다.
