...
> 앵커 주의 — **정정(t11 plan 레인, 2026-08-22).** 앞선 판정문은 「run 세션이 준 앵커는 1건이
> 아니라 2건」이라고 적었으나 부정확했다. **맨 문자열** `_effective_width(record, mode),` 는 확실히
> 2건이지만(`:479` 점유 검사 · `:539` 런 경계), run 세션이 실제로 준 것은 **뒤 문맥을 붙인 형태**였고
> 그것들은 **각각 1건으로 유일**하다. 즉 문제는 앵커의 유일성이 아니라, 표 칸에 개행을 리터럴로 적어
> **문맥 포함 여부가 갈리지 않은 전달 방식**이었다. 앵커를 줄 때는 문맥 포함 여부를 형태로 드러내라.
>
> 재측정(t11 plan 레인, HEAD `0948ccd`): 맨 문자열 2건(`:479` · `:539`) · 앵커 + 뒤 한 줄 조합은
> `occupants,` / `record.universe,` 로 각각 **1건**. run 세션이 준 원문 자체는 이 세션이 보지 못했으므로,
> 「run 세션이 문맥을 붙여 주었다」는 부분은 **리드의 진술을 옮긴 것이고 이 세션이 확인하지 않았다.**
