...
### RV1 · RV2 — PR #72 리뷰 결함, 매퍼 폭 산정 (2026-08-22, run 세션 · cycle_type=autofix)

> 라벨 주의: 리드가 처음 `D3`·`D4`로 적었다가 `RV1`·`RV2`로 정정했다(커밋 `a7fa417`). 이 SPEC 에서 `D1~D7`은 이미 plan-audit 1회차 결함이자 run-phase 결함 라벨로 쓰이고 있어 한 글자가 세 가지를 가리켰다. 배차 payload 정본은 `review-72-defects.md`(리드 소유), 이 절은 구현 증거(run 소유)다.
>
> 커밋 `4d30134` · 대상 `server/lxseq/mapper.py` + `server/tests/test_lxseq_mapper.py` 2파일 · **콘솔 무접촉**(두 결함 다 오프라인에서 재현·검증됨).

#### 왜 기존 검사가 못 봤나

두 결함은 **매퍼가 세운 계획값과 콘솔이 실제로 놓을 자리가 어긋나는 축**에 있다. M4 실기 86대는 타입마다 폭이 하나여서 이 갈래를 지나가지 않았다 — 「아직 만나지 않은 입력」이지 이미 벌어진 사고가 아니다. sync 감사 2회의 검사 목록에도 이 축은 없었다.

#### RV1 — 한 타입이 두 모드로 섞이면 주소가 밀린다

모드 확정이 **타입당 1회**만 일어나고 그 결과를 같은 타입의 모든 행에 재사용했다(`mapper.py` `if csv_type not in mode_resolutions:`). 런을 가르는 `boundary_key`에도 폭이 없어 폭이 다른 행이 한 런으로 뭉치고, 런 폭은 머리 행 값이 됐다. **건너뜀 0건 — 경고가 한 줄도 없었다.**

수정: 해석 캐시 키를 `(csv_type, record.channels, record.mode_label)`로 넓히고, `boundary_key`에 실효 폭을 넣었다. `ImportPlan.mode_resolutions`의 키 모양이 바뀌었으므로 저장소 전체를 확인했다 — `grep mode_resolutions server/ ui/src` → `mapper.py`와 그 테스트뿐, **외부 소비자 0건**(리드가 독립 확인).

#### RV2 — `mode_overrides`가 실측 폭 대신 CSV 폭을 썼다

override 분기가 `names`(문자열 리스트)에서 이름만 찾아, 매칭된 모드의 폭이 **구조적으로 사라졌다**. `mode_overrides`는 `mode_unresolved`의 해법이고 `mode_unresolved`는 애초에 「CSV 폭과 같은 실측 모드가 없다」일 때 뜨므로, override 경로는 **폭이 다른 것이 정상**이다. 수정: `mode_read.modes`를 직접 순회해 `ModeChoice.width`를 쓴다.

#### 자리 계약 — 수용기준을 개정했다 (A안 기각)

payload 원안은 「미리보기가 콘솔의 실제 배치와 같은 말을 해야 한다」며 `fid_map[2] == 1.26`을 요구했다. 폭만 고치고 실측한 결과 그 값이 나왔으나, **같은 수정에서 두 가지가 더 드러났다.**

관측 원문 — A안(폭만 수정), CSV 39ch `1.1`·`1.40`, 콘솔 실측 `Mode 2 25ch` override:

```
빈 콘솔:
  런: 1.1 count 2 width 25 mode Mode 2 25ch
  fid_map: {1: '1.1', 2: '1.26'}
  skipped: []
1.26 점유 후:
  런: 1.40 count 1 width 25
  fid_map: {2: '1.40', 1: '1.1'}
  skipped: [(1, 'address_occupied')]
```

1. 점유를 걸었더니 건너뛴 것이 **FID 2 가 아니라 FID 1**이다. 점유 검사는 `record.channels`(CSV 39)로 발자국을 `1..39`로 잡아 26을 자기 것으로 셌고, 자리 잡기는 실측 폭 25로 돈다 — **두 발자국이 다르다.**
2. **앞 행이 건너뛰어졌는지에 따라 FID2 의 자리가 `1.26` ↔ `1.40` 으로 바뀐다.** 같은 CSV·같은 콘솔인데 결과가 둘이다. 미리보기에서 승인한 것과 apply 사이에 계약이 없다.

**개정 근거(리드 판정, 이 세션의 것보다 강해서 이쪽을 정본으로 적는다): CSV 주소는 감독의 의도가 아니라 장비 자체에 설정된 하드웨어 사실이다.** 픽스처의 DMX 시작 주소는 장비에 물려 있고 콘솔 패치는 그 물리적 사실과 일치해야 한다. FID2 를 `1.26`에 패치하면 콘솔은 **거기 없는 장비**에게 말을 건다. 즉 `1.26`은 「정직하게 밀린 자리」가 아니라 **틀린 자리를 정직하게 보고한 것**이며, 미리보기가 참말을 해도 패치는 여전히 틀린다.

채택(B안): 연속성 판정도 실효 폭으로 한다(`40 != 1 + 25` → 런이 갈린다). 수용기준 `fid_map[2]`가 `1.26` → **`1.40`** 으로 개정됐다(리드 커밋 `3561b3e`). 감독 결정 2026-08-22, 리드 동의.
...
B안 — 1.40 점유 후:
  fid_map: {1: '1.1', 2: '1.40'}
  skipped: [(2, 'address_occupied')]     ← 맞는 행이 건너뛰어진다
```

#### 리드가 요구한 반대 방향 검증 — 구멍이 실재했다

override 가 **더 넓은** 모드일 때는 아무도 보지 않았다. CSV 39ch `1.1`·`1.40`, 콘솔 실측 `Mode 0 50ch`:

```
[수정 전]
  런 1.1  count 1 width 50 → 점유 1..50
  런 1.40 count 1 width 50 → 점유 40..89
  skipped: []
```

**계획이 자기 자신과 `40..50`에서 겹치는데 조용히 통과했다.** 검사가 없어서가 아니라 **세 검사가 전부 다른 곳을 보고 있었다**: 파서의 `address_overlap_in_file`은 **CSV 폭**으로 돌아 모드 확정으로 넓어진 발자국이 시야 밖이고, `_occupancy_skip`은 **콘솔의 기존 점유**만 보며, 그 호출부는 **CSV 폭**을 넘기고 있었다.

수정: `_effective_width`를 모듈 수준 **단일 식**으로 올려 점유 검사 · 런 경계 · 런 폭이 모두 이 하나를 쓰게 하고, `_reject_plan_overlaps`를 신설했다.

```
[수정 후]
  runs: []
  skipped: [(1, 'address_overlap_in_plan'), (2, 'address_overlap_in_plan')]
  detail: 확정된 모드의 폭 50채널로는 계획 안의 다른 행과 1.1–50 구간이 겹친다.
          CSV 폭으로는 겹치지 않았다 — 관여한 행을 전부 거부한다.
```

**왜 한쪽만 살리지 않고 관여한 행을 전부 거부하는가**(파서 R2 와 같은 정책): 겹치는 두 행 중 어느 쪽이 옳은 자리인지 **도구가 정할 근거가 없다.** 먼저 온 행을 살리는 것은 CSV 행 순서를 권위로 삼는 것인데 그 권위는 어디에도 없고, 넓은 쪽을 죽이는 것은 실측 폭을 벌하는 것이라 더 나쁘다. 하나를 골라 살리면 **살아남은 쪽이 옳다는 거짓 신호**가 되어 감독이 그 상태로 apply 한다. 전부 거부하면 감독이 CSV 를 고치거나 override 를 바꾸는 쪽으로 되돌아온다 — 판단을 사람에게 되돌리는 것이 이 자리의 옳은 답이다.

#### 뮤테이션 6회 — 양방향, 복구는 체크섬으로 증명

각 회차는 `mapper.py`만 변형하고 테스트는 불변으로 뒀다. **통과는 복구의 증거가 못 되므로** 복구는 매회 `shasum -a 256 -c`로 원본 일치를 확인했다.

| # | 되돌린 것 | 결과 |
|---|---|---|
| 1 | override 폭을 CSV 폭으로 | 2 failed |
| 2 | 해석 캐시 키를 타입 단독으로 | 7 failed |
| 3 | 런 경계에서 폭 제거 | **40 passed — 아무도 안 잡았다** |
| 4 | 연속성을 CSV 폭으로 | 1 failed |
| 5 | 계획 내 겹침 검사 무력화 | 1 failed |
| 6 | 점유 검사를 CSV 폭으로 | **43 passed — 아무도 안 잡았다** |

**3번·6번을 「이미 안전한가 보다」로 넘기지 않았다.** 둘 다 고쳐 놓고 그 수정이 아무것도 지키지 못하는 상태였다 — 다음 사람이 그 줄을 지워도 스위트가 초록이다.

- **3번이 안 잡힌 이유**: 폭이 다르면 모드 **이름**도 달라 경계가 이미 갈린다. 폭이 유일한 구분자가 되는 것은 **이름이 없는 갈래**(`tree_unread`)뿐이다 → `test_tree_unread_mixed_widths_still_keep_their_csv_addresses` 신설 → 재시도 **1 failed**.
- **6번이 안 잡힌 이유**: CSV 폭과 실효 폭이 다르면서 점유가 **한쪽 발자국에만** 걸리는 입력이 없었다 → CSV 39 / 실효 25, 점유 `1.30`(실효 발자국 `1..25` 밖) → `test_occupancy_uses_the_measured_width_not_the_csv_width` 신설 → 재시도 **1 failed**.

한 건 더 신고한다. **처음 쓴 점유 가드가 공허하게 통과했다.** `'1.26' not in planned`가 참이었는데 이유는 FID2 가 막혀서가 아니라 FID1 이 탈락해서였다. 자리를 상수로 박지 않고 「매퍼가 스스로 고른 자리에 점유를 놓고 다시 계획한다」로 바꿨고, 리드 요구대로 공허성을 재검사했다 — 점유를 아예 걸지 않으면 `target not in planned` → **False**(단언이 깨진다). 공허하지 않다.

#### 검증 원문 (귀속: 이 세션, 2026-08-22, 커밋 `4d30134`)

| # | 명령 | 관측한 출력 | 판정 |
|---|---|---|---|
| ① | `uv run pytest server/tests -q` | `9691 passed, 8 skipped, 1 warning in 139.79s` · exit 0 | PASS |
| ② | `uv run ruff check server/lxseq server/tests/test_lxseq_mapper.py` | `All checks passed!` | PASS |
| ③ | `uv run ruff format --check <같은 범위>` | `4 files already formatted` | PASS |
| ④ | `git diff --stat -- console/lua server/safety server/prechk server/vwx server/paperwork server/rulebook/assets` | **빈 출력** | PASS (봉쇄 구역 0-diff) |

**기준선 귀속**: ①의 9691 은 이 배차의 기준선 **9682**(payload 명시) 대비 **+9**이며, 신규 테스트 9건과 정확히 일치한다. 증거 파일 `.moai/state/verify/t9-d3d4/pytest-final.txt`는 `.gitignore:203` 대상이라 저장소에 없다 — **위 표의 인용문이 정본이다.**

**측정 무효화 1건 신고**: 최초 전체 스위트는 `ruff` 지적(B007 미사용 변수)을 고치기 **전** 트리에서 돌았다. 변수 이름만 바꾼 것이라 영향이 없다고 보았으나 그것은 예상이지 측정이 아니므로, 린트 수정 후 최종본에서 **다시 돌려** 위 ①을 얻었다(두 측정 모두 9691).

#### 미검증 (Gaps — 이 세션이 관측하지 **않은** 것)

- **실기 콘솔 확인 0건.** 두 결함 다 오프라인에서 재현·검증됐고 배차가 콘솔 무접촉을 지정했다. 실제 grandMA3 에서 폭이 섞인 CSV 를 apply 했을 때의 거동은 **아무도 보지 않았다.**
- **`_reject_plan_overlaps`의 O(n²) 비용** — 86행에서는 무시할 수 있으나 큰 리그에서의 비용은 재지 않았다.
- **리드가 예고한 재현 4건**(뮤테이션 3·6 재현, 겹침 검사 무력화, RV1·RV2 원래 입력 재실행)은 **아직 돌지 않았다.** 이 절의 수치는 이 세션 자신의 측정이며 독립 재현이 아니다.

#### 잔여 위험 (관측했음에도 남는 것)

- **좁은 override 에서 콘솔 발자국이 물리 폭보다 좁다.** CSV 39ch 장비를 25ch 모드로 override 하면 콘솔은 `1..25`만 자기 것으로 안다. 실제 장비는 `1..39`를 먹으므로 **콘솔의 겹침 판정이 물리 현실보다 느슨해진다** — 콘솔은 안 겹친다는데 리그에서는 겹칠 수 있다. 이번 범위 밖이고 override 를 쓰는 사람이 감수하는 것이지만, 이 선택을 **고정하는 테스트**(`test_occupancy_uses_the_measured_width_not_the_csv_width`) 본문에 「의도된 선택이며 범위 밖」이라고 못 박아 뒀다 — 나중에 읽는 사람이 결함으로 오해하고 「고치는」 사고를 막기 위해서다. 카드 `t11` 후보.
- **범위 밖으로 남긴 것**: `D2`(FixtureType 핸들) → `t11`. 리뷰 지적 **#3**(`awaited_human` 미전파) · **#5**(완전성 술어 재구현)은 리드가 재현하지 않아 **가설 상태**로 이번 배차에 없었다. **#4**(중복 행이 `rows_total`을 2행 파일에서 4로 보고)는 재현됐으나 콘솔 영향이 없어 손대지 않았다 — `t11` 후보.
- **가짜↔실물 괴리 계열이 이번으로 세 번째다**(D1 절단, D2 핸들, 그리고 이번의 폭 산정 축). 앞의 둘과 달리 이번 것은 실기가 아니라 리뷰가 잡았다. 오프라인 스위트가 못 보는 자리와 **아직 만나지 않은 입력**은 다른 문제이며, 이번 것은 후자였다.
...
sync_audit_pre_fix: FAIL 0.80  # 독립 감사 (HEAD 2b328c6, 수정 **전**) — 아래 「독립 감사」 절
sync_audit: FAIL 0.86          # 재판정 (HEAD 07bc93b, 수정 **후**) — +0.062.
                               # FAIL 동인은 오직 AC-LXSEQ-016(D2 → t11, 감독 결정)이며
                               # 이 카드가 겨냥한 F3·F4·DOC-2·F2 네 건은 전부 닫혔다.
                               # 차단 결함 0 · 신규 코드 결함 0.
...
|---|---|---|---|
| **F1** (D2, 핸들↔이름) | t11 | **t11 (변동 없음)** | 설계 변경이 필요하고 역방향이 `Robin Spiider` 중복으로 모호하다. 감사관의 오프라인 재현 관측(`Counter({'fid_occupied': 86})`)은 **이 세션이 재현하지 않았다** — t11 카드에 「가짜 콘솔 충실도 부족일 가능성」으로 실어 보낸다. **정정(t11 프로브, 2026-08-22)**: 출하된 `FakeConsole` 로는 이 수치가 나오지 않음을 관측했다(대조군 = `Counter({'already_patched': 86})`). 프로퍼티 판독 한 곳을 핸들로 바꾼 변형에서만 `fid_occupied 86` 이 재현된다. 따라서 「감사관이 오프라인에서 재현했다」는 최소한 부정확하다 — 감사관이 변형을 더했는지 라이브 관측을 오기했는지는 **확인되지 않았다**(추정하지 않는다). 증거: `.moai/probes/t11-d2-probe.md` |
...
#### 재판정 (sync-auditor, 2026-08-21 · HEAD `07bc93b` — 수정 **후**)

**판정: FAIL · 가중 조화평균 0.86** (이전 0.80, **+0.062**). 같은 가중치(40/25/20/15)로 재점수해 두 판정을 비교 가능하게 했다.

| 차원 | 수정 전 | 재판정 | 재감사자가 직접 관측한 근거 |
|---|---|---|---|
| Functionality (40%) | 0.70 **FAIL** | **0.80 — 여전히 must-pass FAIL** | F3·F4 뮤테이션을 **양방향으로 직접 재현** · AC-013 게이트가 0건 → 5건이며 기대 결과 ①②③을 **테스트 본문 대조로** 전부 덮음 확인 · 그러나 **AC-LXSEQ-016은 그대로 FAIL** |
| Security (25%) | 0.90 | **0.91** | 제품 코드 exit 0 · 봉쇄 6구역 빈 출력 · 비밀/위험 구문/콘솔 접촉 grep 0건 · F4가 **주소 겹침 가드**(물리 리그 이중 패치를 막는 축)의 판별력을 실제로 증명 |
| Craft (20%) | 0.86 | **0.90** | 커버리지 `TOTAL 346 5 99%` 직접 재측정 · ruff 2종 clean · 스위트 증가분 +1이 F4 신규 1건과 정확히 일치 |
| Consistency (15%) | 0.88 | **0.91** | 거부 부류 목록이 `parser.py` grep 결과와 **정확히 일치**하도록 교정됨 · 소유 경계 넘김을 두 파일에 공개 · 판정 정정 이력을 지우지 않고 병기 |

**FAIL 동인은 오직 AC-LXSEQ-016 한 줄이다.** 그것은 이 커밋이 손대기로 한 항목이 아니다(D2 → 카드 `t11`, 감독 결정). **겨냥한 F3·F4·DOC-2·F2 네 건은 전부 닫혔고 재감사자가 기계로 확인했으며, 점수 상승분 전부가 거기서 나온다.** 차단 결함 **0건** · 신규 코드 결함 **0건**.

재감사자의 표현을 그대로 남긴다 — 「이 FAIL은 **커밋에 대한 비판이 아니라 SPEC의 현재 상태에 대한 서술**이다 — SPEC 스스로도 `implemented`에 멈춰 `completed`가 아니라고 적고 있고, 그 판단은 옳다.」

**재감사가 새로 지적한 것 (전부 low/optional, blocking 0)**

| # | 등급 | 파일 | 내용 | 처분 |
|---|---|---|---|---|
| **D-1** | low | `CHANGELOG.md` | `FAIL 0.80`에 「수정 전」 한정어와 재판정 표기가 없었다(`progress.md`에는 둘 다 있었다). 방향은 **보수적**(과소 표기)이라 조용한 상향은 아니다 | **본 커밋에서 정정** — 두 판정을 나란히 적었다 |
| **D-2** | low | `acceptance.md` §D | DoD 조문은 「AC-016이 **미수행**이면」만 규정하는데 실제 상태는 「**실행했고 1행 실패**」다. 문서는 이를 「미충족」으로 한 뼘 넓게 읽었다 — a fortiori 로 방어되나 조문에 그 갈래가 없다 | **`t11`** (`manager-spec` 소유 · SPEC 본문의 빈칸) |
| **C-1** | info | `test_lxseq_mapper.py` | `_CountingResolutions`가 `get`/`__getitem__`만 계측한다. 훗날 매퍼가 `in`·`.items()`로 접근하면 카운터를 조용히 우회한다 | **오늘은 무해함이 증명됐다** — 접근 지점 `:352`·`:367` 둘 다 `.get`(재감사자 grep 확인). 내구성 주의사항으로만 남긴다 |

**재감사가 재현하지 **않은** 것 (재감사자 자신의 Gaps)** — M4 실기 증거 전량(콘솔 무접촉 준수), run 세션 뮤테이션 20회, 감사 보고서 원문 2종(`.gitignore` 대상이라 「두 감사가 모두 `9681 passed`를 관측했다」는 §E.4 진술을 **독립 검증할 수 없었다**), AC 16건의 개별 판정 근거(브리핑이 지목한 게이트만 검사), F7·F8.

트리 청결 증명: 뮤테이션 2회 모두 실행 직후 복구, 각 복구 직후 `git diff --quiet -- server/` exit 0. 커밋·푸시·amend·머지 0회, PR #72 OPEN 유지.
