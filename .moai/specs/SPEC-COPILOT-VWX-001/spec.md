...
- **ASSUMPTION-69** — **`Absolute Address` 단일값만 있는 파일의 존재. `NEGATIVE 판정(v0.1.6) — 원리적으로 구분 불가함을 실물로 확인`.** `vectorworks_worksheet_absolute_address_only.csv`(Universe/DMX Address 컬럼 없음, Absolute Address만 존재)로 최초로 이 경로가 실제 실행됐다. 확인 결과: (1) `contiguous_512_confirmed`가 기본값 False인 한 여전히 추측하지 않고 `absolute_address_premise_unverified`로 보류한다(REQ-VWX-009 불변). (2) System A/Universe1/Absolute=1과 System B/Universe1/Absolute=1은 **완전히 같은 숫자값**이다 — `abs=(u-1)*512+a` 역산 공식 자체에는 System을 구분할 변수가 없으므로, System 문자가 `fields["system"]`으로 **별도 보존**되지 않으면 Absolute Address 단독으로는 두 레코드를 원리적으로 구분할 수 없다(`classify_and_resolve`에 `contiguous_512_confirmed=True`를 강제로 주입해 두 레코드가 동일한 `(universe, address)`로 역산됨을 직접 확인, `test_vwx_multisystem_real_samples.py::TestPerSystemAbsoluteAddressAmbiguity`). 결함 1(P0) 수정이 도입한 `(system, universe, address)` 아이덴티티 확장이 정확히 이 문제의 구조적 해법이다 — System을 주소 해석과 별개의 축으로 보존해야만 해소된다.
