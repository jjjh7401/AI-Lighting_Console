...
status: draft (v0.1.2, 2026-08-05) · Tier L · AC 26건 계획. 본 문서는 spec.md의 요구를 관측 가능한 검증 기준으로 전개한다.
...
- 기대 결과:
  ...
  - ③ **(v0.1.2 — run-phase 결함 2 잔존 교정)** 판독이 패치 출처로 성립하지 않은 경우(`not_patch_source` 또는 `worksheet_block_undetected`가 `read_failures`에 존재), `diffs`는 `missing_in_console`/`address_collision`/`quantity_mismatch` 빈 배열 3종을 **생략**하고 `{"performed": false, "reason": …}` 형태로만 존재한다 — "찾아봤는데 없다"(빈 배열)와 "애초에 수행하지 않았다"(미수행)를 구조적으로 구별한다. 정상 대조(레코드 존재)에서는 `{"performed": true, ...}` + 기존 3키가 그대로 유지된다(비공허성 — 정상 케이스에서 `performed: true` + 3키가 실제로 존재함을 함께 확인).
...
- 기대 결과:
  ...
  - ③ **(v0.1.2 — run-phase 결함 2 잔존 교정)** `summary_ko()`는 대조가 성립하지 않을 때(위 AC-VWX-022 ③ 조건) **"차이 없음"이라는 문구를 절대 포함하지 않는다** — 대신 거부 사유로 문장을 시작한다(예: "패치 출처로 성립하지 않는다 — …. 대조를 수행하지 않았다."). 정상 대조(레코드 존재, 차이 없음)에서는 "차이 없음" 문구가 그대로 등장한다(비공허성 — 두 경로 모두 실제로 검증해 문자열 검사기가 살아 있음을 증명한다).
