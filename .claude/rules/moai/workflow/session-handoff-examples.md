...
> This is a path-scoped reference file for `session-handoff.md`. It holds illustrative Example sections and the full 4-locale Localization Table extracted from the always-loaded doctrine file to reduce context weight. The core doctrine (6-block skeleton, cut-line markers, the binding per-block summary, Pre-emit self-check, Auto-Memory Integration, Diet Constraints) remains in `session-handoff.md`; the full per-block detail was relocated to the second-level companion `session-handoff-fields.md` § Field-by-Field Specification (full), which this file points at from § Field-by-Field Specification (full) below.

> **Detail companion**: `session-handoff-fields.md` (paths-scoped to the handoff trio) — the full per-block field specification for Blocks 0-6. Split out to keep this file within the 40,000-character rule budget; content moved verbatim.
...
> Moved to `session-handoff-fields.md` § Field-by-Field Specification (full) — the per-block detail for Blocks 0-6: the `mode:` orchestration-seed enum and its directive couplings, the fan-out steering phrase, the two `ultracode` forms, the `source_session_id` environment fallback, and the Block 5 arm-only consequence.
