...
if command -v moai &> /dev/null; then
    printf '%s' "$INPUT" | moai hook stop-goal 2>>"$MOAI_HOOK_STDERR_LOG"
    exit $?
fi

# Try default ~/go/bin/moai
if [ -f "$HOME/go/bin/moai" ]; then
    printf '%s' "$INPUT" | "$HOME/go/bin/moai" hook stop-goal 2>>"$MOAI_HOOK_STDERR_LOG"
    exit $?
fi

# Try ~/.local/bin/moai (Linux install location)
if [ -f "$HOME/.local/bin/moai" ]; then
    printf '%s' "$INPUT" | "$HOME/.local/bin/moai" hook stop-goal 2>>"$MOAI_HOOK_STDERR_LOG"
    exit $?
fi
