...
> **용어** — 이 문서에서 **`Main` 워크트리**는 `…/Code/AI-Lighting_Console`,
> 즉 리포지터리 **루트** 체크아웃을 가리킨다. 브랜치 `main`과 **다른 것**이며,
> 실제로 `Main` 워크트리는 지금 `main`이 아니라 `jjjh7401/paperwork-p0`에 있다.
> 그 둘을 섞어 읽으면 "메인이 스테일이다"를 브랜치 문제로 오해한다.

| 경로 | 브랜치 | 상태 |
|---|---|---|
| `…/Code/AI-Lighting_Console` — **`Main` 워크트리(루트)** | `jjjh7401/paperwork-p0` | ⚠️ **무관한 미커밋 83건 그대로.** `git add -A` 절대 금지. 브랜치는 **스테일**이며 내용은 #31로 전부 반영됐다(유실 0 — 함께 떨어진 MCP-001 plan 커밋 2건은 `origin/feature/SPEC-COPILOT-MCP-001`에 보존). 체크아웃 중이라 삭제 불가 |
...
`spec-feasibility` 워크트리는 제거됐다. `Main` 워크트리(루트)는 미커밋 83건이라 거기서 리베이스할 수
...
5. **⚠️ `Main` 워크트리(루트)가 더러우면 거기서 리베이스할 수 없다.** 남의 미커밋 83건을
