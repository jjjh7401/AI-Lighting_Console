...
### 대조군 7건 (t33, 2026-08-23) — 경계가 어디인지 재서 고정한다

「6회 막혔다」는 빈도이지 경계가 아니다. 어느 조건에서 막히고 어느 조건에서 통과하는지를
변수 하나씩 바꿔 쟀다. 세션 프로젝트 루트는 **세션이 시작된 디렉터리**다.

| # | 세션 프로젝트 루트 | 도구 · 대상 | 결과 |
|---|---|---|---|
| 1 | `orca/…/LX-SEQ` | Write · 워크트리 t33 안 (상대 경로) | ❌ 거부 |
| 2 | `orca/…/LX-SEQ` | Write · 워크트리 t33 안 (절대 경로) | ❌ 거부 |
| 3 | `orca/…/LX-SEQ` | Write · 스크래치패드 | ❌ 거부 |
| 4 | `orca/…/LX-SEQ` | **Read** · 워크트리 t33 안 | ✅ 통과 |
| 5 | 워크트리 t33 (헤드리스) | Write · `/tmp` | ❌ 거부 |
| 6 | 워크트리 t33 (헤드리스) | Write · `/private/tmp/…` + **`--add-dir`** | ❌ 거부 |
| 7 | 워크트리 t33 (헤드리스) | Write · **자기 워크트리 안** | ✅ 통과 |

**7 이 양성 대조군이다.** 워크트리 안에서 **시작한** 세션은 그 워크트리에 쓴다. 막히는 것은
워크트리라서가 아니라 **거기서 시작하지 않아서**다.

기전이 이걸로 확정된다. 이 세션의 시작 디렉터리와 이동해 간 워크트리는 **같은 저장소의
형제 워크트리**다 — 부모/자식이 아니라 파일시스템상 완전히 다른 가지다.

```
$ cat /Users/studiox/orca/workspaces/AI-Lighting_Console/LX-SEQ/.git
gitdir: /…/AI-Lighting_Console/.git/worktrees/LX-SEQ
$ cat /…/AI-Lighting_Console/.claude/worktrees/t33/.git
gitdir: /…/AI-Lighting_Console/.git/worktrees/t33
```

`EnterWorktree` 는 cwd 를 옮기지만 프로젝트 루트는 시작 지점에 남는다. 그래서 **런타임이
자기 쓰기 가드가 금지하는 자리로 세션을 옮겨 놓는다.** 가드가 과잉인 것도, 도구 사용법이
틀린 것도 아니다 — 두 구성요소가 「프로젝트 경계」에 대해 서로 다른 답을 갖고 있다.

### `--add-dir` 는 이 제한을 풀지 못한다 (음성 결과)

`claude --add-dir <directories...>` (v2.1.241) 는 "Additional directories to allow tool
access to" 라고 안내하지만, 위 5·6 에서 **같은 메시지로 거부됐다.** `/tmp` 가 macOS 에서
심링크인 교란을 배제하려고 `/private/tmp` 실경로로 한 번 더 쟀고 결과는 같았다.

이전 판(v1)은 "설정으로 못 푼다 — `additionalDirectories` 류 키가 없다(grep 0건)"라고
적었다. 그 grep 은 **설정 파일만** 본 것이라 CLI 플래그를 못 봤다. 플래그는 실재하지만
**듣지 않는다** — 결론은 같고 근거가 달라졌다. 범위를 좁게 잡은 grep 으로 "없다"를
말하면, 있는데 안 듣는 것과 아예 없는 것을 구별하지 못한다.

### 그래서 지금 할 수 있는 것 — 상류를 기다리지 않아도 된다

**워크트리 안에서 세션을 시작하면 이 마찰이 없다**(7). `EnterWorktree` 로 형제 워크트리에서
이동해 들어가는 경로가 문제를 만든다. 조작자가 `moai cc -w <name>` 처럼 워크트리를 세션의
시작 지점으로 삼으면 `Write` · `Edit` 이 그대로 동작할 것으로 예측된다 — 7 이 그 예측의
근거이고, **아직 실제 `moai cc -w` 세션으로는 확인하지 않았다**(미검증).
