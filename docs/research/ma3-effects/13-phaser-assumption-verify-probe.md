...
## 확인 기록 (사후 추가, 2026-08-19)

**A1·A2(recall 적재)는 사용자 육안 확인으로 종결됐다.** 사용자가 앱에 "Drop Slam
쳐줘"를 실행해 조명이 페이저로 재생됨을 onPC 앞에서 직접 관측했다("실행이 잘돼",
2026-08-19). 이 관측이 확정하는 것:

- **A1**: 저장 프리셋(All 1 21.21)은 진짜 멀티스텝 페이저다 — 재생됐으므로.
- **A2(적재)**: `Fixture … ; At Preset 21.21` recall은 페이저를 통째로 싣는다.
- §A2.2의 MEMORYFOOTPRINT 부정 정황(1741 < 2104)은 **recall→재저장 경로의 저장
  크기**에 관한 것이었고, 라이브 재생과는 별개임이 판명됐다 — 그 필드가 무엇을
  재는지는 여전히 미확인(재저장 fidelity는 우리 기능이 쓰지 않는 경로라 종결
  불요).

**여전히 열려 있는 것**: A2(큐 보존 — 참조 vs 평탄화, 곡 큐 재생 육안 확인 필요),
A3(Rectangle 하드컷 — "Chase RB 쳐줘" 관측 대기).
