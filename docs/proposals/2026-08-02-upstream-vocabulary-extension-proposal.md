...
1. **표제 문장이 붙는다**: `find_scene("파란 백라이트가 천천히 웨이브하는 씬 만들어줘")` →
   ...
   그 사유까지 없애려면 조합 씬 자산 추가가 별도 항목이 된다.
...
5. 뮤테이션: `하는` 항목 제거 시 `웨이브하는` 매칭 테스트가 죽는다 / `파란` 제거 시 그 슬롯을
   지키는 테스트가 죽는다. ⚠️ **정정(2026-08-02, 하단 §7)**: 초안의 "`파란` 별칭 제거 시 표제 문장
   테스트가 죽는다"는 **메커니즘이 틀렸다** — 표제 문장의 look 축은 룩 별칭이 아니라
   `server/scene/library/core.yaml`의 씬 무드 키워드에서 온다.

## 5. 착수 절차 (다음 세션)

```bash
# 1. 상태 확인 (SCENE §0 킥오프 킷의 4종 그대로)
git log --oneline -3          # 4598c36 … 85611a1 … e4bc78e
...
5. 뮤테이션 실측: 아래 수치는 **머지 전 리뷰가 재현 불가로 판정했고 재측정으로 정정됐다 —
   정본은 §7이다.** 초안 기록(`하는` 제거 → 5 failed / `파란`(ballad) 제거 → 6 failed)은
   측정 대상과 원복 범위가 뒤섞인 값이었다. ⚠️
...
## 7. 정정 — §4.5·§6-5의 뮤테이션 기록 (2026-08-02, 머지 전 리뷰 이후)

**append-only 규율에 따라 위 기록을 지우지 않고 여기에 정정을 덧붙인다.** 독립 리뷰어
(ReviewGateAndVocab)가 `761f01e`에서 §6-5의 두 수치가 **재현되지 않는다**고 보고했고,
코디네이터가 `9c54cd1`(리뷰 소인 후)에서 전체 스위트로 **직접 재측정**해 확인했다.

### 무엇이 틀렸나

1. **수치**: `파란`(ballad 별칭) 제거는 6 failed가 아니다. `5+1=6`은 앞선 `하는` 뮤턴트를
   원복하지 않은 채 누적된 값으로 보인다 — 즉 **한 번에 하나만 주입한다는 절차를 지키지 못한
   기록**이다.
2. **메커니즘**: §4.5는 "`파란` 별칭 제거 → 표제 문장 테스트 사망"이라고 적었으나 **거짓**이다.
   표제 문장의 look 축은 `match_scene`이 **씬 자산 자기 항목**에서 세운다
   (`server/scene/matching.py` `_terms_for`). 룩 별칭은 그 경로에 없다.

### 재측정 정본 (`9c54cd1`, 전체 `pytest server/tests/`, 매 뮤턴트마다 원복 · baseline 3970/5)

| 뮤턴트 | 결과 | 죽은 테스트 |
|---|---|---|
| fx `_ENDINGS`에서 `하는` 제거 (한쪽만) | **6 failed** | 어미 2벌 동치 가드 + `웨이브하는` 계열 |
| fx·scene 양쪽에서 `하는` 제거 | **5 failed** | `웨이브하는` 계열 (동치는 유지되므로 가드는 산다) |
| ballad 별칭 `파란 밤` 제거 | **1 failed** | `test_every_blue_green_entry_has_a_blue_twin_in_the_same_slot` |
| `scene/library/core.yaml`의 `파란` 제거 | **2 failed** | `TestHeadlineSentence` 2건 |

### 승계 교훈

**뮤테이션 기록은 "한 번에 하나 · 매번 원복 · 대상 명시" 없이는 근거가 아니다.** 이 사이클은
그 절차를 지키지 않은 수치에 ✅를 붙여 인수 항목으로 올렸고, 그것을 잡은 것은 스위트가 아니라
**독립 리뷰**였다(교훈 21의 다섯 번째 실증). 특히 **"측정된 것 옆에 측정되지 않은 것을 같은
표기로 두지 말 것"** — 이 SPEC 계열이 트래킹 축에서 세운 규율이 자기 문서에서 깨졌다.
