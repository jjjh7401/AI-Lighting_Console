...
describe("buildAnalysisSummary", () => {
  ...
  it("director_decisions 의 color_usage 축(modulate)을 색 줄에 이어 붙인다 (AC-COLORMODE-009)", () => {
    const timeline: SongTimelineView = {
      ...BASE,
      director_decisions: [
        { step: "Q2B_COLOR_USAGE", axis: "color_usage", value: "modulate", confirmed: true, source: "option" },
      ],
    };
    const summary = buildAnalysisSummary(timeline);
    const colorLine = summary.find((line) => line.label === "색");
    expect(colorLine?.text).toContain("색 운용");
    expect(colorLine?.text).toContain("변조");
  });

  it("color_usage 축(single)을 색 줄에 이어 붙인다 (AC-COLORMODE-009)", () => {
    const timeline: SongTimelineView = {
      ...BASE,
      director_decisions: [
        { step: "Q2B_COLOR_USAGE", axis: "color_usage", value: "single", confirmed: true, source: "option" },
  ...
  it("color_usage 축(per_chorus)을 색 줄에 이어 붙인다 (AC-COLORMODE-009)", () => {
    const timeline: SongTimelineView = {
      ...BASE,
      director_decisions: [
        { step: "Q2B_COLOR_USAGE", axis: "color_usage", value: "per_chorus", confirmed: true, source: "free_text" },
      ],
    };
    const summary = buildAnalysisSummary(timeline);
    const colorLine = summary.find((line) => line.label === "색");
    expect(colorLine?.text).toContain("후렴마다 다른 포인트 색");
  });

  it("color_usage 의 source 가 default_accepted 면 기본값 수용 표시를 덧붙인다 (AC-COLORMODE-010)", () => {
    const timeline: SongTimelineView = {
      ...BASE,
      director_decisions: [
        {
    ...
    const summary = buildAnalysisSummary(timeline);
    const colorLine = summary.find((line) => line.label === "색");
    expect(colorLine?.text).toContain("(기본값 수용)");
  });

  it("color_usage 의 source 가 option 이면 기본값 수용 표시를 붙이지 않는다", () => {
    const timeline: SongTimelineView = {
      ...BASE,
      director_decisions: [
        { step: "Q2B_COLOR_USAGE", axis: "color_usage", value: "modulate", confirmed: true, source: "option" },
    ...
    const summary = buildAnalysisSummary(timeline);
    const colorLine = summary.find((line) => line.label === "색");
    expect(colorLine?.text).not.toContain("기본값 수용");
  });
