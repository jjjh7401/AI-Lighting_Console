...
from rig_data import FIXTURES, FID_BASE, UNIVERSE_PLAN, GROUPS, PRESET_DIM, PRESET_COL, PRESET_POS, PRESET_BM, FX_LIB
...
for uni in {p[2] for p in patch}:
    ...
    for i in range(1, len(spans)):
        if spans[i][0] <= spans[i-1][1]: bad.append(("U%d" % uni, "겹침", spans[i][2]))
    if spans and spans[-1][1] > 512: bad.append(("U%d" % uni, "512 초과", spans[-1][1]))
chk("R2", "주소 무충돌 · 512ch 이내", not bad, str(bad[:3]) if bad else
    " · ".join("U%d %dch" % (u, max(p[4] for p in patch if p[2]==u)) for u in sorted({p[2] for p in patch})))

# R3: 풋프린트 산술 (그룹 총 ch = ch × 수량)
errs = [g for g in FIX if FIX[g][3] > 0 and
        sum(p[4]-p[3]+1 for p in patch if p[1]==g) != FIX[g][3]*FIX[g][4]]
...
chk("R4", "DMX 장비 유니버스 배치 완전성", not miss, str(miss) if miss else "%d개 DMX 그룹 전부 배치" % len(dmx))
...
chk("R5", "GROUP 정의 유효성", not badg, str(badg) if badg else "%d그룹 (기본 %d + 파생 %d)" % (len(gnames), len(gnames&base), len(gnames&derived)))
...
chk("R7", "Sugar CUE-EX 그룹 ⊆ RIG GROUP", not badsg,
    str(badsg) if badsg else "%d그룹 매핑 (LED-W는 영상팀 관할 예외)" % len(song_groups))

# R8: 곡 파일 프리셋 참조가 RIG 프리셋에 존재 (COL/POS/BM/FX)
rig_pids = set(pids)
badp = sorted({v for r in exec_data.CUE_EX for v in (r[3], r[4], r[5], r[6])
               if v not in ("", "OFF") and v not in rig_pids})
chk("R8", "Sugar 프리셋 참조 ⊆ RIG 프리셋", not badp, str(badp) if badp else "곡 파일 참조 전부 RIG에 정의")
