...
# DMX 미사용 장비 (FOLLOW)
manual_rows = [
    (FID_BASE[f[0]], f[0], f[1], f[2], "—", "—", "—", "수동 운용", f[5])
    for f in FIXTURES
    if f[3] == 0
]
    ...
    for row in rows:
        for i, v in enumerate(row, start=1):
            ...
            c.alignment = Alignment(
                vertical="center", wrap_text=True, horizontal="center" if i in mono_cols else "left"
            )
...
sheet(
    "RIG-HEAD",
    "LX-SEQ v2.1 — RIG 팩 (쇼 단위 콘솔 기본설정) · 곡 큐시트가 참조하는 공통 기반",
    ["항목", "값"],
    RIG_HEAD,
    [16, 96],
    mono_cols=(1,),
...
sheet(
    "FIXTURE",
    "장비 인벤토리 — 실제 공연 표준 기종 제안 · 풋프린트=제조사 DMX 차트 실측 (보유/대여 확정 필요)",
    ["Group", "기종(실제 제안)", "Mode", "ch", "수량", "리깅 위치", "회로", "용도", "Sugar"],
    FIXTURES,
    [10, 20, 10, 6, 6, 18, 6, 26, 8],
    mono_cols=(1, 3, 4, 5, 7),
)
...
ws = sheet(
    "PATCH",
    "패치 — 픽스처 단위 (주소 자동 계산 초안 · 총 %d대 · %dch)" % (len(patch_rows), total_ch),
    ["FID", "Group", "기종", "Mode", "ch", "Uni", "Addr", "주소 범위", "리깅 위치"],
    patch_rows + manual_rows,
    [7, 10, 20, 10, 6, 6, 7, 15, 18],
    mono_cols=(1, 2, 4, 5, 6, 7, 8),
)
usage_txt = "  |  ".join("U%d: %d/512ch" % (u, m) for u, m in sorted(uni_usage.items()))
ws.cell(2, 1, "유니버스 사용량 — " + usage_txt + "  (여유분은 예비·추가 장비용)").font = Font(
    name="맑은 고딕", size=9, italic=True, color="4A5568"
)

# 4 GROUP
sheet(
    "GROUP",
    "MA3 그룹 풀 — CUE-EX Fixture Group은 이 이름만 사용",
    ["Group#", "이름", "구성", "용도"],
    GROUPS,
    [8, 12, 26, 24],
    mono_cols=(1, 2),
...
ws.cell(
    r0 - 1, 1, "■ 컬러 프리셋 (연출 팔레트 P.xx와 1:1 · COL 견본색은 곡 파일 HEAD 참조)"
...
for j, row in enumerate(PRESET_COL):
    for i, v in enumerate(row, start=1):
        c = ws.cell(r0 + 1 + j, i, v)
        c.font = MONO if i in (1, 3) else BODY
        c.border = BORDER
...
ws = sheet(
    "PRESET-POS·BM",
    "포지션 프리셋 — 전부 현장 레코드 대상 (가이드 기준으로 레코드)",
    ["ID", "무대 의미", "대상 그룹", "레코드 가이드"],
    PRESET_POS,
    [10, 24, 14, 46],
    mono_cols=(1, 3),
)
...
ws = sheet(
    "FX",
    "FX(Phaser) 라이브러리 — Rate는 SpeedMaster 1(곡 BPM) 종속 · 공식: SongBPM÷사이클박수",
    ["ID", "이름", "어트리뷰트", "파형·스텝", "기본 Rate", "Width", "Phase", "비고"],
    FX_LIB,
    [8, 16, 10, 16, 13, 7, 9, 34],
    mono_cols=(1, 2, 5, 6, 7),
)

# 8 CONSOLE
ws = sheet("CONSOLE", "grandMA3 콘솔 기본설정", ["항목", "설정"], CONSOLE_SETUP, [14, 96])
r0 = 3 + len(CONSOLE_SETUP) + 3
ws.cell(r0 - 1, 1, "■ 이그제큐터 레이아웃 (Page 1)").font = Font(
...
OUT_DIR = os.environ.get("LXSEQ_RIG_OUT") or os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "02_RIG팩"
)
...
write_csv(
    "patch",
    ["FID", "Group", "FixtureType", "Mode", "Ch", "Universe", "Address", "AddrRange", "Position"],
    patch_rows,
)
...
write_csv(
    "fx", ["ID", "Name", "Attribute", "WaveSteps", "BaseRate", "Width", "Phase", "Note"], FX_LIB
)
