...
SPEC_HDR = ["Q#","Section","TC In","TC Out","Dur","Mood","Color(주/보조)","Intensity",
            "Fixture Group","Movement","Effect","Transition","Fade","Note"]
SEC_ENUM = {"PRE-ROLL","AMBI","INTRO","VERSE1","VERSE2","VERSE3","PRE1","PRE2","PRE3",
            "CHORUS1","CHORUS2","CHORUS3","CHORUS4","BRIDGE","BREAK","DROP","SOLO",
            "OUTRO","TAG"}  # v1.1
...
chk(3, "Section enum 준수 (v1.1)", not bad, ("이탈: " + ", ".join(bad)) if bad else "10개 섹션 전부 정의값 (PRE1·PRE2 포함)")
# 4
tb = [r[0] for r in rows if r[11] not in TRANS_ENUM]
fb = [r[0] for r in rows if (r[11] in ("SNAP", "BUMP")) != (float(r[12]) == 0.0)]
chk(4, "Transition enum + Fade 정합", not tb and not fb,
...
ovl = [rows[i][0] for i in range(len(rows) - 1)
       if rows[i][3] and secs(rows[i][3]) > tins[i + 1] + 1e-9]
chk(5, "TC In 단조증가 · 구간 무중첩", mono and not ovl,
...
chk(6, "Fixture Group 약칭 HEAD 정의", not undef, ("미정의: %s" % undef) if undef else "%d개 약칭 전부 정의됨" % len(used))
# 7
bad7 = []
for r in rows:
    for tok in [t.strip() for t in r[6].split("/")]:
        ...
        if not m or m.group(1) not in PAL_ID or m.group(2) != PAL_NM[m.group(1)]:
            bad7.append((r[0], tok))
chk(7, "Color = 팔레트ID + 한글색상명 병기", not bad7, ("위반 %s" % bad7[:3]) if bad7 else "18큐 전부 병기")
# 8
...
chk(8, "전환 밀도 ≤ 8초/1큐 (§3.6.5 예외)", len(viol) == len(term),
    "최소간격 %.1fs · 위반 %d건(종료암전 예외 %d건)" % (min(g[2] for g in gaps), len(viol), len(term)))
# 9
last = rows[-1]
chk(9, "마지막 큐 = 암전/인계 명시", "블랙아웃" in last[6] and last[7] == "0" and "인계" in last[13],
...
chk(10, "확인필요 항목 NOTE 기록", len(nrows) >= 10 and any("확인필요" == r[0] for r in nrows),
    "NOTE %d건 (확인필요 %d · 장비이슈 %d · 스펙개정후보 %d)" % (
        len(nrows), sum(1 for r in nrows if r[0] == "확인필요"),
...
chk(11, "TC_METHOD 기입 · DERIVED 경고", grade in ("VERIFIED", "DERIVED") and (grade == "VERIFIED" or warned),
...
ex_qs = []
for r_ in CUE_EX:
    if r_[0] not in ex_qs: ex_qs.append(r_[0])
...
for r_ in CUE_EX:
    ...
    if is_snap_cue and dim and dim != "0" and snap != "Y":
        ...
        if float(r_[10] or 0) == 0.0: pass
        else: mismatch.append((q, grp, "SNAP큐 점등행 페이드>0"))
...
for q in cue_qs:
    for r_ in CUE_EX:
        if r_[0] != q: continue
        dim = r_[2]
        if dim in ("", None): continue
        if r_[1] == "ALL":
            for g_ in allg: state[g_] = int(dim)
        else:
            state[r_[1]] = int(dim)
    inten = cue_by_q[q][7]
...
chk(12, "CUE-EX 커버리지 · CUE 정합", not miss_q and not extra_q and not mismatch,
    ("누락 %s / 잉여 %s / 모순 %s" % (miss_q, extra_q, mismatch[:3])) if (miss_q or extra_q or mismatch)
...
for r_ in CUE_EX:
    for idx, typ in ((3,"COL"),(4,"POS"),(5,"BM"),(6,"FX")):
        v = r_[idx]
        if v in ("", "OFF", None): continue
        if v not in pids: badref.append((r_[0], r_[1], v))
chk(13, "PRESET 참조 무결성", not badref,
...
chk(14, "CUE-EX Group PATCH 정의", not badgrp,
    ("미정의 그룹 %s" % badgrp) if badgrp else "%d그룹 전부 PATCH 정의" % len({r_[1] for r_ in CUE_EX}))

# 15: FX-Rate BPM 공식
VALID_RATES = {SONG_BPM/0.5, SONG_BPM/1, SONG_BPM/4, SONG_BPM/8, SONG_BPM/16}  # 240,120,30,15,7.5
badrate = []
for r_ in CUE_EX:
    if r_[6] in ("", "OFF", None): continue
    rate = r_[7]
    if rate in ("", None): badrate.append((r_[0], r_[1], "Rate 누락")); continue
    if float(rate) not in VALID_RATES: badrate.append((r_[0], r_[1], rate))
chk(15, "FX-Rate BPM 공식 (§11.3)", not badrate,
