...
ps.cell(1, 1, "PATCH — 장비 패치 (기종·주소 전부 가정값, 현장 확정 필요)").font = Font(name="맑은 고딕", size=11, bold=True, color="1F4E3D")
header(ps, 3, ["Group", "Fixture Type", "Mode", "Count", "Fixture ID", "Universe.Address", "Note"])
r = 4
for row in PATCH:
    for i, v in enumerate(row, start=1):
        c = ps.cell(r, i, v)
        c.font = MONO if i in (1, 5, 6) else BODY; c.border = BORDER
        if str(v) == "확인필요":
...
qs.cell(1, 1, "PRESET — 프리셋 정의 (POS는 현장 레코드 대상 · COL은 연출 팔레트 P.xx와 1:1)").font = Font(name="맑은 고딕", size=11, bold=True, color="1F4E3D")
...
for row in PRESETS:
    ...
    for i, v in enumerate(row, start=1):
        c = qs.cell(r, i, v)
        c.font = MONO if i == 1 else BODY; c.border = BORDER
...
xs.cell(1, 1, "CUE-EX — 큐 × 그룹별 파라미터·타이밍 (long format · 빈칸=트래킹 · 타깃 grandMA3)").font = Font(name="맑은 고딕", size=11, bold=True, color="1F4E3D")
xs.cell(2, 1, "FX-Rate는 BPM 단위, 곡 120BPM 기준: @1/8=240 · @1beat=120 · @1bar=30 · @2bar=15 · @4bar=7.5  |  Speed Master 120BPM 종속 권장").font = Font(name="맑은 고딕", size=9, italic=True, color="4A5568")
HR = 4
...
for row in CUE_EX:
    q = row[0]
    if q not in qseen: qseen.append(q)
    ...
    for i, v in enumerate(row, start=1):
        ...
        c.alignment = Alignment(vertical="center", horizontal="center" if i < 17 else "left",
                                wrap_text=(i == 17))
        if i == 16 and v == "Y":
