...
SEC_FILLS = {
    "INTRO": "FFF2DC", "VERSE1": "FFF8EC", "VERSE2": "FFF8EC",
    "PRE1": "E4F8F8", "PRE2": "E4F8F8",
    "CHORUS1": "FFE2F0", "CHORUS2": "FFE2F0", "CHORUS3": "FFD2E8",
    "BRIDGE": "EAE2FA", "OUTRO": "EDEFF2",
}
...
ws.title = "HEAD"
r = 1
ws.cell(r, 1, "LX-SEQ v2.0 — 조명연출 시퀀스 큐시트").font = Font(name="맑은 고딕", size=14, bold=True, color="2F3B52")
r += 2
ws.cell(r, 1, "■ 곡 · 공연 정보").font = Font(name="맑은 고딕", size=10, bold=True)
r += 1
for k, v in HEAD_META:
    c1 = ws.cell(r, 1, k); c1.font = Font(name="Consolas", size=9, bold=True); c1.fill = PatternFill("solid", fgColor="EDEFF2"); c1.border = BORDER
    c2 = ws.cell(r, 2, v); c2.font = BODY_FONT; c2.border = BORDER; c2.alignment = Alignment(wrap_text=True, vertical="center")
    ws.merge_cells(start_row=r, start_column=2, end_row=r, end_column=5)
    r += 1

r += 1
ws.cell(r, 1, "■ 장비 그룹 정의 (FIXTURE_GROUP) — CUE 시트는 이 약칭만 사용").font = Font(name="맑은 고딕", size=10, bold=True)
r += 1
for i, h in enumerate(["약칭", "정의", "기종·대수", "비고"], start=1):
    c = ws.cell(r, i, h); c.font = HDR_FONT; c.fill = HDR_FILL; c.border = BORDER; c.alignment = Alignment(horizontal="center")
r += 1
for row in FIXTURE_GROUPS:
    for i, v in enumerate(row, start=1):
        c = ws.cell(r, i, v); c.font = MONO_FONT if i == 1 else BODY_FONT; c.border = BORDER
    r += 1

r += 1
ws.cell(r, 1, "■ 색 팔레트 (PALETTE) — CUE 시트 Color 열은 'ID 한글색상명' 병기 필수").font = Font(name="맑은 고딕", size=10, bold=True)
r += 1
...
for pid, name, ref, hexv, use in PALETTE:
    ...
    sw = ws.cell(r, 4, hexv); sw.fill = PatternFill("solid", fgColor=hexv.lstrip("#"))
...
HEADERS = ["Q#", "Section", "TC In", "TC Out", "Dur", "Mood", "Color(주/보조)", "Intensity",
           "Fixture Group", "Movement", "Effect", "Transition", "Fade", "Note"]
cs.cell(1, 1, "Maroon 5 — Sugar  |  03:56.0  |  120BPM 4/4  |  LX-SEQ v2.1  |  r3").font = Font(name="맑은 고딕", size=11, bold=True, color="2F3B52")
cs.merge_cells(start_row=1, start_column=1, end_row=1, end_column=14)
cs.cell(2, 1, "TC_ORIGIN: 00:00.0 = 곡 첫 음 (카운트인 없음)   |   TC_METHOD: DERIVED — 리허설 LTC 대조 필수").font = Font(name="맑은 고딕", size=9, italic=True, color="B03030")
...
for cue in CUES:
    ...
    for i, v in enumerate(vals, start=1):
        ...
        c.fill = fill
        c.border = BORDER
        c.alignment = Alignment(vertical="center", wrap_text=(i in (6, 7, 9, 14)),
...
ns.cell(1, 1, "NOTE — 확인 필요 항목 · 변경 이력 · 스펙 개정 후보").font = Font(name="맑은 고딕", size=11, bold=True, color="2F3B52")
for i, h in enumerate(["구분", "대상 Q#", "내용", "기록일", "상태"], start=1):
    c = ns.cell(3, i, h); c.font = HDR_FONT; c.fill = HDR_FILL; c.border = BORDER; c.alignment = Alignment(horizontal="center")
GUBUN_COLOR = {"확인필요": "FFE0E0", "장비이슈": "FFF0D8", "스펙개정후보": "E2ECFF", "리허설변경": "E8F5E0"}
...
for g, q, txt, d, st in NOTES:
    ...
    for i, v in enumerate(vals, start=1):
        ...
        c.alignment = Alignment(vertical="center", wrap_text=(i == 3), horizontal="center" if i in (1, 2, 4, 5) else "left")
        if i == 1:
