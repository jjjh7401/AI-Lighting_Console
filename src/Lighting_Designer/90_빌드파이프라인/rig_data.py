...
RIG_HEAD = [
    ...
    ("REV",         RIG_REV),
    ("DATA_STATUS", "PROPOSED — 실제 공연 표준 기종으로 제안. 풋프린트는 제조사 DMX 차트 실측값. 보유/대여 리스트 확정 필요."),
    ("SONG_FILES",  "LXSEQ_SAMPLE_01_Sugar (r3) — 이 RIG의 그룹·프리셋 체계를 참조"),
]

# 장비 인벤토리: Group | 기종(실제) | Mode | ch(제조사 실측) | 수량 | 리깅 위치 | 회로/전원 | 용도 | Sugar 사용
FIXTURES = [
    ("KEY",     "ETC S4 LED S3 Lustr X8",   "Direct 12ch", 12, 6,  "FOH 브리지",        "L1", "인물 페이스 · 보컬 중심",       "사용"),
    ("FOH",     "ETC S4 LED S3 Lustr X8",   "Direct 12ch", 12, 8,  "FOH 트러스",        "L1", "전면 워시 · 전체 페이스",        "미사용"),
    ("BACK",    "Martin MAC Aura XB",       "Extended 25ch",25, 12, "업스테이지 트러스", "L2", "백라이트 · 실루엣 · Aura FX",   "사용"),
    ("SIDE-L",  "Martin MAC Aura XB",       "Extended 25ch",25, 6,  "하수 붐 (3단×2)",   "L3", "사이드 컬러 · 폭 확장",          "사용"),
    ("SIDE-R",  "Martin MAC Aura XB",       "Extended 25ch",25, 6,  "상수 붐 (3단×2)",   "L3", "사이드 컬러 · 폭 확장",          "사용"),
    ("WASH-U",  "Martin RUSH PAR 2 RGBW Z", "9ch",         9,  10, "업스테이지 플로어", "L4", "업 워시 · 바닥 컬러",            "사용"),
    ("WASH-D",  "Martin RUSH PAR 2 RGBW Z", "9ch",         9,  10, "다운스테이지 플로어","L4", "다운 워시",                     "사용"),
    ("MOVER-U", "Robe MegaPointe",          "Mode 1 39ch", 39, 8,  "업 트러스",         "L5", "에어빔 · 고보 · 프리즘 주력",    "사용"),
    ("MOVER-D", "Robe Spiider",             "Mode 1 49ch", 49, 8,  "다운 트러스",       "L5", "무빙 워시빔 · 플라워 FX",        "사용"),
    ("BLIND",   "Elation CUEPIX Blinder WW2","4ch",        4,  6,  "다운 트러스 전면",  "L6", "2-cell 블라인더 (각도 리허설 확정)","사용"),
    ("STROBE",  "Martin Atomic 3000 LED",   "Extended 14ch",14, 4,  "업 트러스",         "L6", "스트로브 + Aura 백라이트 (광과민 고지)","사용"),
    ("HAZE",    "Look Unique 2.1",          "2ch",         2,  2,  "무대 양측 플로어",  "L7", "펌프/팬 분리 제어 · 빔 가시화",   "사용"),
    ("FOLLOW-1","Robert Juliat 1.2kW",      "수동",        0,  1,  "FOH 부스",          "L8", "팔로우 (수동 운용 · DMX 없음)",   "미사용"),
]

# FID 시작 번호 (기존 곡 파일과 일치)
FID_BASE = {
    "KEY": 101, "FOH": 111, "BACK": 201, "SIDE-L": 301, "SIDE-R": 311,
    "WASH-U": 401, "WASH-D": 421, "MOVER-U": 501, "MOVER-D": 521,
    "BLIND": 601, "STROBE": 611, "HAZE": 621, "FOLLOW-1": 631,
}
...
PRESET_POS = [
    ...
    ("POS.05", "팬아웃 종점 (객석 상단)",   "MOVER-ALL", "객석 상단 45° · 직사 금지선 위 · 좌우 부채꼴"),
...
FX_LIB = [
    ("FX.01", "DIM-CHASE",      "Dimmer",   "2-step 100/0",      "240 (@1/8)",  "50%", "0..360",  "런 체이스 · ODD/EVEN 분리 가능"),
    ...
    ("FX.06", "SHUTTER-STROBE", "Shutter",  "Strobe",            "15 (@2bar)",  "—",   "0",       "광과민 고지 필수 · Rate 상한 15"),
...
RIG_NOTES = [
    ("리허설변경", "FIXTURE 전체", "r3: lighting-designer 플러그인 파이프라인 재생성판. 풋프린트 제조사 DMX 차트 재확인 완료 (Robe 공식 DMX 차트: MegaPointe M1=39ch·Spiider M1=49ch / Martin: Aura XB Ext=25ch·Atomic 3000 LED Ext=14ch·RUSH PAR 2=9ch).", "2026-08-21", "해결(r3)"),
    ("리허설변경", "FIXTURE 전체", "r2: 가정 기종 → 실제 공연 표준 기종으로 교체 (S4 LED S3 Lustr X8 · MAC Aura XB · RUSH PAR 2 · MegaPointe · Spiider · CUEPIX WW2 · Atomic 3000 LED · Unique 2.1). 풋프린트는 제조사 DMX 차트 실측. 전 기종 grandMA3 픽스처 라이브러리/GDTF 존재.", "2026-08-20", "해결(r2)"),
    ("확인필요", "FIXTURE 전체", "제안 기종이며 보유/대여 리스트와 대조 필요. 기종·모드 변경 시 풋프린트만 고치면 PATCH 자동 재계산.", "2026-08-20", "미해결"),
    ("확인필요", "PATCH", "Universe 배치·주소는 자동 계산 초안 (U1–U5, U6 예비). 현장 노드 구성·케이블 런 확정 후 조정.", "2026-08-20", "미해결"),
    ("확인필요", "POS.01~08", "포지션 프리셋 전부 현장 레코드 필요 — 레코드 가이드 열 기준.", "2026-08-20", "미해결"),
    ("확인필요", "CONSOLE", "grandMA3 버전·sACN 노드·TC 소스 기기 미확정.", "2026-08-20", "미해결"),
    ("장비이슈", "STROBE·BLIND", "Inhibit 페이더 상시 배치 (Exec 104·105) — 운용 안전 규칙.", "2026-08-20", "미해결"),
]
